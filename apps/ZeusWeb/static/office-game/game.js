...
function getExt(pngFile) {
  // star-working-spritesheet.png too wide for WebP, always use PNG
...
async function loadMemo() {
  ...
  try {
    const response = await fetch('/v1/memory/files/memory/' + new Date(Date.now() - 86400000).toISOString().split('T')[0] + '.md?t=' + Date.now(), { cache: 'no-store' });
...
function updateLoadingProgress() {
  ...
  if (loadingText) {
    loadingText.textContent = `Loading Zeus Office... ${percent}%`;
  }
}
...
const BUBBLE_TEXTS = {
  idle: [
    ...
    'Taking a breather',
    'Elegantly efficient today',
    'Patience for precision',
    'Coffee is hot, ideas flowing',
    'Buffing you in the background',
    'Status: recharging',
    'Taking it easy'
  ],
  writing: [
  ...
  error: [
    ...
    'On it: finding root cause',
    'Seen this before, don\'t worry',
    'Alerting: let the issue reveal itself'
  ],
  cat: [
    'Meow~',
    'Purrrr...',
...
// Agent positions per zone (staggered for multiple agents)
...
// Status control bar (for testing)
function setState(state, detail) {
  fetch('/v1/office/state', {
    method: 'POST',
...
function preload() {
  ...
  // New desks: force PNG (transparency)
...
function create() {
  ...
  const plaqueText = game.add.text(plaqueX, plaqueY, 'Zeus Fleet Office', {
  ...
  // === New desks (from LAYOUT, forced PNG) ===
  ...
  // Debug: render test agent only in debug mode
...
function fetchAgents() {
  fetch('/v1/agents?t=' + Date.now(), { cache: 'no-store' })
    .then(response => response.json())
    .then(data => {
      if (!Array.isArray(data)) return;
      // Reset position counters
      // Assign different positions per zone to avoid overlap
...
function renderAgent(agent) {
  ...
  // Opacity (offline/pending/rejected = semi-transparent)
  let alpha = 1;
  if (authStatus === 'pending') alpha = agent._trust != null ? agent._trust : 0.7;
  ...
  if (!agents[agentId]) {
    ...
    // Current task label
    const taskText = agent.current_task || agent.currentTask || '';
    const taskLabel = game.add.text(0, 22, taskText ? '📋 ' + taskText : '', {
      fontFamily: 'ArkPixel, monospace',
      fontSize: '10px',
      fill: '#94a3b8',
      stroke: '#000',
    ...
    container.add([starIcon, statusDot, nameTag, taskLabel]);
    agents[agentId] = container;

    // Fetch live reputation for trust/alpha
    fetch('/v1/marketplace/reputation/' + agentId)
      .then(r => r.ok ? r.json() : null)
      .then(data => { if (data && data.trust != null) agent._trust = data.trust; })
      .catch(() => {});
  } else {
    // Update agent
    ...
    // Update task label
    const taskLabel = container.list ? container.list.find(c => c.name === 'taskLabel') : null;
    if (taskLabel) {
      const taskText = agent.current_task || agent.currentTask || '';
      taskLabel.setText(taskText ? '📋 ' + taskText : '');
    }
    // Refresh reputation periodically
    if (!agent._trustFetched || Date.now() - agent._trustFetched > 10000) {
      agent._trustFetched = Date.now();
      fetch('/v1/marketplace/reputation/' + agentId)
        .then(r => r.ok ? r.json() : null)
        .then(data => { if (data && data.trust != null) agent._trust = data.trust; })
        .catch(() => {});
