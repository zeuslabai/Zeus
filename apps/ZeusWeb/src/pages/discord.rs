...
pub fn DiscordPage() -> impl IntoView {
    ...
    spawn_local(async move {
        if let Ok(r) = api::fetch_channels().await
            && let Some(ch) = r.channels.iter().find(|c| c.channel_type == "discord" || c.platform == "discord") {
                channel_id.set(ch.id.clone());
                is_connected.set(ch.status == "connected");
                bot_token.set(ch.config["token"].as_str().unwrap_or("").to_string());
            }
        loaded.set(true);
    });
    ...
    let do_save = move |_| {
        ...
        spawn_local(async move {
            ...
            let result = if cid.is_empty() {
                api::create_channel(&api::CreateChannelReq { channel_type: "discord".into(), name: "Discord".into(), config }).await.map(|_| ())
            } else {
    ...
    view! {
        <div style="padding: 32px; max-width: 640px;">
            <div style="display: flex; align-items: center; gap: 16px; margin-bottom: 32px;">
                ...
                <div>
                    <h1 style="font-family: 'Orbitron', monospace; font-size: 14px; letter-spacing: 6px; color: rgba(255,245,240,0.9); margin: 0;">"DISCORD"</h1>
            ...
            <Show when=move || loaded.get()>
                <Card>
                    ...
                    <div>
                        ...
                        <input type="password" placeholder="MTxxxxxxxx.xxxxxx.xxxxx..." style=input_style
                            prop:value=move || bot_token.get()
                            on:input=move |ev| bot_token.set(event_target_value(&ev))
                        />
                        <div style="font-family: 'Rajdhani', sans-serif; font-size: 12px; color: rgba(255,245,240,0.5); margin-top: 4px;">"Discord Developer Portal \u{2192} Bot \u{2192} Token"</div>
                    </div>
