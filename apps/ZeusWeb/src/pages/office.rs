...
fn AgentCard(
    ...
    let pulse = if matches!(agent.status.to_lowercase().as_str(), "online" | "active" | "healthy") {
    ...
    view! {
        <div
            ...
            on:click=move |_| {
                ...
                if current.as_deref() == Some(&id2) {
                    selected_id.set(None);
                } else {
                    selected_id.set(Some(id2.clone()));
                }
            }
        >
            <div style={move || format!(
                "background: linear-gradient(135deg, rgba(10,15,30,0.95) 0%, rgba(5,10,25,0.98) 100%);\
                 ...
                 box-shadow: 0 0 {}px {}40, inset 0 1px 0 rgba(255,255,255,0.04);\
                 transition: all 0.3s ease; position: relative; overflow: hidden;",
                if is_selected() { format!("{}cc", sc) } else { format!("{}33", sc) },
                if is_selected() { "20" } else { "8" },
                sc,
            )}>
                // Scanline overlay
                ...
                <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 10px; position: relative;">
                    <div style={format!(
                        "width: 10px; height: 10px; border-radius: 50%; background: {};\
                         box-shadow: 0 0 10px {}, 0 0 20px {}66; {}",
                        sc, sc, sc, pulse
                    )}></div>
                    <span style="font-family: 'Orbitron', 'JetBrains Mono', monospace; font-size: 12px;\
                        font-weight: 700; color: #e0e8f0; letter-spacing: 1px; text-transform: uppercase;">
                        {name}
                    </span>
                    <span style="margin-left: auto; font-size: 20px;">{re}</span>
                </div>
                ...
                <div style="margin-top: 10px; display: flex; align-items: center; gap: 6px;">
                    ...
                    <div>
                        <div style="font-size: 10px; color: #8899bb; font-family: 'JetBrains Mono', monospace;">
                            {agent.machine.clone()}
                        </div>
                        <div style="font-size: 9px; color: #556688; font-family: 'JetBrains Mono', monospace;">
                            {agent.ip.clone()}
                        </div>
                    </div>
                </div>
                ...
                // Role
                <div style="margin-top: 6px; font-size: 9px; color: #7788aa;\
                    font-family: 'Orbitron', monospace; text-transform: uppercase; letter-spacing: 1.5px;">
                    {agent.role.clone()}
                </div>

                // Load + Health bars
                <div style="margin-top: 10px;">
                    <div style="display: flex; justify-content: space-between; font-size: 8px; color: #556688;\
                        font-family: 'Orbitron', monospace; letter-spacing: 1px; margin-bottom: 3px;">
                        <span>"LOAD"</span>
                        <span>{format!("{}%", load_pct)}</span>
                    </div>
                    <div style="width: 100%; height: 3px; background: #0a1020; border-radius: 2px; overflow: hidden;">
                        <div style={format!(
                            "width: {}%; height: 100%; background: linear-gradient(90deg, {}66, {}); border-radius: 2px;\
                             box-shadow: 0 0 6px {}44; transition: width 0.5s ease;",
                            load_pct.min(100), sc, sc, sc
                        )}></div>
                    </div>
                </div>
                <div style="margin-top: 5px;">
                    <div style="display: flex; justify-content: space-between; font-size: 8px; color: #556688;\
                        font-family: 'Orbitron', monospace; letter-spacing: 1px; margin-bottom: 3px;">
                        <span>"HP"</span>
                        <span>{format!("{}%", health_pct)}</span>
                    </div>
                    <div style="width: 100%; height: 3px; background: #0a1020; border-radius: 2px; overflow: hidden;">
                        <div style={format!(
                            "width: {}%; height: 100%; background: linear-gradient(90deg, #00ff8866, #00ff88); border-radius: 2px;\
                             box-shadow: 0 0 6px #00ff8844; transition: width 0.5s ease;",
                            health_pct.min(100)
                        )}></div>
                    </div>
                </div>
