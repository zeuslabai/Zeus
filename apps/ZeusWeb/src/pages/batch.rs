...
pub fn BatchPage() -> impl IntoView {
    ...
    let do_lookup = move |_: leptos::ev::MouseEvent| {
        ...
        <div style="padding: 32px; max-width: 1200px; margin: 0 auto; font-family: 'Rajdhani', sans-serif; color: rgba(255,245,240,0.9);">
            ...
            {move || show_create.get().then(|| view! {
                <div style="background: rgba(255,255,255,0.03); border: 1px solid rgba(255,60,20,0.15); border-radius: 12px; padding: 20px; margin-bottom: 20px;">
                    ...
                    >{move || if creating.get() { "Queuing..." } else { "Queue Batch" }}</button>
                </div>
            })}
            ...
            <div style="background: rgba(255,255,255,0.02); border: 1px solid rgba(255,60,20,0.1); border-radius: 10px; padding: 16px; margin-bottom: 20px; display: flex; gap: 10px; align-items: center;">
                <input
                    ...
                    on:keydown=move |e: leptos::ev::KeyboardEvent| { if e.key() == "Enter" { do_lookup(leptos::ev::MouseEvent::new("click").unwrap()); } }
                ...
                <button
                    ...
                    on:click=do_lookup
                    style="padding: 9px 18px; background: rgba(255,60,20,0.1); border: 1px solid rgba(255,60,20,0.3); border-radius: 8px; color: rgba(255,140,80,0.9); font-size: 12px; cursor: pointer;"
                >{move || if looking_up.get() { "Looking up..." } else { "Look Up" }}</button>
            </div>
