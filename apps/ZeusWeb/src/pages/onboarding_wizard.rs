...
fn qr_svg(data: &str) -> String {
    ...
    for (i, color) in colors.iter().enumerate() {
        if *color == Color::Dark {
            ...
            svg.push_str(&format!(
                r##"<rect x="{x}" y="{y}" width="{c}" height="{c}" fill="#ff3c14"/>"##,
                x = x, y = y, c = cell
            ));
        }
    }
...
const FEATURES: &[Feature] = &[
    Feature { id: "cognitive", name: "Nous Cognitive Engine", desc: "Intent recognition, reasoning chains, meta-cognition", default: true },
    Feature { id: "memory", name: "Mnemosyne Memory", desc: "SQLite FTS5, vector embeddings, temporal versioning", default: true },
    Feature { id: "orchestration", name: "Prometheus Orchestration", desc: "Task planner, cooking loop, heartbeat, cron", default: true },
    Feature { id: "browser", name: "Browser Automation", desc: "Chrome CDP — navigate, click, screenshot, JS", default: false },
    Feature { id: "voice", name: "Voice Pipeline", desc: "Twilio calls, Whisper STT, OpenAI TTS", default: false },
    Feature { id: "macos", name: "macOS Automation", desc: "193 Talos tools — Calendar, Notes, Mail, UI", default: true },
    Feature { id: "sandbox", name: "Aegis Security", desc: "Seatbelt sandbox, command filtering, approvals", default: true },
    Feature { id: "mcp", name: "MCP Server", desc: "Model Context Protocol for Claude Code/Desktop", default: true },
    Feature { id: "skills", name: "Skills Runtime", desc: "SKILL.md plugin system", default: false },
    Feature { id: "agora", name: "Agora Marketplace", desc: "Agent skill trading (experimental)", default: false },
];
...
const SECURITY_LEVELS: &[SecurityLevel] = &[
    ...
    SecurityLevel { id: "strict", name: "STRICT", desc: "Full Seatbelt sandbox, mandatory approvals for all shell and web operations. Maximum security.", risk: "Maximum security", color: "#ff3c14", features: &["macOS Seatbelt", "Mandatory approvals", "Process isolation", "Complete audit trail"] },
];
...
impl Default for OnboardConfig {
    fn default() -> Self {
        Self {
            ...
            providers: vec!["anthropic".to_string(), "ollama".to_string()],
        ...
        async move {
            ...
            if let Ok(status) = api::fetch_json::<serde_json::Value>("/v1/onboarding/status").await {
                ...
                if let Some(provider) = status.get("provider").and_then(|v| v.as_str()) {
                    config.update(|c| {
                        if !c.providers.contains(&provider.to_string()) {
                    ...
                    view! {
                        <div style="padding: 18px 32px; display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid rgba(255,60,20,0.1); flex-shrink: 0;">
                            ...
                            <div style="display: flex; align-items: center; gap: 6px;">
                                {(1..=12usize).map(|i| {
                                    let bg = if i < s { "#ff3c14" } else if i == s { "rgba(255,60,20,0.6)" } else { "rgba(255,60,20,0.15)" };
                        ...
                        <div style="padding: 18px 32px; border-top: 1px solid rgba(255,60,20,0.1); display: flex; justify-content: space-between; align-items: center; flex-shrink: 0; background: rgba(5,5,8,0.92); backdrop-filter: blur(16px);">
                            <button
                                ...
                                on:click=move |_| {
                                    ...
                                    // When going back from step 3 (identity), skip step 2 if mode != quickstart
                                    if cur == 3 && config.get_untracked().onboarding_mode != "quickstart" {
                                        step.set(1);
                                    } else {
                            ...
                            <button
                                style=move || {
                                    ...
                                    format!(
                                        "font-family: 'Orbitron', monospace; font-size: 10px; letter-spacing: 3px; text-transform: uppercase; background: rgba(255,60,20,0.12); border: 1px solid rgba(255,60,20,0.25); color: #ff3c14; padding: 12px 28px; border-radius: 10px; cursor: {}; transition: all 0.3s; display: inline-flex; align-items: center; gap: 8px; opacity: {}; box-shadow: 0 0 30px rgba(255,60,20,0.15);",
                                ...
                                on:click=move |_| {
                                    let cur = step.get_untracked();
                                    // When advancing from step 1 (mode), skip step 2 (quickstart config) if mode != quickstart
                                    if cur == 1 && config.get_untracked().onboarding_mode != "quickstart" {
                                        step.set(3);
                                    } else {
                                        step.update(|v| *v += 1);
                                    }
    ...
    view! {
        <div style="position: relative; width: 42px; height: 42px;">
            <svg width="42" height="42" viewBox="0 0 42 42">
                ...
                <circle cx="21" cy="21" r="18" fill="none" stroke="#ff3c14" stroke-width="2.5"
                ...
                <Show when=move || { phase.get() >= 3 }>
                    <FadeIn delay_ms=1000>
                        ...
                        <div style="margin-top: 24px;">
                            <button
                                style="font-family: 'Orbitron', monospace; font-size: 10px; letter-spacing: 3px; text-transform: uppercase; background: rgba(255,60,20,0.12); border: 1px solid rgba(255,60,20,0.25); color: #ff3c14; padding: 12px 28px; border-radius: 10px; cursor: pointer; transition: all 0.3s; display: inline-flex; align-items: center; gap: 8px; box-shadow: 0 0 30px rgba(255,60,20,0.15);"
    ...
    view! {
        <div class="opacity-0 animate-fade-in">
            <div style="margin-bottom: 28px;">
                <div class="font-rajdhani text-[22px] font-semibold text-text-z-text">
                    <TypeWriter text="How would you like to set up Zeus?" speed_ms=40 />
                </div>
                <FadeIn delay_ms=800>
                    <p class="text-sm text-text-z-text/70 mt-2.5 leading-[1.7]">
                            ...
                            <div class="flex items-center gap-[14px]">
                                <div style=move || {
                                    ...
                                    format!(
                                        ...
                                        if selected { "#ff3c14" } else { "rgba(255,60,20,0.2)" }
                                    )
                                }>
                                    {move || {
                                        let selected = config.get().onboarding_mode == id_inner;
                                        if selected {
                                            view! { <div style="width: 8px; height: 8px; border-radius: 50%; background: #ff3c14;" /> }.into_any()
                                        } else {
                                            view! { <div /> }.into_any()
                                        }
                                    }}
    ...
    view! {
        <div class="opacity-0 animate-fade-in">
            ...
            <div class="mb-4">
                ...
                <input
                    class="w-full box-border px-4 py-3 bg-bg-z-surface border border-border-z-border rounded-[10px] text-text-z-text font-rajdhani text-[15px] outline-none transition-all duration-300 focus:border-border-border-z-border-active"
                    type="text"
            ...
            // Org input
            <div class="mb-4">
                <label class="font-orbitron text-[10px] tracking-[3px] text-text-z-text/70 block mb-2 uppercase">"Organization"</label>
                <input
            ...
            // Gateway URL
            <div class="mb-4">
                <label class="font-orbitron text-[10px] tracking-[3px] text-text-z-text/70 block mb-2 uppercase">"Gateway URL"</label>
                <input
                    style="width: 100%; padding: 12px 16px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,60,20,0.1); border-radius: 10px; color: rgba(255,245,240,0.9); font-family: 'Rajdhani', sans-serif; font-size: 15px; outline: none; box-sizing: border-box; transition: all 0.3s;"
                    type="text"
                            ...
                            >
                                <div style="display: flex; align-items: flex-start; gap: 12px;">
                                    <div style=move || format!(
                                        ...
                                        if config.get().personality == pid_c2 { "#ff3c14" } else { "rgba(255,60,20,0.1)" }
                                    )>
                                        <Show when=move || config.get().personality == pid_c3>
                                            <div style="width: 6px; height: 6px; border-radius: 3px; background: #ff3c14;" />
                                        </Show>
                                    </div>
                                    <div>
...
fn StepIntelligence(config: RwSignal<OnboardConfig>, providers_data: RwSignal<Vec<DynProvider>>, key_status: RwSignal<std::collections::HashMap<String, KeyTestStatus>>) -> impl IntoView {
            ...
            if let Ok(keys) = crate::api::fetch_keys().await {
                ...
                for key in &keys.keys {
                    if key.configured {
                        ...
                        config.update(|c| {
                            if !c.providers.contains(&pid.to_string()) {
    ...
    view! {
        <div class="opacity-0 animate-fade-in">
            <div class="mb-5">
                <div class="font-orbitron text-[22px] font-semibold text-text-z-text">
                    ...
                    view! {
                        <div
                            ...
                            on:click={
                                ...
                                move |_| config.update(|c| {
                                    if c.providers.contains(&pid) {
                                        c.providers.retain(|x| x != &pid);
                                    } else {
                        ...
                        >
                            <div style="display: flex; align-items: center; gap: 10px;">
                                ...
                                <div class="flex-1">
                                    <div style="display: flex; align-items: center; gap: 6px;">
                                        ...
                                        {is_hot.then(|| view! {
                                            <span style="font-family: 'Orbitron', monospace; font-size: 8px; font-weight: 700; letter-spacing: 1.5px; color: #ff3c14; background: rgba(255,60,20,0.20); padding: 3px 8px; border-radius: 4px;">"HOT"</span>
                                        })}
                                        {is_local.then(|| view! {
                                ...
                                <div style=move || format!(
                                    ...
                                    if config.get().providers.contains(&pid_c2) { "#ff3c14" } else { "rgba(255,60,20,0.1)" },
                                    ...
                                    <Show when=move || config.get().providers.contains(&pid_c3)>
                                        <div style="width: 10px; height: 10px; border-radius: 3px; background: #ff3c14;" />
                                    </Show>
                                </div>
                            </div>
                            ...
                            <div
                                ...
                                <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 10px;">
                                    <label style="font-family: 'Orbitron', monospace; font-size: 10px; letter-spacing: 3px; color: rgba(255,245,240,0.7); text-transform: uppercase;">
                                        {move || {
                                            ...
                                            if auth == "oauth_token" { format!("{} CLAUDE TOKEN", name_c) } else { format!("{} API KEY", name_c) }
                                        }}
                                    </label>
                                    <div style="margin-left: auto; display: flex; gap: 4px;">
                                        <button
                                            style=move || format!(
                                                ...
                                                if config.get().auth_types.get(&pid_auth2).cloned().unwrap_or_else(|| "api_key".to_string()) == "api_key" { "#ff3c14" } else { "rgba(255,245,240,0.5)" }
                                            )
                                            on:click={
                                        ...
                                        <button
                                            style=move || format!(
                                                ...
                                                if config.get().auth_types.get(&pid_auth4).cloned().unwrap_or_else(|| "api_key".to_string()) == "oauth_token" { "#ff3c14" } else { "rgba(255,245,240,0.5)" }
                                            )
                                            on:click={
                                ...
                                <div style="display: flex; align-items: center; gap: 8px;">
                                    <input
                                        ...
                                        type="password"
                                        placeholder="Paste API key or Claude token — auto-detected"
                            ...
                            <div
                                ...
                                <label class="font-orbitron text-[10px] tracking-[3px] text-text-z-text/70 block mb-2 uppercase">"OLLAMA ENDPOINT URL"</label>
                                <div style="display: flex; align-items: center; gap: 8px;">
                                    <input
                                        style="flex: 1; padding: 12px 16px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,60,20,0.1); border-radius: 10px; color: rgba(255,245,240,0.9); font-family: 'Orbitron', monospace; font-size: 12px; outline: none; box-sizing: border-box; transition: all 0.3s;"
                                        type="text"
            ...
            <div style="margin-top: 14px; padding: 14px 18px; background: rgba(255,255,255,0.03); border-radius: 12px; border: 1px solid rgba(255,60,20,0.1); display: flex; align-items: center; gap: 14px; transition: all 0.3s ease;">
                <div style="font-family: 'Orbitron', monospace; font-size: 10px; color: #ff3c14; letter-spacing: 3px; font-weight: 700;">
    ...
    view! {
        <div class="opacity-0 animate-fade-in">
            <div class="mb-5">
                ...
                <p class="opacity-0 animate-fade-in" style="font-size: 13px; color: rgba(255,245,240,0.7); margin-top: 8px; line-height: 1.7;">
            ...
            <div class="opacity-0 animate-fade-in flex flex-col gap-1.5">
                {move || {
                    ...
                    let models: Vec<(String, String, String, String)> = provs.iter()
                        ...
                        .flat_map(|p| p.models.iter().map({
                            ...
                            move |m| (format!("{}/{}", pid, m), pname.clone(), pcolor.clone(), pid.clone())
                        }))
                        .collect();
                        ...
                        view! {
                            <div class="flex flex-col gap-1.5">
                                {models.into_iter().map(|(full, provider_name, color, _pid): (String, String, String, String)| {
                                    ...
                                    view! {
                                        <div
                                            class=move || if config.get().default_model == full_c { "zselect-card selected flex items-center gap-[14px]" } else { "zselect-card flex items-center gap-[14px]" }
                                            on:click={
                                                let full = full.clone();
                                        ...
                                        >
                                            ...
                                            <div style=move || format!(
                                                ...
                                                if config.get().default_model == full_c4 { "#ff3c14" } else { "rgba(255,60,20,0.1)" }
                                            )>
                                                <Show when={
                ...
                // Add fallback input
                {
                    ...
                    view! {
                        <div style="display: flex; gap: 8px;">
                            ...
                            />
                            <button
                                style="padding: 10px 16px; background: rgba(255,60,20,0.15); border: 1px solid rgba(255,60,20,0.3); border-radius: 8px; color: #ff3c14; font-family: 'Orbitron', monospace; font-size: 10px; letter-spacing: 1px; cursor: pointer; white-space: nowrap;"
                                on:click=move |_| {
                                    let val = fb_input.get();
                                    if !val.trim().is_empty() {
                                        config.update(|c| c.fallback_models.push(val.trim().to_string()));
                        ...
                        >
                            <div class="flex items-center gap-[14px]">
                                <div style=move || format!(
                                    ...
                                    if config.get().channels.contains(&cid_c2) { "#ff3c14" } else { "rgba(255,245,240,0.5)"
                                    }
                                )>
                                    {icon_text}
                                </div>
                                ...
                                <div style=move || format!(
                                    "width: 22px; height: 22px; border-radius: 6px; flex-shrink: 0; border: 1.5px solid {}; background: {}; display: flex; align-items: center; justify-content: center; transition: all 0.3s;",
                                    if config.get().channels.contains(&cid_c3) { "#ff3c14" } else { "rgba(255,60,20,0.1)" },
                        ...
                        {enabled.into_iter().filter_map(|ch_id| {
                            ...
                            Some(view! {
                                <div style="margin-bottom: 16px; padding: 14px; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px solid rgba(255,60,20,0.08);">
                                    <div style="font-family: 'Orbitron', monospace; font-size: 9px; letter-spacing: 2px; color: #ff3c14; margin-bottom: 10px; text-transform: uppercase;">{ch_label}</div>
                                    ...
                                    {if ch_id_outer == "signal" {
                                        view! {
                                            <div style="margin-top:10px; padding-top:10px; border-top:1px solid rgba(255,60,20,0.08);">
                                                ...
                                                <button
                                                    style="padding:7px 14px; background:rgba(255,60,20,0.12); border:1px solid rgba(255,60,20,0.3); border-radius:6px; color:#ff3c14; font-family:'Rajdhani',sans-serif; font-size:13px; cursor:pointer; transition:all 0.2s;"
                                                    on:click=move |_| {
                        ...
                        view! {
                            ...
                            >
                                <div style="display: flex; align-items: center; gap: 14px;">
                                    <div style=move || format!(
                                        "width: 22px; height: 22px; border-radius: 11px; flex-shrink: 0; border: 2px solid {}; display: flex; align-items: center; justify-content: center; transition: all 0.3s;",
                                        if config.get().allow_bots_mode == id_c2 { "#ff3c14" } else { "rgba(255,60,20,0.1)" }
                                    )>
                                        <Show when={
                        ...
                        >
                            <div style="display: flex; align-items: flex-start; gap: 16px;">
                                <div style=move || format!(
                                    ...
                                    if config.get().security_level == lid_c2 { "#ff3c14" } else { "rgba(255,60,20,0.1)" }
                                )>
    ...
    view! {
        <div class="opacity-0 animate-fade-in">
            ...
            <div style="margin-top: 14px; padding: 14px 18px; background: rgba(255,255,255,0.03); border-radius: 12px; border: 1px solid rgba(255,60,20,0.1); transition: all 0.3s ease;">
                <span style="font-family: 'Orbitron', monospace; font-size: 10px; color: #ff3c14; letter-spacing: 3px; font-weight: 700;">
        ...
        <div class="opacity-0 animate-fade-in">
                                ...
                                >
                                    <div style="display: flex; align-items: center; justify-content: space-between;">
                                        ...
                                        <div style=move || format!(
                                            ...
                                            if config.get().image_gen_provider == pid_r { "#ff3c14" } else { "rgba(255,60,20,0.2)" },
                                            if config.get().image_gen_provider == pid_r { "#ff3c14" } else { "transparent" }
                                        ) />
                                    </div>
                                </div>
                                {
                                                        ...
                                                        view! {
                                                            <div>
                                                                ...
                                                                <input
                                                                    ...
                                                                    class="w-full box-border bg-white/[0.04] border border-z-accent/15 rounded-lg px-3.5 py-[9px] text-[13px] text-text-z-text font-rajdhani-mono outline-none"
                                                                />
                                                            </div>
                                                        }.into_any()
                                                    } else {
                ...
                <div style="display: flex; flex-direction: column; gap: 10px; padding: 16px 18px; border-radius: 12px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,60,20,0.1);">
                    <div>
                        <div class="flex justify-between items-baseline mb-1">
                            <div class="text-[13px] font-bold text-text-z-text/80">"Piper TTS URL"</div>
                            <div class="text-[11px] text-text-z-text/35">"Text-to-speech engine"</div>
                        </div>
                        <input
                            type="text"
                    ...
                    <div>
                        <div class="flex justify-between items-baseline mb-1">
                            <div class="text-[13px] font-bold text-text-z-text/80">"Whisper STT URL"</div>
                            <div class="text-[11px] text-text-z-text/35">"Speech-to-text engine"</div>
                        </div>
                        <input
                            type="text"
                    ...
                    <div>
                        <div class="flex justify-between items-baseline mb-1">
                            <div class="text-[13px] font-bold text-text-z-text/80">"ElevenLabs API Key"</div>
                            <div class="text-[11px] text-text-z-text/35">"Premium cloud TTS (optional — overrides Piper)"</div>
                        </div>
                        <input
                            type="password"
                            placeholder="sk_..."
            ...
            <div class="mb-4">
                ...
                <div style="padding: 16px 18px; border-radius: 12px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,60,20,0.1);">
                    <div class="flex justify-between items-baseline mb-1">
                        <div class="text-[13px] font-bold text-text-z-text/80">"ComfyUI / Video Gen URL"</div>
                        <div class="text-[11px] text-text-z-text/35">"Wan2.1, CogVideo, AnimateDiff..."</div>
                    </div>
                    <input
                        type="text"
        ...
        <div class="opacity-0 animate-fade-in">
                        ...
                        view! {
                            ...
                            >
                                <div style="display: flex; align-items: center; gap: 14px;">
                                    <div style=move || format!(
                                        "width: 22px; height: 22px; border-radius: 11px; flex-shrink: 0; border: 2px solid {}; display: flex; align-items: center; justify-content: center; transition: all 0.3s;",
                                        if config.get().orchestration_mode == id_c2 { "#ff3c14" } else { "rgba(255,60,20,0.1)" }
                                    )>
                                        <Show when={
                                            let id_show = id_c3.clone();
            ...
            <Show when=move || config.get().orchestration_mode == "custom">
                <div class="mb-5">
                    ...
                    <div style="padding: 16px 18px; border-radius: 12px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,60,20,0.1);">
                        <div class="flex justify-between items-baseline mb-1">
                            <div class="text-[13px] font-bold text-text-z-text/80">"Interval (seconds)"</div>
                            <div class="text-[11px] text-text-z-text/35">"How often Prometheus checks for proactive tasks"</div>
                        </div>
                        <input
                            type="text"
                            placeholder="300"
                        ...
                        view! {
                            ...
                            >
                                <div style="display: flex; align-items: center; gap: 14px;">
                                    <div style=move || format!(
                                        "width: 22px; height: 22px; border-radius: 11px; flex-shrink: 0; border: 2px solid {}; display: flex; align-items: center; justify-content: center; transition: all 0.3s;",
                                        if config.get().hermes_channel == id_c2 { "#ff3c14" } else { "rgba(255,60,20,0.1)" }
                                    )>
                                        <Show when={
                                            let id_show = id_c3.clone();
                    ...
                    <div style="flex: 1; min-width: 200px;">
                        ...
                        >
                            <div style=move || format!(
                                ...
                                if config.get().gateway_mentions_only { "#ff3c14" } else { "rgba(255,60,20,0.2)" }
                            )>
                                <Show when=move || config.get().gateway_mentions_only>
                                    <div style="width: 8px; height: 8px; border-radius: 2px; background: #ff3c14;" />
                                </Show>
                            </div>
        ...
        <div class="opacity-0 animate-fade-in">
            ...
            <div style="margin-top: 18px; text-align: center;">
                <a
                    ...
                    style="font-family: 'Orbitron', monospace; font-size: 10px; letter-spacing: 2px; color: #ff3c14; text-decoration: none; opacity: 0.7; transition: opacity 0.3s;"
                >
                                ...
                                on:click=move |_| {
                                    ...
                                    spawn_local(async move {
                                        ...
                                        let config_json = serde_json::json!({
                                            ...
                                            "hermes": {
                                                "default_channels": [c.hermes_channel],
                                            },
                                        });
                                        ...
                                        // Step 2: Save credentials (API keys or OAuth tokens)
                                        ...
                                        for (provider, key) in &c.api_keys {
                                            ...
                                            if is_oauth {
                                                ...
                                                // Regular API key
                                                if let Err(e) = api::auth_save_credentials(provider, key).await {
                                                    web_sys::console::warn_1(&format!("Zeus: save key for {} failed: {}", provider, e).into());
                                                }
                                            }
                                        }
                                        ...
                                        let auth_method = if c.auth_types.get(&provider).map(|t| t == "oauth_token").unwrap_or(false)
                                            || c.api_keys.get(&provider).map(|k| k.starts_with("sk-ant-oat01-")).unwrap_or(false) {
                                            "oauth_token"
                                        } else {
                                            "api_key"
                                        };
                                        if let Err(e) = api::complete_onboarding(
                                            "full",
                                            &provider,
                                            auth_method,
                                            Some(model.as_str()),
                                            ...
                                            web_sys::console::warn_1(&format!("Zeus: complete_onboarding failed: {}", e).into());
                                        }

                                        // Step 5b: Generate workspace files (SOUL.md, AGENTS.md, etc.)
                                        save_detail.set("Workspace files...".to_string());
                                        // Workspace files are generated server-side by complete_onboarding
                                        // Also save config with all collected values
                                        let _ = api::save_config(&serde_json::json!({
                                            "model": c.default_model,
