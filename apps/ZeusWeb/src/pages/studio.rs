...
pub fn StudioPage() -> impl IntoView {
                            ...
                            messages.update(|m| {
                                if let Some(last) = m.last_mut()
                                    && last.role == "assistant" {
                                        ...
                                        // No prior assistant bubble (inbox path — no text_chunk events)
                                        m.push(ChatMessage {
                                            role: "assistant".to_string(),
    ...
    // ── Keyboard shortcuts ──
    {
        let ws_ref_esc = ws_ref.clone();
        let messages_esc = messages;
        let is_streaming_esc = is_streaming;
        let orb_mode_esc = orb_mode;
        spawn_local(async move {
            let window = web_sys::window().unwrap();
            let cb = Closure::wrap(Box::new(move |ev: web_sys::KeyboardEvent| {
                ...
                // Escape: cancel in-flight stream OR clear input
                if ev.key() == "Escape" {
                    if is_streaming.get() {
                        // Signal cancel to gateway via WS
                        let ws_borrow = ws_ref_esc.borrow();
                        if let Some(ws) = ws_borrow.as_ref() {
                            if ws.ready_state() == web_sys::WebSocket::OPEN {
                                let cancel_payload = serde_json::json!({ "type": "cancel_stream" });
                                let _ = ws.send_with_str(&cancel_payload.to_string());
                            }
                        }
                        // Stop UI appending immediately
                        messages_esc.update(|msgs| {
                        ...
                        is_streaming_esc.set(false);
                        orb_mode_esc.set("dormant".to_string());
                    } else {
                        input.set(String::new());
                    }
                }
    ...
    view! {
        <div style="display: flex; height: 100vh;">
                ...
                <div style="width: 260px; border-right: 1px solid rgba(255,60,20,0.1); display: flex; flex-direction: column; flex-shrink: 0; background: rgba(0,0,0,0.3);">
                    <div style="padding: 16px; border-bottom: 1px solid rgba(255,60,20,0.08);">
                        <Button
                            ...
                            style="width: 100%; padding: 10px; justify-content: center; gap: 8px;"
                        >
            ...
            <div style="padding: 16px 24px; border-bottom: 1px solid rgba(255,60,20,0.1); display: flex; align-items: center; gap: 16px; flex-shrink: 0;">
                // Sidebar toggle
                <Button
                    on_click=Some(Callback::new(move |_| sidebar_open.update(|v| *v = !*v)))
                    style="width: 36px; height: 36px; padding: 0; border-radius: 8px; background: rgba(255,255,255,0.03); color: rgba(255,245,240,0.4); justify-content: center; flex-shrink: 0;"
                >
                ...
                // TTS toggle
                // design: left raw — stateful toggle
                <button
                    on:click=toggle_tts
            ...
            <div style="padding: 16px 24px; border-top: 1px solid rgba(255,60,20,0.1); flex-shrink: 0;">
                <div style="display: flex; gap: 10px; align-items: flex-end;">
                    ...
                    <Button
                        ...
                        style="width: 44px; height: 44px; padding: 0; border-radius: 10px; background: rgba(255,255,255,0.03); color: rgba(255,245,240,0.5); justify-content: center; flex-shrink: 0;"
                    >
                ...
                <div style="width: 240px; border-left: 1px solid rgba(255,60,20,0.1); display: flex; flex-direction: column; flex-shrink: 0; background: rgba(0,0,0,0.2);">
                    ...
                    <div style="flex: 1; overflow-y: auto; padding: 8px;">
                        {move || session_tools.get().into_iter().map(|t| {
                            ...
                            view! {
                                <Card style="padding: 8px; margin-bottom: 6px; border-radius: 6px;">
            ...
            <Show when=move || puppet_mode.get()>
                <div style="width: 50%; border-left: 1px solid rgba(255,60,20,0.12); display: flex; flex-direction: column; flex-shrink: 0; background: rgba(0,0,0,0.2); min-width: 320px;">
                    ...
                    <div style="padding: 14px 18px; border-bottom: 1px solid rgba(255,60,20,0.08); display: flex; align-items: center; gap: 10px; flex-shrink: 0;">
                        ...
                        <Button
                            small=true
                            on_click=Some(Callback::new(move |_| { puppet_actions.set(Vec::new()); puppet_screenshot_b64.set(None); }))
                            style="border: 1px solid rgba(255,245,240,0.08); border-radius: 5px; color: rgba(255,245,240,0.3); font-size: 7px; letter-spacing: 1px;"
                        >"CLEAR"</Button>
                    </div>
                    // Screenshot frame
