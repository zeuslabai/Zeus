...
#[component]
pub fn MatrixPage() -> impl IntoView {
    let channel_id = RwSignal::new(String::new());
    let homeserver = RwSignal::new(String::new());
    ...
    spawn_local(async move {
        if let Ok(r) = api::fetch_channels().await
            && let Some(ch) = r.channels.iter().find(|c| c.channel_type == "matrix" || c.platform == "matrix") {
                channel_id.set(ch.id.clone());
                is_connected.set(ch.status == "connected");
                if let Some(v) = ch.config["homeserver"].as_str() {
                    homeserver.set(v.to_string());
                }
                if let Some(v) = ch.config["username"].as_str() {
                    username.set(v.to_string());
                }
    ...
    let do_save = move |_| {
        ...
        let hs = homeserver.get_untracked();
        ...
        spawn_local(async move {
            ...
            config_map.insert("homeserver".into(), serde_json::Value::String(hs));
            config_map.insert("username".into(), serde_json::Value::String(un));
            if !pw.is_empty() {
                config_map.insert("password".into(), serde_json::Value::String(pw));
            }
                ...
                <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 32px;">
                    ...
                    <div>
                        ...
                        <p style="margin: 0; font-size: 13px; color: rgba(255,245,240,0.7);">
                            "Matrix protocol via matrix-sdk (native Rust)"
                        </p>
                    </div>
                ...
                <div style="background: rgba(255,245,240,0.03); border: 1px solid rgba(255,60,20,0.1); border-radius: 8px; padding: 24px; margin-bottom: 24px; display: flex; flex-direction: column; gap: 20px;">
                    <div>
                        ...
                        <input
                            ...
                            on:input=move |e| homeserver.set(event_target_value(&e))
                    ...
                    <div>
                        ...
                        />
                        <p style="margin: 6px 0 0; font-size: 12px; color: rgba(255,245,240,0.5);">
                            "Full Matrix ID including server (e.g. @user:server.org)"
                        </p>
                    </div>
                    <div>
                        <label style="display: block; font-family: 'Orbitron', monospace; font-size: 9px; letter-spacing: 2px; color: rgba(255,245,240,0.7); margin-bottom: 8px;">
                        ...
                        />
                        <p style="margin: 6px 0 0; font-size: 12px; color: rgba(255,245,240,0.5);">
                            "Access token will be stored after first successful login"
                        </p>
                    </div>
                </div>
