...
#[component]
pub fn TelegramPage() -> impl IntoView {
    ...
    spawn_local(async move {
        if let Ok(r) = api::fetch_channels().await
            && let Some(ch) = r.channels.iter().find(|c| c.channel_type == "telegram" || c.platform == "telegram") {
    ...
    let do_save = move |_| {
        let token = bot_token.get_untracked();
        if token.trim().is_empty() { is_err.set(true); msg.set("Bot token is required".into()); return; }
        let cid = channel_id.get_untracked();
        saving.set(true); is_err.set(false); msg.set(String::new());
        spawn_local(async move {
            let config = serde_json::json!({ "bot_token": token });
            let result = if cid.is_empty() {
                api::create_channel(&api::CreateChannelReq { channel_type: "telegram".into(), name: "Telegram".into(), config }).await.map(|_| ())
            } else {
    ...
    view! {
        <div style="padding: 32px; max-width: 640px;">
            <div style="display: flex; align-items: center; gap: 16px; margin-bottom: 32px;">
                ...
                <div>
                    <h1 style="font-family: 'Orbitron', monospace; font-size: 14px; letter-spacing: 6px; color: rgba(255,245,240,0.9); margin: 0;">"TELEGRAM"</h1>
                    <p style="font-family: 'Rajdhani', sans-serif; font-size: 13px; color: rgba(255,245,240,0.7); margin: 4px 0 0;">{move || {
                        ...
                        else if is_connected.get() { "Connected \u{2014} Bot active".to_string() }
            ...
            <Show when=move || loaded.get()>
                <Card>
                    <div style="font-family: 'Orbitron', monospace; font-size: 9px; letter-spacing: 3px; color: rgba(255,245,240,0.7); margin-bottom: 20px;">"BOT CONFIGURATION"</div>
                    <div>
                        <div style="font-family: 'Orbitron', monospace; font-size: 9px; letter-spacing: 2px; color: rgba(255,245,240,0.7); margin-bottom: 6px;">"BOT TOKEN *"</div>
                        <input type="password" placeholder="123456789:ABC-DEF..." style=input_style
                            prop:value=move || bot_token.get()
                            on:input=move |ev| bot_token.set(event_target_value(&ev))
                        />
                        <div style="font-family: 'Rajdhani', sans-serif; font-size: 12px; color: rgba(255,245,240,0.5); margin-top: 4px;">"Obtain from @BotFather on Telegram"</div>
                    </div>
