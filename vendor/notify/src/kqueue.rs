...
impl Drop for KqueueWatcher {
    fn drop(&mut self) {
        // ZEUS PATCH (#208): the upstream code unwraps these two sends, on the
        // assumption "we expect the event loop to live => unwrap must not panic".
        // On BSD that assumption does not hold: the event-loop thread can die
        // early (e.g. `poll.poll()` errors on a revoked/deleted watched fd ->
        // `panic!("poll failed")` at this file's event_loop_thread), which drops
        // the receiver. The subsequent Shutdown send then returns SendError and
        // the unwrap panics *inside Drop* — a panic-during-drop that aborts the
        // whole gateway process. A non-critical file-watch shutdown failing must
        // degrade gracefully, never take down the host. Swallow the send error.
