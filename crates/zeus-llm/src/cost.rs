...
pub fn lookup_pricing(provider: &str, model: &str) -> Option<ModelPricing> {
    ...
    match p.as_str() {
        ...
        "sakana" => sakana_pricing(&m),
        "ollama" | "local" => Some(ModelPricing::flat(0.0, 0.0)),
        _ => None,
    }
}

/// Sakana Fugu Ultra pricing — a multi-agent-system-as-a-model.
/// CONFIRMED 2026-06-27 vs Sakana's first-party pricing (sakana.ai/fugu):
///
/// - base (context ≤ 272K): $5/M input · $30/M output · $0.50/M cache-read
/// - long-context (> 272K): $10/M input · $45/M output · $1.00/M cache-read
///
/// We model the base rate (the common case) as a flat tariff. The >272K tier
/// (2× in/1.5× out) is a known under-estimate on very long prompts — tracked
/// as a fast-follow once cost.rs can take context length into account.
/// See docs/Fugu-Research.md.
fn sakana_pricing(_model: &str) -> Option<ModelPricing> {
    // Fugu Ultra is currently Sakana's only model; any model id (or an unknown
    // one) falls back to the published base $5/M in · $30/M out rather than
    // returning "cost unavailable".
    Some(ModelPricing::flat(5.0, 30.0))
}
...
mod tests {
    ...
    #[test]
    fn sakana_fugu_pricing() {
        // Fugu Ultra: $5/M input · $30/M output.
        let c = calculate_cost("sakana", "fugu-ultra", 1_000_000, 1_000_000, 0).unwrap();
        assert!((c.total_usd - (5.0 + 30.0)).abs() < 1e-9);
    }
