...
/// Idle-reset window for streaming responses (#255).
///
/// Unlike the client-level `request_timeout` (a hard wall measured from
/// request start — see `build_llm_client`), this is the maximum time the
/// stream may go **without any new chunk**. Each arriving token/chunk resets
/// the window, so a slow-but-streaming response is never killed; the timeout
/// fires only on a true no-progress idle (a stalled/dead connection). 120s is
/// generous enough to outlast slow inference between tokens while still
/// surfacing a genuinely hung stream promptly.
...
/// Includes WAF/CDN origin errors (520–524) common behind Aliyun-WAF and Cloudflare.
fn is_retryable_status(status: u16) -> bool {
    matches!(status, 429 | 500 | 502 | 503 | 504 | 520 | 521 | 522 | 524)
}
...
/// Resolve the ZAI (Zhipu / GLM) OpenAI-compatible base URL.
///
/// GLM-5.2 (and the current flagship line) ships on the **global**
/// platform `https://api.z.ai/api/paas/v4`. The legacy CN platform
/// `https://open.bigmodel.cn/api/paas/v4` does **not** serve 5.2, so
/// the default here is `api.z.ai` — pinning bigmodel would list 5.2
/// in onboarding but fail at runtime ("model not found").
///
/// Resolution order:
/// 1. `ZAI_BASE_URL` — explicit full-URL override (any host/path).
/// 2. `ZAI_REGION=cn` — opt back into the legacy bigmodel.cn host
///    (for users whose key is only provisioned on the CN platform).
/// 3. Default — `https://api.z.ai/api/paas/v4` (global, serves 5.2).
///
/// Both hosts speak the identical `/api/paas/v4` OpenAI-compatible
/// surface, so only the host swaps — the request/response path is
/// unchanged.
pub fn resolve_zai_base_url() -> String {
    // 1. Explicit full-URL override.
    ...
    // 3. Default: global platform — serves GLM-5.2.
...
pub fn qwen_bundled_catalog() -> Vec<QwenBundledModel> {
    vec![
        ...
        QwenBundledModel {
            // #141: the model raspi actually runs (Ollama-served, `:27b` tag).
            // The static supports_image_input() catalog lookup is exact-match
            // and runs for every vision-capable provider (incl. Ollama, whose
            // static caps are vision:true), so this exact id is caught here and
            // returns Err → the guard strips images + injects the decline note.
            // Without it, falls through to the provider-true default and
            // fabricates "I can see it."
            id: "qwen3.6:27b",
            context_window: 262_144,
            supports_vision: false,
            reasoning: false,
            scope: QwenEndpointScope::Any,
            note: "Ollama-served Qwen3.6 27B — text-only",
        },
        QwenBundledModel {
            id: "MiniMax-M2.5",
            context_window: 1_000_000,
        ...
        QwenBundledModel {
            // #141: the model ZM actually runs — text-only, no vision.
            // Without this exact id, it falls through to the provider-true
            // default and fabricates "I can see it." Catalogued vision:false.
            id: "MiniMax-M2.6-highspeed",
            context_window: 1_000_000,
            supports_vision: false,
            reasoning: true,
            scope: QwenEndpointScope::Any,
            note: "MiniMax high-speed reasoning via Alibaba",
        },
        QwenBundledModel {
            id: "glm-5",
...
pub struct LlmClient {
    ...
    seed: Option<u64>,
    /// Ordered fallback model chain ("provider/model" strings) tried after primary exhausts all retries.
    /// Each is tried in order; the first to succeed wins. Health tracking skips recently-failed providers.
    ...
    /// Per-request timeout — overrides the client-level timeout for individual HTTP calls.
    /// Zai (bigmodel.cn) gets 30s to fail fast through the Aliyun WAF and retry sooner;
    /// other providers keep the default 300s (5 min) for long inference runs.
...
impl LlmClient {
    ...
    /// The resolved API base URL for this client's provider.
    ...
    pub fn new(provider: Provider, model: String) -> Result<Self> {
        ...
        Ok(Self {
            ...
            // Zai (bigmodel.cn) is behind CN/Aliyun-WAF — chronically flaky.
            // Enable persistent retry to survive degraded-latency windows.
            persistent_retry: provider == Provider::Zai,
            // Zai (bigmodel.cn) behind CN/Aliyun-WAF: fail fast per-attempt (30s),
            // retry aggressively. Other providers keep 300s for long inference.
            request_timeout: if provider == Provider::Zai {
    ...
    /// Appends to the fallback chain.
    pub fn with_fallback_model(mut self, model: impl Into<String>) -> Self {
        self.fallback_models.push(model.into());
        self
    }

    /// Set the full ordered fallback model chain. Each entry is a "provider/model" string.
    /// Tried in order after primary exhausts all retries; first success wins.
    pub fn with_fallback_models(mut self, models: Vec<String>) -> Self {
        self.fallback_models = models;
        self
    }

    /// Enable persistent retry mode: retries indefinitely with 5-min max backoff.
    /// Use for chronically flaky endpoints (e.g., CN/Aliyun-WAF proxied APIs).
    pub fn with_persistent_retry(mut self, enabled: bool) -> Self {
        self.persistent_retry = enabled;
        self
    }

    /// Override per-request timeout. Default: 30s for Zai (fail fast through WAF),
    /// 300s for all other providers. Shorter timeout + more retries = better resilience
    /// for flaky endpoints; longer timeout = better for long inference runs.
    ...
    fn inject_openai_sampling(&self, body: &mut serde_json::Value, has_tools: bool) {
        ...
        // Some providers (e.g. Sakana Fugu Ultra — an orchestration/reasoning
    ...
    pub async fn complete(
        ...
        // Vision-honesty guard (#141): strip images + inject in-band decline note
        // for non-vision models, BEFORE dispatch — covers all providers incl. MiniMax.
        let sanitized = self.sanitize_images_for_vision(&sanitized);
        ...
        loop {
            ...
            match result {
                ...
                Err(e) => {
                    // Primary model exhausted — try fallback chain if configured
                    if !self.fallback_models.is_empty() {
                        warn!(
                            "Primary model {} failed after {} attempts ({}), trying {} fallback(s)",
                            self.model, attempt, e, self.fallback_models.len()
                        );
                        for (i, fallback) in self.fallback_models.iter().enumerate() {
                            info!("Fallback chain [{}/{}]: trying {}", i + 1, self.fallback_models.len(), fallback);
                            let fallback_client = self.build_fallback_client(fallback);
                            match fallback_client.complete_inner(messages, tools, system).await {
                                Ok(resp) => {
                                    info!("Fallback chain: {} succeeded", fallback);
                                    return Ok(resp);
                                }
                                Err(fe) => {
                                    warn!("Fallback chain: {} failed — {}", fallback, fe);
                                }
                            }
                        }
                        warn!("All {} fallback models exhausted", self.fallback_models.len());
                    }
                    return Err(e);
                }
            }
    ...
    ///
    /// Retries on: 429 (rate limit), 500/502/503 (server errors), overloaded,
    /// WAF/CDN errors (520–524), transport errors (timeout, connection reset, etc.).
    ...
    fn is_retryable_error(e: &Error) -> bool {
        ...
        // HTTP status-class errors (including WAF/CDN origin errors)
        msg.contains("429") || msg.contains("rate_limit")
            || msg.contains("500 ") || msg.contains("502 ")
            || msg.contains("503 ") || msg.contains("overloaded")
            || msg.contains("520 ") || msg.contains("521 ")
            || msg.contains("522 ") || msg.contains("524 ")
            // Transport-class errors (reqwest connect/timeout/DNS)
            // These produce strings like "error sending request for url (...)"
            // or "connection closed" or "timed out" — all transient, all retryable.
        ...
        loop {
            ...
            match result {
                ...
                Err(e) => {
                    ...
                    if !self.fallback_models.is_empty() {
                        warn!(
                            "Primary model {} stream failed after {} attempts ({}), trying {} fallback(s)",
                            self.model, attempt, e, self.fallback_models.len()
                        );
                        for (i, fallback) in self.fallback_models.iter().enumerate() {
                            info!("Stream fallback chain [{}/{}]: trying {}", i + 1, self.fallback_models.len(), fallback);
                            let fallback_client = self.build_fallback_client(fallback);
                            match fallback_client.stream_inner(messages, tools, system).await {
                                Ok(pair) => {
                                    info!("Stream fallback chain: {} succeeded", fallback);
                                    return Ok(pair);
                                }
                                Err(fe) => {
                                    warn!("Stream fallback chain: {} failed — {}", fallback, fe);
                                }
                            }
                        }
                        warn!("All {} stream fallback models exhausted", self.fallback_models.len());
                    }
                    return Err(e);
                }
            }
    ...
    async fn stream_anthropic(
        ...
        let handle = tokio::spawn(async move {
            ...
            let response = match request.timeout(request_timeout).json(&body).send().await {
                Ok(r) => r,
                Err(e) => {
            ...
            loop {
                // #255 idle-reset: each chunk resets the window; fire only on true no-progress idle.
                let chunk = match tokio::time::timeout(STREAM_IDLE_TIMEOUT, stream.next()).await {
                    Ok(Some(c)) => c,
                    Ok(None) => break,
                    Err(_) => {
                        warn!("LLM stream idle for {:?} with no new chunk - treating as no-progress timeout", STREAM_IDLE_TIMEOUT);
                        break;
                    }
                };
    ...
    fn to_anthropic_messages(&self, messages: &[Message]) -> Vec<serde_json::Value> {
        ...
        // #294 — Trailing assistant-prefill guard (thinking-gated).
        //
        // When extended thinking is enabled, Anthropic REJECTS a request whose
        // final message is an `assistant` turn:
        //   400 "Expected `thinking` or `redacted_thinking`, but found `text`.
        //        When `thinking` is enabled, a final `assistant` message must
        //        start with a thinking block …" / "conversation must end with a
        //        user message."
        //
        // This happens when a cook loop ends on an assistant turn and the next
        // request carries that dangling assistant message as a trailing prefill.
        // Assistant prefill is a LEGITIMATE feature for NON-thinking models, so
        // we gate strictly on `thinking_level.is_some()` — never kill prefill
        // globally. We pop trailing assistant turn(s) so the request ends on a
        // user turn. (We only strip when a prior user turn exists, so we never
        // emit an empty message list.)
    ...
    async fn complete_openai(
        ...
        // Output cap is per-provider via the capability registry (Sakana Fugu
        // Ultra = 128k; others default to a conservative 4096).
        let max_output = capabilities::capabilities(&self.provider).max_output_tokens;
        if uses_new_api {
        ...
        if !skip_thinking
            && let Some(thinking) = Self::thinking_param(&self.provider, &self.model)
        {
    ...
    async fn stream_openai(
        ...
        // Output cap is per-provider via the capability registry (Sakana = 128k).
        let max_output = capabilities::capabilities(&self.provider).max_output_tokens;
        if uses_new_api {
            body["max_completion_tokens"] = serde_json::json!(max_output);
        } else {
    ...
    /// Vision-honesty guard (#141): when the target model does NOT support image
    /// input, strip image attachments AND inject an in-band note into the message
    /// the model actually sees — so it declines honestly instead of fabricating
    /// "I can see it."
    ///
    /// This runs once at the top of `complete`/`stream`, BEFORE provider dispatch,
    /// so it covers every path uniformly — including providers (e.g. MiniMax) whose
    /// message-formatter bypasses the per-call `should_include_images()` gate.
    ///
    /// Rationale: the previous behavior silently dropped images (only a
    /// `tracing::debug/warn` to logs) — the model received "describe this image"
    /// with no image and no signal one was removed, so it hallucinated. The fix is
    /// to make the drop *loud and in-band*: the model can't decline what it doesn't
    /// know was removed.
    ///
    /// Non-image attachments (documents/audio) are left untouched — this only
    /// targets image MIME types, which are the vision-hallucination vector.
    fn sanitize_images_for_vision(&self, messages: &[Message]) -> Vec<Message> {
        // Only act when the model genuinely lacks vision. Ok(true) → passthrough.
        let supports_vision = matches!(
            capabilities::supports_image_input(&self.provider, &self.model),
            Ok(true)
        );
        if supports_vision {
        ...
        for msg in messages {
            ...
            // Strip image attachments; keep non-image attachments (docs/audio).
            let mut m = msg.clone();
            m.attachments
                .retain(|a| !a.mime_type.starts_with("image/"));

            // Inject the in-band honesty note so the model declines, not fabricates.
            let noun = if image_count == 1 { "image was" } else { "images were" };
            let note = format!(
                "[SYSTEM: {image_count} {noun} attached to this message, but the \
                 current model ({model}) cannot process images. The image data has \
                 been removed and is NOT available to you. Do NOT claim to see or \
                 describe it — instead, honestly tell the user you cannot view \
                 images and ask them to describe it or switch to a vision-capable \
                 model.]",
                model = self.model
            );
            ...
            tracing::warn!(
                ...
                model = %self.model,
                image_count,
                "Non-vision model: stripped image attachments + injected in-band decline note (#141)"
            );
    ...
    /// Decide the OpenAI-compat `thinking` request param for Moonshot/XiaomiMimo.
    ///
    /// Returns `Some(json)` to set `body["thinking"]`, or `None` to OMIT the param.
    ///
    /// #237 Bug-1: Moonshot rejects `{"type":"disabled"}` with
    /// `invalid thinking: only type=enabled` — it accepts ONLY `type=enabled` or the
    /// param being absent. So for non-thinking Kimi models we return `None` (omit)
    /// rather than sending `disabled`. XiaomiMimo still needs explicit `disabled`
    /// for non-thinking models (Bug-4). All other providers never send `thinking`.
    fn thinking_param(provider: &Provider, model: &str) -> Option<serde_json::Value> {
        match provider {
            Provider::Moonshot => {
    ...
    fn sanitize_tool_pairs_for_provider(messages: &[Message], provider: &Provider) -> Vec<Message> {
        ...
        //
        // #237 Bug-2: Phases 2a and 2b previously used INCONSISTENT id-sets. 2a
        // strips an assistant tool_call when its result is missing from the
        // position-aware window (results only until the next turn boundary); but 2b
        // validated surviving tool_results against the GLOBAL pre-strip
        // `all_tool_call_ids`. So a result whose tool_call 2a just stripped (window
        // miss) was still kept by 2b — yielding a `tool_call_id is not found` 400 on
        // Moonshot/Kimi. We now track the ids 2a actually strips and subtract them in
        // 2b, so a result never survives its own (stripped) call.
        let mut stripped_call_ids: std::collections::HashSet<String> =
            ...
            if msg.role == Role::Assistant && !msg.tool_calls.is_empty() {
                ...
                if !orphan_ids.is_empty() {
                    if strip_orphans {
                        ...
                        // #237 Bug-2: record what we strip so Phase 2b can drop the
                        // now-dangling results that reference these ids.
                        for id in &orphan_ids {
                            stripped_call_ids.insert((*id).to_string());
                        }
                ...
                cleaned.tool_results.retain(|tr| {
                    !tr.call_id.is_empty()
                        && all_tool_call_ids.contains(&tr.call_id)
                        // #237 Bug-2: a result whose tool_call was stripped in 2a is
                        // now orphaned — drop it so the id-pairing stays consistent.
                        && !stripped_call_ids.contains(&tr.call_id)
                });
    ...
    fn parse_openai_response(&self, text: &str) -> Result<LlmResponse> {
        ...
        // #99: Inline tool-call salvage (DEFENSIVE, gated).
        // Some moonshot/kimi deployments occasionally emit tool calls inline in
        // the `content` field as a JSON block instead of the structured
        // `tool_calls[]` array. The structured path above then yields zero tool
        // calls, the cooking loop sees "no tool calls", and the model degenerates
        // into a repeat-content loop (#99 root-cause). This gate activates ONLY
        // when the structured field is empty AND content parses to a call-block,
        // so anthropic/opus (which always populates tool_calls[]) never hits it —
        // zero risk to the non-kimi path.
        if tool_calls.is_empty()
            && let Some(salvaged) = Self::salvage_inline_tool_calls(&content)
            && !salvaged.is_empty()
        {
            warn!(
                "parse_openai_response: structured tool_calls empty but salvaged {} \
                 inline tool-call(s) from content (#99 kimi inline-emission quirk)",
                salvaged.len()
            );
            tool_calls = salvaged;
    ...
    /// #99: Salvage tool calls emitted inline in the content string.
    /// Recognizes a JSON object/array of `{"name": ..., "arguments": ...}` (or
    /// the OpenAI `{"function": {"name", "arguments"}}` shape) embedded anywhere
    /// in `content`. Returns None if nothing parseable is found. Defensive only —
    /// the gate in parse_openai_response ensures this never runs on providers that
    /// populate the structured tool_calls[] field.
    fn salvage_inline_tool_calls(content: &str) -> Option<Vec<ToolCall>> {
        fn extract_one(v: &serde_json::Value) -> Option<ToolCall> {
        ...
        // Find the first balanced JSON object/array in the content.
    ...
    async fn complete_openrouter(
        ...
        // #44-b-ii-B / #44-c: Consult OpenRouterResolver for dynamic model capabilities.
        // - Pin temperature=1.0 for thinking_mode_temperature_lock=Some(true) (e.g. MiMo).
        // - Strip tools from body when supports_tools=false (dynamic gate, #44-c).
        ...
        if let Some(caps) = <OpenRouterResolver as crate::capabilities::ModelCapabilityResolver>::resolve(&resolver, &self.model).await {
            ...
            if !caps.supports_tools {
                body.as_object_mut().map(|m| { m.remove("tools"); m.remove("tool_choice"); });
                tracing::debug!(model = %self.model, "dynamic caps: supports_tools=false — tools stripped from request");
            }
        }
    ...
    async fn stream_openrouter(
        ...
        // Also strips tools when supports_tools=false (#44-c dynamic gate).
        let resolver = OpenRouterResolver::new(
            self.client.clone(),
            self.base_url.clone(),
            Some(api_key.clone()),
        ...
        if let Some(caps) = <OpenRouterResolver as crate::capabilities::ModelCapabilityResolver>::resolve(&resolver, &self.model).await {
            ...
            if !caps.supports_tools {
                body.as_object_mut().map(|m| { m.remove("tools"); m.remove("tool_choice"); });
                tracing::debug!(model = %self.model, "dynamic caps: supports_tools=false — tools stripped from stream request");
            }
        }
...
mod tests {
    ...
    #[test]
    fn test_xai_provider_and_base_url() {
        // xai/grok-2 must resolve to Provider::XAI with the x.ai base URL,
        // routed through the OpenAI-compatible path (mirrors OpenRouter wiring).
        let client = LlmClient::new(Provider::XAI, "grok-2".to_string())
            .expect("xAI client construction should succeed");
        assert_eq!(*client.provider(), Provider::XAI);
        assert_eq!(client.base_url(), "https://api.x.ai");

        // "xai/grok-2" model string parses to Provider::XAI via zeus-core.
        let mut cfg = zeus_core::Config::default();
        cfg.model = "xai/grok-2".to_string();
        assert_eq!(cfg.parse_model().0, Provider::XAI);

        // "grok/" alias also maps to XAI.
        cfg.model = "grok/grok-2-latest".to_string();
        assert_eq!(cfg.parse_model().0, Provider::XAI);
    }
    ...
    // #294 — Trailing assistant-prefill guard.

    #[test]
    fn test_anthropic_thinking_strips_trailing_assistant_prefill() {
        // When thinking is enabled and the conversation ends on an assistant
        // turn, that dangling prefill is stripped so the request ends on a user
        // message (otherwise Anthropic returns 400).
        let client = LlmClient::new(Provider::Anthropic, "claude-opus-4-20250514".to_string())
            .expect("should construct")
            .with_thinking("high");
        let msgs = vec![
            Message::user("hello"),
            Message::assistant("partial answer that should be dropped"),
        ];
        let result = client.to_anthropic_messages(&msgs);
        assert_eq!(result.len(), 1, "trailing assistant turn should be popped");
        assert_eq!(result.last().unwrap()["role"], "user");
    }

    #[test]
    fn test_anthropic_thinking_strips_multiple_trailing_assistants() {
        let client = LlmClient::new(Provider::Anthropic, "claude-opus-4-20250514".to_string())
            .expect("should construct")
        ...
        let result = client.to_anthropic_messages(&msgs);
        assert_eq!(result.len(), 1, "all trailing assistant turns popped");
        assert_eq!(result.last().unwrap()["role"], "user");
    }

    #[test]
    fn test_anthropic_thinking_keeps_trailing_user() {
        // A normal conversation that already ends on a user turn is untouched.
        let client = LlmClient::new(Provider::Anthropic, "claude-opus-4-20250514".to_string())
            .expect("should construct")
            .with_thinking("high");
        let msgs = vec![
        ...
        let result = client.to_anthropic_messages(&msgs);
        assert_eq!(result.len(), 3, "user-terminated convo is unchanged");
        assert_eq!(result.last().unwrap()["role"], "user");
    }

    #[test]
    fn test_anthropic_non_thinking_preserves_assistant_prefill() {
        // Blast-radius guard: WITHOUT thinking, a trailing assistant prefill is
        // a legitimate feature and must be preserved.
        let client = LlmClient::new(Provider::Anthropic, "claude-opus-4-20250514".to_string())
            .expect("should construct");
        let msgs = vec![
            Message::user("write a poem starting with:"),
            Message::assistant("Roses are red,"),
        ];
        let result = client.to_anthropic_messages(&msgs);
        assert_eq!(result.len(), 2, "prefill preserved for non-thinking");
        assert_eq!(result.last().unwrap()["role"], "assistant");
    }

    #[test]
    fn test_anthropic_thinking_never_empties_message_list() {
        // Edge case: a lone trailing assistant turn must NOT be stripped to an
        // empty list (the `len() > 1` guard). An empty messages array is itself
        // a 400.
        let client = LlmClient::new(Provider::Anthropic, "claude-opus-4-20250514".to_string())
            .expect("should construct")
            .with_thinking("high");
        let msgs = vec![Message::assistant("lone assistant")];
        let result = client.to_anthropic_messages(&msgs);
        assert_eq!(result.len(), 1, "never strip down to empty list");
    }
    ...
    #[test]
    fn test_vision_guard_strips_and_notes_nonvision_model() {
        // #141: a non-vision model (MiniMax-M2.5, catalog supports_vision:false)
        // must have image attachments STRIPPED and an in-band decline note
        // injected — so it cannot fabricate "I can see it."
        let client = LlmClient::new(Provider::Minimax, "MiniMax-M2.5".to_string())
            .expect("should construct");
        let msg = Message::user_with_attachments(
            "What is in this image?",
            vec![Attachment {
        ...
        let out = client.sanitize_images_for_vision(&[msg]);
        assert_eq!(out.len(), 1);
        // Image attachment removed.
        assert!(
            out[0].attachments.is_empty(),
            "image attachment must be stripped for non-vision model"
        );
        // Original text preserved AND in-band note appended.
        assert!(out[0].content.contains("What is in this image?"));
        assert!(
            out[0].content.contains("cannot process images")
                && out[0].content.contains("Do NOT claim to see"),
            "in-band decline note must be injected so the model declines honestly"
        );
    }

    #[test]
    fn test_vision_guard_passthrough_for_vision_model() {
        // A genuine vision model (Anthropic Sonnet) must NOT be touched —
        // images pass through untouched, no note injected.
        let client = LlmClient::new(Provider::Anthropic, "claude-sonnet-4-20250514".to_string())
            .expect("should construct");
        let msg = Message::user_with_attachments(
            "What is in this image?",
        ...
        let out = client.sanitize_images_for_vision(&[msg]);
        assert_eq!(out.len(), 1);
        assert_eq!(out[0].attachments.len(), 1, "vision model keeps the image");
        assert_eq!(out[0].content, "What is in this image?", "no note for vision model");
    }

    #[test]
    fn test_vision_guard_preserves_nonimage_attachments() {
        // Documents/audio must survive the image-only strip on a non-vision model.
        let client = LlmClient::new(Provider::Minimax, "MiniMax-M2.5".to_string())
            .expect("should construct");
        let msg = Message::user_with_attachments(
            "Summarize this.",
        ...
        let out = client.sanitize_images_for_vision(&[msg]);
        assert_eq!(out[0].attachments.len(), 1, "non-image attachment preserved");
        assert_eq!(out[0].attachments[0].mime_type, "application/pdf");
        assert!(out[0].content.contains("cannot process images"));
    }
    ...
    fn clear_zai_env() {
        unsafe {
            ...
            std::env::remove_var("ZAI_REGION");
        }
    }

    #[test]
    fn test_zai_base_url_defaults_to_global_zai() {
        // GLM-5.2 lives on the global platform — default must be api.z.ai,
        // NOT the legacy bigmodel.cn host (which doesn't serve 5.2).
        let _g = QWEN_ENV_MUTEX.lock().unwrap_or_else(|p| p.into_inner());
        clear_zai_env();
        assert_eq!(resolve_zai_base_url(), "https://api.z.ai/api/paas/v4");
        clear_zai_env();
    }

    #[test]
    fn test_zai_base_url_region_cn_opts_into_bigmodel() {
        let _g = QWEN_ENV_MUTEX.lock().unwrap_or_else(|p| p.into_inner());
        clear_zai_env();
        unsafe {
            std::env::set_var("ZAI_REGION", "CN");
        }
        assert_eq!(
    ...
    #[test]
    fn test_zai_base_url_explicit_override_wins() {
        let _g = QWEN_ENV_MUTEX.lock().unwrap_or_else(|p| p.into_inner());
        clear_zai_env();
        unsafe {
            std::env::set_var("ZAI_BASE_URL", "https://proxy.example.com/zai/v4/");
            std::env::set_var("ZAI_REGION", "cn");
        }
        // Explicit full-URL override beats region select; trailing slash trimmed.
        assert_eq!(resolve_zai_base_url(), "https://proxy.example.com/zai/v4");
        clear_zai_env();
    }

    #[test]
    fn test_zai_base_url_env_select_does_not_panic_for_seat() {
        // Confirm the resolver is total: every supported select path yields a
        // non-empty https URL, so a seat on zai/glm-5.2 always gets a base_url.
        let _g = QWEN_ENV_MUTEX.lock().unwrap_or_else(|p| p.into_inner());
        clear_zai_env();
        for (region, expect_host) in [("", "api.z.ai"), ("cn", "open.bigmodel.cn")] {
            clear_zai_env();
            if !region.is_empty() {
                unsafe { std::env::set_var("ZAI_REGION", region); }
            }
            let url = resolve_zai_base_url();
            assert!(url.starts_with("https://"), "must be https: {url}");
            assert!(url.contains(expect_host), "region {region:?} -> {url}");
            assert!(url.ends_with("/api/paas/v4"), "v4 path: {url}");
        }
        clear_zai_env();
    }
    ...
    fn test_qwen_catalog_has_expected_models() {
        ...
        // #141: the exact ids the fabricating seats actually run.
        assert!(ids.contains(&"MiniMax-M2.6-highspeed"));
        assert!(ids.contains(&"qwen3.6:27b"));
    }

    #[test]
    fn test_141_fabricating_seat_ids_caught_as_non_vision() {
        // #141 close-out: ZM runs MiniMax-M2.6-highspeed, raspi runs
        // qwen3.6:27b. Both must resolve to a hard non-vision Err via the
        // exact-match catalog lookup, NOT fall through to the provider-true
        // default (which is what made them fabricate "I can see it").
        use capabilities::supports_image_input;

        let zm = supports_image_input(&Provider::Minimax, "MiniMax-M2.6-highspeed");
        assert!(
            zm.is_err(),
            "MiniMax-M2.6-highspeed must be caught as non-vision, got {zm:?}"
        );

        // raspi's model is Ollama-served; the static catalog lookup runs for
        // Ollama too (its static caps are vision:true) so the exact id is caught.
        let raspi = supports_image_input(&Provider::Ollama, "qwen3.6:27b");
        assert!(
            raspi.is_err(),
            "qwen3.6:27b must be caught as non-vision, got {raspi:?}"
        );

        // And the Qwen-cloud route catches it identically.
        let qwen_cloud = supports_image_input(&Provider::Qwen, "qwen3.6:27b");
        assert!(
            qwen_cloud.is_err(),
            "qwen3.6:27b (Qwen provider) must be caught as non-vision, got {qwen_cloud:?}"
        );
    }
    ...
    #[test]
    fn test_is_retryable_error_transport_errors() {
        // The literal observed production error from bigmodel.cn (reqwest transport)
        let transport = Error::Llm("error sending request for url (https://open.bigmodel.cn/api/paas/v4/chat/completions)".to_string());
        assert!(LlmClient::is_retryable_error(&transport), "transport send error must be retryable");

        // Other transport-class errors
        let timeout = Error::Llm("request timed out".to_string());
        assert!(LlmClient::is_retryable_error(&timeout), "timeout must be retryable");

        let conn_closed = Error::Llm("connection closed".to_string());
        assert!(LlmClient::is_retryable_error(&conn_closed), "connection closed must be retryable");

        let conn_refused = Error::Llm("connection refused".to_string());
        assert!(LlmClient::is_retryable_error(&conn_refused), "connection refused must be retryable");

        // Existing HTTP status-class errors still work
        let rate_limit = Error::Llm("429 Too Many Requests".to_string());
        assert!(LlmClient::is_retryable_error(&rate_limit), "429 must be retryable");

        let server_err = Error::Llm("502 Bad Gateway".to_string());
        assert!(LlmClient::is_retryable_error(&server_err), "502 must be retryable");

        // Client errors must NOT be retryable
        let auth_err = Error::Llm("401 Unauthorized".to_string());
        assert!(!LlmClient::is_retryable_error(&auth_err), "401 must not be retryable");

        let bad_req = Error::Llm("400 Bad Request".to_string());
        assert!(!LlmClient::is_retryable_error(&bad_req), "400 must not be retryable");
    }
}
...
mod additional_tests {
    ...
    /// GLM unit-proof (#185 P0): the key-provider seam end-to-end. The fixed
    /// onboarding persists a plain key to `[credentials]` keyed by `env_key()`;
    /// at gateway startup the S70 bridge iterates `config.credentials` and
    /// `set_var`s each into the process env; `from_config` then reads the env
    /// var and resolves `AuthMethod::ApiKey`, which the request builder maps
    /// unconditionally to the `x-api-key` header (lib.rs:2082). This test models
    /// the post-bridge runtime state (env var set) and proves GLM→ZAI resolves
    /// to the x-api-key seam (NOT Bearer). Complements the live Anthropic-OAuth
    /// E2E which proves the [provider_credentials] cred_type=oauth → Bearer seam.
    ///
    /// Serialized behind a local mutex because it mutates process env (ZAI_API_KEY).
    #[test]
    fn test_glm_plain_key_resolves_to_apikey_xapikey_seam() {
        static ZAI_ENV_MUTEX: std::sync::Mutex<()> = std::sync::Mutex::new(());
        let _guard = ZAI_ENV_MUTEX.lock().unwrap_or_else(|e| e.into_inner());
        // Canonical prefix the fixed onboarding emits (glm→zai via Provider::from_prefix).
        let mut cfg = zeus_core::Config::default();
        cfg.model = "zai/glm-4.6".to_string();
        assert_eq!(cfg.parse_model().0, Provider::Zai);
        assert_eq!(Provider::Zai.env_key(), "ZAI_API_KEY");

        // Model the S70 bridge's runtime export of the [credentials] plain key.
        // `unsafe { set_var }` required by Rust 2024 edition (see clear_qwen_env).
        unsafe {
            std::env::set_var("ZAI_API_KEY", "test-zai-plain-key");
        }
        let client = LlmClient::from_config(&cfg).expect("GLM client should build from config");
        // Plain key → ApiKey variant → x-api-key header (NOT Bearer). The seam.
        let result = match client.auth_method() {
            AuthMethod::ApiKey(k) => {
                assert_eq!(k, "test-zai-plain-key");
                Ok(())
            }
            other => Err(format!("expected AuthMethod::ApiKey (x-api-key seam), got {other:?}")),
        };
        unsafe {
    ...
    // ── #237 Bug-1: Moonshot `thinking` param shape ───────────────────────

    #[test]
    fn test_237_moonshot_nonthinking_omits_thinking_param() {
        // Moonshot rejects {"type":"disabled"} with `invalid thinking: only
        // type=enabled`. A non-thinking Kimi model must OMIT the param entirely.
        let t = LlmClient::thinking_param(&Provider::Moonshot, "kimi-k2.5");
        assert!(
            t.is_none(),
            "non-thinking Moonshot must omit `thinking` (got {t:?})"
        );
    }

    #[test]
    fn test_237_moonshot_thinking_model_sends_enabled() {
        // A thinking-capable Kimi model sends exactly {"type":"enabled"}.
        let t = LlmClient::thinking_param(&Provider::Moonshot, "kimi-k2-thinking");
        assert_eq!(
            t,
            Some(serde_json::json!({"type": "enabled"})),
            "thinking Moonshot model must send type=enabled"
        );
    }

    #[test]
    fn test_237_moonshot_never_sends_disabled() {
        // Hard guard against the 400-causing shape for ANY non-thinking Kimi name.
        for model in ["kimi-k2.5", "kimi-k2.7", "moonshot-v1-8k"] {
            let t = LlmClient::thinking_param(&Provider::Moonshot, model);
            assert_ne!(
                t,
                Some(serde_json::json!({"type": "disabled"})),
                "Moonshot must never send type=disabled (model={model})"
            );
        }
    }

    #[test]
    fn test_237_xiaomimimo_nonthinking_still_disabled() {
        // Regression guard: XiaomiMimo (Bug-4) still needs explicit disabled.
        let t = LlmClient::thinking_param(&Provider::XiaomiMimo, "mimo-v2.5");
        assert_eq!(t, Some(serde_json::json!({"type": "disabled"})));
    }

    #[test]
    fn test_237_non_moonshot_provider_omits_thinking() {
        // Other providers never carry a `thinking` param.
        assert!(LlmClient::thinking_param(&Provider::OpenAI, "gpt-5").is_none());
        assert!(LlmClient::thinking_param(&Provider::Anthropic, "claude-opus-4").is_none());
    }

    // ── #237 Bug-2: tool_call/tool_result id threading (Moonshot) ──────────

    fn assistant_with_tool_call(text: &str, call_id: &str, name: &str) -> Message {
    ...
    #[test]
    fn test_237_paired_tool_call_and_result_survive() {
        // A normal in-window pair must pass through untouched on Moonshot.
        let msgs = vec![
            Message::user("run it"),
            assistant_with_tool_call("calling", "call_1", "shell"),
            Message::tool("call_1", true, "ok"),
        ];
        let out = LlmClient::sanitize_tool_pairs_for_provider(&msgs, &Provider::Moonshot);
        // assistant keeps its tool_call …
        let asst = out.iter().find(|m| m.role == Role::Assistant).unwrap();
        assert_eq!(asst.tool_calls.len(), 1, "paired tool_call must survive");
        // … and the matching result survives too.
        let has_result = out
            .iter()
            .any(|m| m.role == Role::Tool && m.tool_results.iter().any(|r| r.call_id == "call_1"));
        assert!(has_result, "paired tool_result must survive");
    }

    #[test]
    fn test_237_stripped_call_drops_its_dangling_result() {
        // THE BUG: assistant tool_call is orphaned within its turn window (result
        // sits AFTER a turn boundary), so Phase 2a strips the call. Phase 2b must
        // then ALSO drop the now-dangling result — else Moonshot 400s with
        // `tool_call_id is not found`.
        let msgs = vec![
            assistant_with_tool_call("calling", "call_x", "shell"),
            // turn boundary: a new user turn before the result arrives
            Message::user("never mind, do this instead"),
            // dangling result referencing the stripped call
            Message::tool("call_x", true, "late output"),
        ];
        let out = LlmClient::sanitize_tool_pairs_for_provider(&msgs, &Provider::Moonshot);

        // The assistant's orphaned tool_call was stripped …
        let asst_calls: usize = out
            .iter()
            .filter(|m| m.role == Role::Assistant)
            .map(|m| m.tool_calls.len())
            .sum();
        assert_eq!(asst_calls, 0, "orphaned tool_call must be stripped");

        // … and CRITICALLY its dangling result must NOT survive (the #237 fix).
        let dangling = out
            .iter()
            .any(|m| m.role == Role::Tool && m.tool_results.iter().any(|r| r.call_id == "call_x"));
        assert!(
            !dangling,
            "dangling tool_result for a stripped call must be removed (Moonshot 400 guard)"
        );
    }

    #[test]
    fn test_237_every_result_has_matching_call_after_sanitize() {
        // Invariant the fix guarantees for Moonshot: after sanitize, every surviving
        // tool_result references a surviving assistant tool_call id.
        let msgs = vec![
            assistant_with_tool_call("a", "call_1", "shell"),
            Message::tool("call_1", true, "ok"),
            assistant_with_tool_call("b", "call_2", "shell"),
            Message::user("interrupt"),
            Message::tool("call_2", true, "late"), // dangling after boundary
        ];
        let out = LlmClient::sanitize_tool_pairs_for_provider(&msgs, &Provider::Moonshot);

        let surviving_call_ids: std::collections::HashSet<String> = out
            .iter()
            .filter(|m| m.role == Role::Assistant)
            .flat_map(|m| m.tool_calls.iter().map(|tc| tc.id.clone()))
            .collect();
        for m in out.iter().filter(|m| m.role == Role::Tool) {
            for r in &m.tool_results {
                assert!(
                    surviving_call_ids.contains(&r.call_id),
                    "tool_result {} has no matching tool_call after sanitize",
                    r.call_id
                );
            }
        }
    ...
    // ── #44-c Dynamic tool-capability gate tests ──────────────────────────

    #[test]
    fn test_static_tool_gate_anthropic_supports_tools() {
        // Anthropic is a tools-capable provider — static gate should pass
        let caps = capabilities::capabilities(&zeus_core::Provider::Anthropic);
        assert!(caps.supports_tools, "Anthropic static caps: supports_tools must be true");
    }

    #[test]
    fn test_static_tool_gate_openai_supports_tools() {
        let caps = capabilities::capabilities(&zeus_core::Provider::OpenAI);
        assert!(caps.supports_tools, "OpenAI static caps: supports_tools must be true");
    }

    #[test]
    fn test_static_tool_gate_ollama_supports_tools() {
        // Ollama is model-dependent but provider-level flag is true (augmented at runtime)
        let caps = capabilities::capabilities(&zeus_core::Provider::Ollama);
        assert!(caps.supports_tools, "Ollama static caps: supports_tools must be true (model-level via resolver)");
    }

    #[test]
    fn test_static_tool_gate_xiaomimimo_supports_tools() {
        // XiaomiMimo shares catch-all with XAI/Cerebras/DeepSeek — all supports_tools=true
        let caps = capabilities::capabilities(&zeus_core::Provider::XiaomiMimo);
        assert!(caps.supports_tools, "XiaomiMimo static caps: supports_tools must be true");
    }

    #[test]
    fn test_map_openrouter_entry_tools_false_for_text_only_model() {
        // A model entry with no "tools" in supported_parameters → supports_tools=false
        let entry = serde_json::json!({
        ...
        let caps = map_openrouter_entry("some-vendor/text-only-model", &entry);
        assert!(caps.is_some(), "map_openrouter_entry must return Some for valid entry");
        let caps = caps.unwrap();
        assert!(!caps.supports_tools, "text-only model must have supports_tools=false");
    }

    #[test]
    fn test_map_openrouter_entry_tools_true_for_tools_capable_model() {
        // A model entry with "tools" in supported_parameters → supports_tools=true
        let entry = serde_json::json!({
            "id": "some-vendor/tool-capable-model",
            "architecture": {
                "input_modalities": ["text"],
                "modality": "text->text"
            },
        ...
        let caps = map_openrouter_entry("some-vendor/tool-capable-model", &entry);
        assert!(caps.is_some());
        assert!(caps.unwrap().supports_tools, "tools-capable model must have supports_tools=true");
    }

    #[test]
    fn test_map_openrouter_entry_mimo_temperature_lock() {
        // MiMo models via OpenRouter: thinking_mode_temperature_lock=Some(true)
        let entry = serde_json::json!({
            "id": "xiaomi/mimo-vl-7b-rl",
            "architecture": {
                "input_modalities": ["text"],
        ...
        assert!(caps.is_some());
        let caps = caps.unwrap();
        assert_eq!(caps.thinking_mode_temperature_lock, Some(true),
            "MiMo models must set thinking_mode_temperature_lock=Some(true)");
    }
    #[test]
    fn test_99_salvage_inline_tool_calls_openai_shape() {
        let content = r#"Let me run that: {"function":{"name":"shell","arguments":"{\"cmd\":\"ls\"}"}}"#;
        let out = LlmClient::salvage_inline_tool_calls(content).expect("should salvage");
        assert_eq!(out.len(), 1);
    ...
    #[test]
    fn test_99_salvage_inline_tool_calls_bare_and_negative() {
        let out = LlmClient::salvage_inline_tool_calls(r#"{"name":"todo_read","arguments":{}}"#)
            .expect("bare shape salvages");
        assert_eq!(out[0].name, "todo_read");
        assert!(LlmClient::salvage_inline_tool_calls("just thinking out loud").is_none());
        assert!(LlmClient::salvage_inline_tool_calls(r#"{"name":"","arguments":{}}"#).is_none());
    }

    // ── Fallback chain builder tests ──────────────────────────────────────

    #[test]
    fn test_with_fallback_model_appends_to_chain() {
        let client = LlmClient::new(Provider::Ollama, "llama3.2".into())
            .expect("ollama client should construct")
            .with_fallback_model("openai/gpt-4o")
            .with_fallback_model("anthropic/claude-sonnet-4-20250514");
        assert_eq!(client.fallback_models, vec!["openai/gpt-4o", "anthropic/claude-sonnet-4-20250514"]);
    }

    #[test]
    fn test_with_fallback_models_replaces_chain() {
        let client = LlmClient::new(Provider::Ollama, "llama3.2".into())
            .expect("ollama client should construct")
            .with_fallback_model("openai/gpt-4o")
            .with_fallback_models(vec!["anthropic/claude-sonnet-4-20250514".into(), "ollama/mistral".into()]);
        assert_eq!(client.fallback_models, vec!["anthropic/claude-sonnet-4-20250514", "ollama/mistral"]);
    }

    #[test]
    fn test_empty_fallback_chain_by_default() {
        let client = LlmClient::new(Provider::Ollama, "llama3.2".into())
            .expect("ollama client should construct");
        assert!(client.fallback_models.is_empty());
    }
}
