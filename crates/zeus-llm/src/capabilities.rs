...
// ── Stub resolvers (#44-b) ─────────────────────────────────────────────
//
// Static-fallback resolvers for providers without dynamic per-model catalogs.
// Each `resolve()` returns `None`, instructing callers to fall back to the
// static `ProviderCapabilities` table. Future cuts may upgrade these to
// real catalog-fetching impls (e.g. Anthropic /v1/models, Google /models).
//
// `OllamaResolver` (in `ollama.rs`) and `OpenRouterResolver` (in `lib.rs`,
// added in #44-b Step 2) provide the dynamic-catalog cases.
