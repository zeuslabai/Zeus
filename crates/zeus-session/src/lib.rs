...
pub use channel_router::{ChannelKey, ChannelSessionRouter, ContextScope, derive_context_session_id, derive_session_id, derive_titan_session_id};
...
/// Default recent-history window fed to the LLM (#51 seam).
///
/// The number of most-recent messages [`Session::recent_history_for_llm`]
/// passes to the model. Hoisted from the `len() > 50` magic number that was
/// hand-rolled at 3 gateway call sites.
pub const DEFAULT_LLM_HISTORY_WINDOW: usize = 50;
...
impl Session {
    ...
    /// Produce the recent-history window to feed to the LLM.
    ///
    /// This is the engine seam (#51): a single place that turns the session's
    /// full persisted history into the bounded `Vec<Message>` an LLM call
    /// consumes. Previously this window-trim was hand-rolled at every call
    /// site in `gateway.rs` (the `if msgs.len() > N { msgs[len-N..].to_vec() }`
    /// pattern, duplicated 3×). Centralizing it here keeps the policy — how
    /// much history the model sees — in one load-bearing wall.
    ///
    /// Returns a clone (not a borrow) because consumers in `zeus-llm` need an
    /// owned `Vec<Message>` they can mutate (orphan-repair, system-prompt
    /// injection) without holding a read-lock on the session.
    ///
    /// Data flows context → llm: this method PRODUCES llm-input and lives where
    /// the history lives. zeus-llm consumes the `Vec<Message>` with zero new
    /// dependency on zeus-session — the arrow stays
    /// `zeus-session → zeus-core ← zeus-llm`.
    pub fn history_for_llm(&self, window: usize) -> Vec<Message> {
        let len = self.messages.len();
        if window == 0 || len <= window {
            self.messages.clone()
        } else {
            self.messages[len - window..].to_vec()
        }
    }

    /// Convenience wrapper over [`Session::history_for_llm`] using the default
    /// recent-history window. Matches the legacy `len() > 50` call sites.
    pub fn recent_history_for_llm(&self) -> Vec<Message> {
        self.history_for_llm(DEFAULT_LLM_HISTORY_WINDOW)
    }
...
mod tests {
    ...
    /// Build an in-memory session with `n` user messages (no disk I/O) —
    /// just enough to exercise the #51 history_for_llm seam.
    fn session_with_n_messages(n: usize) -> Session {
        let mut s = Session::new(std::path::Path::new("/tmp/seam-test-unused"));
        s.messages = (0..n).map(|i| Message::user(i.to_string())).collect();
        s
    }

    #[test]
    fn test_history_for_llm_under_window_returns_all() {
        let s = session_with_n_messages(10);
        let h = s.history_for_llm(50);
        assert_eq!(h.len(), 10, "history under window should return everything");
        assert_eq!(h.first().unwrap().content, "0");
        assert_eq!(h.last().unwrap().content, "9");
    }

    #[test]
    fn test_history_for_llm_over_window_trims_to_most_recent() {
        let s = session_with_n_messages(120);
        let h = s.history_for_llm(50);
        assert_eq!(h.len(), 50, "history over window should trim to window size");
        // Must keep the TAIL (most recent), not the head.
        assert_eq!(h.first().unwrap().content, "70", "oldest kept = len-window");
        assert_eq!(h.last().unwrap().content, "119", "newest kept = last message");
    }

    #[test]
    fn test_history_for_llm_exact_window_boundary() {
        let s = session_with_n_messages(50);
        let h = s.history_for_llm(50);
        assert_eq!(h.len(), 50, "len == window keeps all (no off-by-one)");
        assert_eq!(h.first().unwrap().content, "0");
    }

    #[test]
    fn test_history_for_llm_zero_window_returns_all() {
        let s = session_with_n_messages(7);
        let h = s.history_for_llm(0);
        assert_eq!(h.len(), 7, "window=0 means unbounded (return all)");
    }

    #[test]
    fn test_recent_history_uses_default_window() {
        let s = session_with_n_messages(60);
        // recent_history_for_llm() == history_for_llm(DEFAULT_LLM_HISTORY_WINDOW)
        let recent = s.recent_history_for_llm();
        let explicit = s.history_for_llm(DEFAULT_LLM_HISTORY_WINDOW);
        assert_eq!(recent.len(), DEFAULT_LLM_HISTORY_WINDOW);
        assert_eq!(recent.len(), explicit.len());
        assert_eq!(recent.last().unwrap().content, "59");
    }
