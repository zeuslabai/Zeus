...
//! # Key Scheme
//!
//! Session IDs are derived deterministically from channel + chat identifiers:
//!
//! ```text
...
mod tests {
    ...
    fn test_session_id_format_is_filesystem_safe() {
        ...
        for id in ids {
            ...
            assert!(!id.contains('\\'), "session id must not contain \\: {}", id);
            assert!(!id.contains('\0'), "session id must not contain NUL: {}", id);
    ...
    async fn test_per_channel_sessions_isolated_on_disk() {
        ...
        assert_ne!(sid_a, sid_b, "different channels must produce different session IDs");
        ...
        assert!(path_a.exists() && path_b.exists(), "both session files must exist");
