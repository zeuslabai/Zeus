...
pub struct InboundConfig {
    ...
    /// Directory to persist inbound attachments to disk.
    /// Defaults to `~/.zeus/inbox`.
    pub inbox_dir: PathBuf,
    /// Maximum file size (MiB) for a single persisted attachment.
    /// Files exceeding this are skipped with a warning.
    pub inbox_max_size_mb: u32,
}
...
impl Default for InboundConfig {
    fn default() -> Self {
        Self {
            ...
            inbox_max_size_mb: 100,
        }
    }
}

/// Default inbox directory: `~/.zeus/inbox`.
...
async fn process_message(
    ...
    let ProcessedAttachments {
        ...
        audio_transcripts,
        text_contents,
    } = process_attachments(&msg.attachments, &channel_type, config).await;
...
async fn process_attachments(
    ...
    for att in attachments {
        ...
        // Persist every attachment to disk so the agent can reference it later.
        match persist_attachment(att, platform, config).await {
            Ok(path) => {
                let display_name = att
                    .filename
                    .as_deref()
                    .unwrap_or("unnamed");
            ...
            Err(e) => {
                warn!("Failed to persist attachment to inbox: {}", e);
            }
        }
...
async fn convert_attachments(attachments: &[ChannelAttachment]) -> Vec<zeus_core::Attachment> {
    process_attachments(attachments, "discord", &InboundConfig::default())
        .await
        .visual
}
...
/// Persist an inbound attachment to the inbox directory on disk.
///
/// Resolves the path as `<inbox_dir>/<YYYY-MM-DD>/<timestamp>-<sanitized_filename>`.
/// Downloads bytes from URL if needed, or uses inline `data`. Skips files
/// exceeding `inbox_max_size_mb`. Returns the persisted path on success.
async fn persist_attachment(
    att: &ChannelAttachment,
    platform: &str,
    config: &InboundConfig,
) -> Result<String, String> {
    let max_bytes = (config.inbox_max_size_mb as usize) * 1024 * 1024;

    // Resolve bytes: prefer inline data, fall back to URL download.
    let bytes: Vec<u8> = if let Some(data) = &att.data {
        if data.len() > max_bytes {
            return Err(format!(
                "attachment too large: {} bytes > {} MiB cap",
                data.len(),
                config.inbox_max_size_mb
            ));
        }
        data.clone()
    } else if let Some(url) = &att.url {
        download_attachment_bytes_capped(url, max_bytes).await?
    } else {
        return Err("attachment has neither data nor url".into());
    };

    // Build path via the shared resolver:
    //   <workspace>/channels/<platform>/received/<YYYY-MM-DD>/<timestamp>-<safe_name>
    // The resolver date-nests and sanitizes the filename (path-traversal safe).
    // We prefix a timestamp so same-named files in one day don't collide.
    let timestamp = chrono::Utc::now().timestamp_millis();
    let raw_name = att.filename.as_deref().unwrap_or("unnamed");
    let file_name = format!("{}-{}", timestamp, raw_name);

    let path = zeus_core::received_file_path(platform, &file_name)
        .map_err(|e| format!("resolve received path: {}", e))?;
    let dir = path
        .parent()
        .ok_or_else(|| "received path has no parent".to_string())?;
    tokio::fs::create_dir_all(dir)
        .await
        .map_err(|e| format!("create received dir: {}", e))?;
    tokio::fs::write(&path, &bytes)
        .await
        .map_err(|e| format!("write inbox file: {}", e))?;

    let path_str = path.to_string_lossy().to_string();
    info!(
        "Persisted attachment to inbox: {} ({} bytes, mime={})",
        path_str,
        bytes.len(),
        att.mime_type
    );
...
mod tests {
    ...
    #[tokio::test]
    async fn test_inbox_dir_default_resolves_to_home_zeus_inbox() {
        let dir = default_inbox_dir();
        let home = dirs::home_dir().unwrap_or_else(|| std::path::PathBuf::from("."));
        assert_eq!(dir, home.join(".zeus").join("inbox"));
    }

    #[tokio::test]
    async fn test_received_path_scheme_includes_platform_date_filename() {
        let date = chrono::Utc::now().format("%Y-%m-%d").to_string();
        let att = ChannelAttachment::from_data(b"hello".to_vec(), "text/plain")
            .with_filename("test.txt");
        let config = InboundConfig::default();
        let path = persist_attachment(&att, "discord", &config)
            .await
            .expect("persist ok");
        // New layout: <workspace>/channels/discord/received/<date>/<ts>-test.txt
        assert!(
            path.contains("/channels/discord/received/"),
            "path should be platform-bucketed: {}",
            path
        );
    ...
    #[tokio::test]
    async fn test_inbound_text_attachment_persists_to_inbox() {
        let att = ChannelAttachment::from_data(b"text content here".to_vec(), "text/plain")
            .with_filename("notes.txt");
        let config = InboundConfig::default();
        let path = persist_attachment(&att, "discord", &config).await.expect("persist ok");
        let contents = tokio::fs::read_to_string(&path).await.expect("read back");
        assert_eq!(contents, "text content here");
        // Cleanup
        let _ = tokio::fs::remove_file(&path).await;
        let date = chrono::Utc::now().format("%Y-%m-%d").to_string();
        let _ = tokio::fs::remove_dir(config.inbox_dir.join(&date)).await;
    }

    #[tokio::test]
    async fn test_inbound_image_attachment_persists_to_inbox() {
        // Minimal 1x1 PNG
        let png: &[u8] = &[
            0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A,
            0x00, 0x00, 0x00, 0x0D, 0x49, 0x48, 0x44, 0x52,
        ];
        let att = ChannelAttachment::from_data(png.to_vec(), "image/png")
            .with_filename("photo.png");
        let config = InboundConfig::default();
        let path = persist_attachment(&att, "discord", &config).await.expect("persist ok");
        let back = tokio::fs::read(&path).await.expect("read back");
        assert_eq!(back, png);
        // Cleanup
        let _ = tokio::fs::remove_file(&path).await;
        let date = chrono::Utc::now().format("%Y-%m-%d").to_string();
    ...
    #[tokio::test]
    async fn test_inbound_unknown_mime_persists_to_inbox() {
        let att = ChannelAttachment::from_data(b"zip bytes".to_vec(), "application/zip")
            .with_filename("archive.zip");
        let config = InboundConfig::default();
        let path = persist_attachment(&att, "discord", &config).await.expect("persist ok");
        let contents = tokio::fs::read_to_string(&path).await.expect("read back");
        assert_eq!(contents, "zip bytes");
        // Cleanup
        let _ = tokio::fs::remove_file(&path).await;
        let date = chrono::Utc::now().format("%Y-%m-%d").to_string();
        let _ = tokio::fs::remove_dir(config.inbox_dir.join(&date)).await;
