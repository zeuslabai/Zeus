...
pub async fn test_provider(
    ...
    match req.provider.as_str() {
        "anthropic" => {
            ...
            // OAuth (sk-ant-oat) tokens are Claude Code-scoped: Anthropic rejects any
            // /v1/messages that doesn't carry the Claude Code identity in `system` and
            // the matching beta headers. Mirror the working chat path (zeus-llm/src/lib.rs)
            // exactly, or validation 401s on a perfectly valid token.
            let body = if is_oauth {
                ...
                r#"{"model":"claude-haiku-4-5-20251001","max_tokens":1,"system":[{"type":"text","text":"You are Claude Code, Anthropic's official CLI for Claude."}],"messages":[{"role":"user","content":"hi"}]}"#
            } else {
            ...
            match resp {
                Ok(r) => {
                    ...
                    if status == 200 {
                        // Fetch the live model list so the WebUI picker reflects what the
                        // account can actually use (fixes #217: stale hardcoded set).
                        let mut models: Vec<String> = {
                            ...
                            match r.send().await {
                                Ok(resp) if resp.status().is_success() => resp
                                    ...
                                    .ok()
                                    .and_then(|b| {
                                        b.get("data").and_then(|d| d.as_array()).map(|arr| {
                                            arr.iter()
                                                .filter_map(|m| {
                                                    m.get("id")
                        ...
                        if models.is_empty() {
                            // Offline/fallback set — bare ids; the UI adds the provider prefix.
                            models = vec![
...
pub async fn fetch_provider_models(
    ...
    match req.provider.as_str() {
        ...
        "zai" => {
            // ZAI (Zhipu / GLM) — OpenAI-compatible /v4/models. GLM-5.2 ships
            // on the GLOBAL platform (api.z.ai); the legacy CN platform
            // (open.bigmodel.cn) does not serve it. Try z.ai first, fall back
            // to bigmodel.
            if key.is_empty() {
                return Ok(Json(json!({ "models": [], "error": "No API key provided" })));
            }
            ...
            for url in [
                    ...
                    .send().await
                {
                    Ok(resp) if resp.status().is_success() => {
                        ...
                        last_err = "empty model list".to_string();
                    }
