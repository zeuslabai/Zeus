        ...
        && let Ok(mut rd) = tokio::fs::read_dir(&dir).await
    {
        while let Ok(Some(entry)) = rd.next_entry().await {
            ...
            if path.extension().and_then(|e| e.to_str()) == Some("json")
                ...
                && let Ok(mut agent) = serde_json::from_str::<Value>(&content)
            {
                // Ensure TUI-expected fields exist with defaults (#249).
                // Agent JSON files may not have these fields — add them
                // so the Advanced→Agents panel can consume real data.
                if let Some(obj) = agent.as_object_mut() {
                    obj.entry("health_score").or_insert(json!(100.0));
                    obj.entry("load_pct").or_insert(json!(0.0));
                    obj.entry("last_heartbeat").or_insert(json!(""));
                    obj.entry("metadata").or_insert(json!({}));
                    obj.entry("current_task").or_insert(json!(null));
                    // host: derive from "address" field if present, else empty.
                    let addr = obj.get("address").cloned().unwrap_or(json!(""));
                    obj.entry("host").or_insert(addr);
                    // channels: not tracked per-agent yet, default 0.
                    obj.entry("channels").or_insert(json!(0));
                }
                agents.push(agent);
            }
        }
...
pub async fn create_agent(
    ...
    let agent = json!({
        ...
        "address": "local",
        "created": now,
        // TUI Advanced→Agents panel fields (#249).
        "health_score": 100.0,
        "load_pct": 0.0,
        "last_heartbeat": now,
        "metadata": {},
