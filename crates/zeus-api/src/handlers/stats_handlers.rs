...
pub async fn get_stats(
    ...
    let sessions = Session::list(&state.config.sessions)
        .await
        .unwrap_or_default();

    let workspace_files = state.workspace.list("").await.unwrap_or_default().len();

    let memory_size = state
        .workspace
        .get_memory()
        .await
        .map(|m| m.len())
