...
pub(crate) async fn run_provision(
    ...
    // Guard (#291): never ship a config.toml whose path fields are temp dirs.
    // The template uses portable `~/` form; this catches a future edit that
    // accidentally injects an expanded temp path (the fleet config-nuke class).
    if config_toml.contains("/var/folders/")
        || config_toml.contains("\"/tmp/")
        || config_toml.contains("\"/tmp.")
    {
        fail(
            "Refusing to provision config.toml: generated config contains a temp path".to_string(),
            &jobs,
        )
        .await;
        return;
