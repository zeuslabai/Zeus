...
use crate::{SharedState, AppState};
...
// #168 Phase 4b — REST-side [Work State] recall.
//
// The Discord/cook path code-enforces a `[Work State]` block before answering
// (Prometheus `cook_with_history_interruptible`). The REST/agent.run seats
// (`chat`, `openai_chat_completions`) bypassed that, so TUI/OpenAI-compat seats
// answered status questions blind to the durable work state. This builds the
// SAME block (one formatter, two callers — `zeus_prometheus::format_work_state`)
// from the API-reachable sources and prepends it to the user message before
// `agent.run`. db_path matches every GoalStack site: `~/.zeus/goals.db`.
async fn build_work_state_prefix(state: &AppState) -> Option<String> {
    // Active goals — open the same GoalStack the cook path / goals_handlers read.
    let active_goals: Vec<String> = dirs::home_dir()
        .map(|h| h.join(".zeus").join("goals.db"))
        .and_then(|db| zeus_prometheus::GoalStack::new(&db).ok())
        .and_then(|stack| stack.active_goals().ok())
        .map(|goals| {
            // Match the cook path's active_goals_summary() format exactly (no drift).
            goals
                .iter()
    ...
    // Incomplete plans — same call the cook path makes.
    let incomplete_plans =
        zeus_prometheus::PlanMode::find_incomplete(state.workspace.root()).await;

    // Pending tasks — REST-side source, mirrors the gateway-threaded string.
    let active_tasks = state.task_store.get_active_tasks().await;
    let pending_tasks: Option<String> = if active_tasks.is_empty() {
        None
    } else {
        Some(
            active_tasks
    ...
    zeus_prometheus::format_work_state(&active_goals, &incomplete_plans, pending_tasks.as_deref())
}
...
pub async fn status(State(state): State<SharedState>) -> Json<Value> {
    // Never block: if a cook holds the write guard, return a lightweight
    // "busy" payload immediately (same UX as /health) rather than starving
    // the reader. tokio's RwLock is writer-preferring, so a blocking read()
    // here flips the TUI badge to "disconnected" while the gateway is alive.
    ...
    // Clone the sessions path, then drop the FS scan OUTSIDE the guard below.
    ...
    let auth_method = if state.config.auth.use_oauth {
        ...
        let env_key = match provider {
            ...
            zeus_core::Provider::KimiCode => "KIMI_CODE_API_KEY",
            zeus_core::Provider::GlmCoding => "GLM_CODING_API_KEY",
            zeus_core::Provider::MinimaxCoding => "MINIMAX_CODING_API_KEY",
            zeus_core::Provider::Zai => "ZAI_API_KEY",
            zeus_core::Provider::Qwen => "QWEN_API_KEY",
            zeus_core::Provider::QwenCoding => "QWEN_CODING_API_KEY",
            zeus_core::Provider::Minimax => "",
            zeus_core::Provider::XiaomiMimo => "XIAOMIMIMO_API_KEY",
            zeus_core::Provider::Sakana => "SAKANA_API_KEY",
    ...
    // Capture owned values, then DROP the guard before the filesystem scan so
    // a concurrent write() on SharedState is never blocked by Session::list().
    ...
    // FS scan happens with no lock held.
    let sessions_count = Session::list(&sessions_path)
        .await
        .map(|s| s.len())
        .unwrap_or(0);
...
pub async fn chat(
    ...
    // #168 Phase 4b — prepend the durable [Work State] block (same formatter as
    // the cook path) so REST/TUI seats recall goals+plans+tasks before answering.
    let cook_message = match build_work_state_prefix(&state).await {
        Some(work_state) => format!("{}\n\n{}", work_state.trim_start(), req.message),
    ...
    //
    // #282 FIX: drop the state read guard BEFORE the cook so the RwLock is not
    // held for up to 30 min. Tokio's RwLock is writer-preferring — holding a
    // read guard across send_and_wait() blocks all writers (config updates,
    // session saves, agent state changes) and queues all subsequent readers
    // (Discord, heartbeat), causing the gateway to wedge (opus) or die (fugu).
    // Mirror the streaming path (openai_chat_stream) which already drops its
    // guard before send_and_stream.
    let agent_arc = state.default_agent.clone();
    let inbox = state.agent_inbox.clone();
    let agent_run_lock = state.agent_run_lock.clone();

    if let Some(ref agent) = agent_arc {
        drop(state);

        // Prefer inbox path (non-blocking, serialized by consumer) over Mutex path.
        let response = if let Some(inbox) = inbox {
            inbox.send_and_wait_with_options(
                cook_message.clone(),
                ...
                zeus_core::inbox::InboxSendOptions::new(
                    vec![],
            ...
            let _run_guard = agent_run_lock.lock().await;
            let mut agent_guard = agent.write().await;
            agent_guard.run(&cook_message).await
                .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
        };
...
async fn openai_chat_non_stream(
    ...
    // #168 Phase 4b — prepend the durable [Work State] block (same formatter as
    // the cook path) so OpenAI-compat seats recall work state before answering.
    let cook_msg = match build_work_state_prefix(&state).await {
        Some(work_state) => format!("{}\n\n{}", work_state.trim_start(), last_user_msg),
        None => last_user_msg.clone(),
    };
    ...
    //
    // #282 FIX: drop the state read guard BEFORE the cook (same as chat() fix).
    // Clone what we need from state, then drop the guard so the RwLock is not
    // held for up to 30 min during send_and_wait().
    let agent_arc = state.default_agent.clone();
    let inbox = state.agent_inbox.clone();
    let agent_run_lock = state.agent_run_lock.clone();
    // Economy spend is non-fatal; do it before dropping the guard.
    {
    ...
    if let Some(ref agent) = agent_arc {
        ...
        let response_text = if let Some(inbox) = inbox {
            inbox.send_and_wait_with_options(
                cook_msg.clone(),
                None,
                zeus_core::inbox::InboxSendOptions::new(
...
async fn openai_chat_stream(
    ...
    // #168 Phase 4b — prepend the durable [Work State] block (same formatter as
    // the cook path) so the OpenAI *streaming* completions seat recalls work
    // state before answering, identical to chat() + openai_chat_non_stream().
    // Built while the read lock is held (it's dropped before send_and_stream).
    let cook_msg = match build_work_state_prefix(&state_read).await {
        Some(work_state) => format!("{}\n\n{}", work_state.trim_start(), last_user_msg),
        None => last_user_msg.clone(),
    };
    ...
    if let Some(inbox) = state_read.agent_inbox.as_ref().cloned() {
        ...
        let mut stream_rx = inbox.send_and_stream_with_options(
            cook_msg,
            Some(zeus_core::ChannelSource {
                channel_type: "tui".to_string(),
                channel_id: None,
...
mod lockfree_tests {
    ...
    /// Core regression: while a cook holds the write guard, /v1/status must
    /// return promptly with status="busy" rather than blocking on the lock
    /// (which previously starved the reader across Session::list and flipped
    /// the TUI badge to "disconnected").
    #[tokio::test]
    async fn status_returns_busy_under_write_contention() {
        let state = test_state();
        let app = crate::create_test_router(state.clone());

        // Simulate a cook: hold the write guard for the duration of the request.
        ...
        .await
        .expect("status handler blocked on write guard — lock starvation regression")
        .unwrap();
    ...
    /// When no writer holds the guard, /v1/status returns the full payload
    /// (status="ok") and the FS scan (Session::list) runs outside the guard.
    #[tokio::test]
    async fn status_returns_ok_when_uncontended() {
        let state = test_state();
        let app = crate::create_test_router(state);
        ...
        .await
        .expect("status handler timed out")
        .unwrap();
    ...
    /// Same contract for /v1/stats — busy under contention, never blocks.
    #[tokio::test]
    async fn stats_returns_busy_under_write_contention() {
        let state = test_state();
        let app = crate::create_test_router(state.clone());
        ...
        .await
        .expect("stats handler blocked on write guard — lock starvation regression")
        .unwrap();
