...
"#;
    ...
    /// Serializes + sandboxes ZEUS_HOME for tests that touch allowlist.json.
    /// Mirrors zeus-core's redirect_zeus_home: the Mutex prevents the two
    /// allowlist tests from racing on the env var / file, and the TempDir
    /// keeps them off the real ~/.zeus (the source of a CI flake where the
    /// PUT test's write clobbered the GET test's default-list assertion).
    fn redirect_zeus_home() -> (std::sync::MutexGuard<'static, ()>, tempfile::TempDir) {
        static ZEUS_HOME_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());
        let lock = ZEUS_HOME_LOCK
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
        let tmp = tempfile::tempdir().expect("create tempdir for ZEUS_HOME");
        // SAFETY: guarded by ZEUS_HOME_LOCK — no concurrent env mutation here.
        unsafe {
    ...
    #[tokio::test]
    async fn test_onboarding_skills_returns_catalog() {
        // Seed a workspace skills/ dir with two SKILL.md entries — one in the
        // built-in default set (code-review), one not (my-custom-skill).
        let state = test_state();
        {
            let st = state.read().await;
            let skills_dir = st.config.workspace.join("skills");
            for (id, desc) in [
                ("code-review", "Review code for correctness"),
                ("my-custom-skill", "A custom test skill"),
            ] {
        ...
        // Flat array of {id, name, description, default} — the WebUI parser shape.
        let arr = json.as_array().expect("response must be a flat array");
        assert_eq!(arr.len(), 2);
        for item in arr {
            assert!(item["id"].is_string());
            assert!(item["name"].is_string());
            assert!(item["description"].is_string());
            assert!(item["default"].is_boolean());
        }
        let by_id = |id: &str| {
            arr.iter()
                .find(|v| v["id"] == id)
        ...
        // code-review is in WorkspaceTemplate::builtins() default_skills → default: true
        assert_eq!(by_id("code-review")["default"], true);
        assert_eq!(by_id("my-custom-skill")["default"], false);
        assert_eq!(by_id("my-custom-skill")["description"], "A custom test skill");
    }

    #[tokio::test]
    async fn test_onboarding_skills_empty_dir_returns_empty_array() {
        // No skills/ dir at all → empty array (WebUI keeps its fallback list).
        let state = test_state();
        let app = test_app(state);
        let req = Request::builder()
            .uri("/v1/onboarding/skills")
    ...
    async fn test_marketplace_publish_response_fields() {
        ...
        let body = axum::body::to_bytes(resp.into_body(), usize::MAX)
            .await
            .unwrap();
        let pr: crate::handlers::marketplace_dto::PublishResponse = serde_json::from_slice(&body).unwrap();
    ...
    async fn test_marketplace_list_response_fields() {
        ...
        let body = axum::body::to_bytes(resp.into_body(), usize::MAX)
            .await
            .unwrap();
        let lr: crate::handlers::marketplace_dto::MarketplaceListResponse = serde_json::from_slice(&body).unwrap();
    ...
    async fn test_marketplace_trade_response_fields() {
        ...
        let body = axum::body::to_bytes(resp.into_body(), usize::MAX)
            .await
            .unwrap();
        let tr: crate::handlers::marketplace_dto::TradeResponse = serde_json::from_slice(&body).unwrap();
    ...
    async fn test_marketplace_ledger_response_structure() {
        let state = test_state();
        // Credit some tokens to the marketplace_store (source of truth post P0-1c cut-5)
        {
            let s = state.read().await;
            s.marketplace_store
        ...
        let body = axum::body::to_bytes(resp.into_body(), usize::MAX)
            .await
            .unwrap();
        let lr: crate::handlers::marketplace_dto::LedgerResponse = serde_json::from_slice(&body).unwrap();
    ...
    async fn test_marketplace_reputation_response_fields() {
        let state = test_state();
        // Record some trades for reputation (marketplace_store = source of truth post P0-1c cut-4)
        {
            let s = state.read().await;
            s.marketplace_store.record_trade_success("rep-agent").await;
            s.marketplace_store.record_trade_success("rep-agent").await;
            s.marketplace_store.record_trade_failure("rep-agent").await;
        }
        let app = test_app(state);
        let req = Request::builder()
        ...
        let body = axum::body::to_bytes(resp.into_body(), usize::MAX)
            .await
            .unwrap();
        let rr: crate::handlers::marketplace_dto::ReputationResponse = serde_json::from_slice(&body).unwrap();
    ...
    #[tokio::test]
    async fn test_economy_transfer_requires_auth() {
        // No Authorization header → 401 when the gate is configured.
        // SAFETY: test-only env mutation; this test is serialized by tokio
        unsafe { std::env::set_var("ZEUS_TRANSFER_ADMIN_TOKEN", "test-transfer-token-s32c") };
        let tmp = tempfile::TempDir::new().unwrap();
        let state = test_state_with_tempdir(&tmp);
        ...
        let req = Request::builder()
            ...
            .header("content-type", "application/json")
            .body(Body::from(
                r#"{"from":"sender-1","to":"receiver-1","amount":100}"#,
            ))
            .unwrap();
        let resp = app.oneshot(req).await.unwrap();
        assert_eq!(resp.status(), StatusCode::UNAUTHORIZED);
    ...
    #[tokio::test]
    async fn test_economy_transfer_invalid_token_rejected() {
    ...
    // ── #224: seed_workspace_skills ──

    #[test]
    fn test_seed_workspace_skills_copies_global_library() {
        // Route ZEUS_HOME to a tempdir so the global skill library is isolated.
        // Must go through redirect_zeus_home(): unguarded set_var races the
        // allowlist tests' ZEUS_HOME_LOCK and flakes under parallel test load.
        let (_lock, tmp) = redirect_zeus_home();

        // Build a fake global library: ~/.zeus/skills/git/SKILL.md
        let global = tmp.path().join("skills").join("git");
        std::fs::create_dir_all(&global).unwrap();
        std::fs::write(global.join("SKILL.md"), "# git skill").unwrap();

        // Fresh workspace with no skills dir
        let workspace = tmp.path().join("workspace");
        std::fs::create_dir_all(&workspace).unwrap();

        crate::handlers::onboarding_handlers::seed_workspace_skills(&workspace);

        let seeded = workspace.join("skills").join("git").join("SKILL.md");
        assert!(seeded.is_file(), "skill should be mirrored into workspace");
        assert_eq!(std::fs::read_to_string(seeded).unwrap(), "# git skill");

        // Second call is a no-op: existing workspace skills are not clobbered
        std::fs::write(
            workspace.join("skills").join("git").join("SKILL.md"),
            "edited",
        )
        .unwrap();
        crate::handlers::onboarding_handlers::seed_workspace_skills(&workspace);
        assert_eq!(
            std::fs::read_to_string(workspace.join("skills/git/SKILL.md")).unwrap(),
            "edited",
            "seed must not overwrite a populated workspace skills dir"
        );
    }
}
