...
pub async fn register_team_wallets(state: &SharedState, team: &[TeamMember]) {
    ...
    for member in team {
        let balance = marketplace.ledger.balance(&member.agent_id).await;
        if balance == 0 {
            marketplace
                .ledger
                .credit(
...
pub async fn settle_mission_payments(state: &SharedState, store: &PantheonStore, mission_id: &str) {
    ...
    for task in &mission.tasks {
        if task.status == TaskStatus::Approved || task.status == TaskStatus::Complete {
            ...
            // Record reputation (in-memory + write-through to SQLite)
            marketplace.reputation.record_trade_success(agent_id).await;
            state_guard
                .marketplace_store
...
pub async fn pantheon_economy(State(state): State<SharedState>) -> impl IntoResponse {
    ...
    let balances = marketplace.ledger.all_balances().await;
    let reputations = marketplace.reputation.all_reputations().await;
    ...
    let agents: Vec<Value> = balances
        ...
        .map(|(agent_id, balance)| {
            ...
            let (trust, completed, failed, badge) = match rep {
                Some(r) => {
                    let fake_row = super::marketplace_store::AgentReputationRow {
                        agent_id: r.agent_id.clone(),
                        trust_score: r.trust_score,
                        total_trades: r.successful_trades + r.failed_trades,
                        successful_trades: r.successful_trades,
                        failed_trades: r.failed_trades,
                        avg_skill_rating: 0.0,
                        skill_rating_count: 0,
                        last_activity: String::new(),
                    };
                    let badge = super::marketplace_store::compute_badge(&fake_row);
    ...
    Json(json!({
        "stats": {
            ...
            "total_token_supply": stats.total_token_supply,
...
/// Bridge between prometheus CookingEvent stream and Pantheon room messages.
///
/// Subscribes to an EventEmitter and posts plan_progress messages to the room
/// for tool executions and iteration progress. Used by multi-step spawns.
#[allow(dead_code)]
pub struct CookingEventBridge {
    store: PantheonStore,
    spawn_id: String,
    agent_name: String,
    room_id: String,
...
impl CookingEventBridge {
    ...
    /// Start listening to CookingEvents and bridging them to room messages.
    /// Returns a JoinHandle that completes when the emitter is dropped.
    pub fn start(self, emitter: &zeus_prometheus::EventEmitter) -> tokio::task::JoinHandle<()> {
        ...
        tokio::spawn(async move {
            loop {
                match rx.recv().await {
                    Ok(event) => {
                        let (content, is_terminal) = match &event {
                            zeus_prometheus::CookingEvent::ToolExecutionStart { name, .. } => {
                                step_counter += 1;
                                (format!("Running tool: **{}**", name), false)
                            }
                            zeus_prometheus::CookingEvent::ToolExecutionComplete {
                                name,
                                is_error,
                                duration_ms,
                                ..
                                ...
                                (
                                    format!("Tool **{}** {} ({}ms)", name, status, duration_ms),
                                    false,
                                )
                            }
                            zeus_prometheus::CookingEvent::MessageStart { .. } => {
                                step_counter += 1;
                                ("Thinking...".to_string(), false)
                            }
                            zeus_prometheus::CookingEvent::MessageEnd { .. } => {
                                ("Response ready".to_string(), false)
                            }
                            zeus_prometheus::CookingEvent::SessionEnd { .. } => {
                                ("Execution complete".to_string(), true)
                            }
                            zeus_prometheus::CookingEvent::Warning { message, .. } => {
                                (format!("Warning: {}", message), false)
                            }
                            _ => continue, // skip text deltas, thinking deltas, etc.
                        };
                        ...
                        let msg = RoomMessage {
                            ...
                            metadata: Some(json!({
                                "step": step_counter,
                                "total": step_counter + 1, // estimated, grows dynamically
                                "spawn_id": &spawn_id,
                            })),
                            reply_to: None,
                            edited: false,
...
async fn handle_slash_command(
    ...
    let response = match cmd.as_str() {
        ...
        "/economy" => {
            let st = state.read().await;
            let mp = &st.marketplace;
            let stats = mp.stats().await;
            let reps = mp.reputation.all_reputations().await;
            let mut lines = vec![
                ...
                format!(
                    ...
                    stats.total_token_supply, stats.total_agents
                ),
            ];
            if !reps.is_empty() {
        ...
        "/skills" | "/list-skills" => {
            let st = state.read().await;
            let listings = st.marketplace.registry.list_active().await;
        ...
        "/search" => {
            if _args.is_empty() {
                ...
                let mut results = st.marketplace.registry.search_by_name(&query).await;
                if results.is_empty() {
                    results = st.marketplace.registry.search_by_tag(&query).await;
                }
                if results.is_empty() {
                    results = st.marketplace.registry.search_by_capability(&query).await;
                }
                if results.is_empty() {
        ...
        "/balance" => {
            let st = state.read().await;
            let bal = st.marketplace.ledger.balance(sender_id).await;
            format!("**{}** — Balance: **{}** credits", sender_name, bal)
        }
        "/balances" => {
            let st = state.read().await;
            let balances = st.marketplace.ledger.all_balances().await;
        ...
        "/publish" => {
            if _args.is_empty() {
                ...
                let listing = zeus_marketplace::SkillListing::new(
                    name.clone(),
                    sender_id.to_string(),
                    format!("Published by {} in #{}", sender_name, room.name),
                )
                .with_price(price)
                .with_tags(tags);

                let st = state.read().await;
                match st.marketplace.registry.publish(listing).await {
                    Ok(id) => {
                        // Write-through to SQLite for persistence across restarts
                    ...
                    Err(e) => format!("Failed to publish: {}", e),
                }
            }
        }
        "/buy" => {
            if _args.is_empty() {
                ...
                let st = state.read().await;
                let listing = match st.marketplace.registry.get(skill_id).await {
                    Ok(l) => l,
                    Err(_) => {
                ...
                let buyer_balance = st.marketplace.ledger.balance(sender_id).await;
                if buyer_balance < listing.price {
                    ...
                    {
                        Ok(_) => {
                            let _ = st.marketplace.registry.record_download(skill_id).await;
                            ...
                            if !transfer_ok {
                                tracing::warn!(
                                    "SQLite transfer write-through failed for buy {}",
                                    skill_id
                                );
                            }
                            format!(
                                ...
                                listing.price,
                                listing.publisher_id,
                                st.marketplace.ledger.balance(sender_id).await
                            )
                        }
...
pub struct AgentReputation {
    ...
    pub quality_rating: f64,     // 0.0 - 5.0
