...
pub async fn deploy_history(
    ...
    let deployments = {
        let state_guard = state.read().await;
        state_guard.deploy_store.list_recent_deployments(limit).await
    };

    // Convert DeploymentRow → DeploymentResponse with target_name join.
    // The TUI's DeploymentResponse expects `target_name` (not `target_id`),
    // so we resolve each deployment's target_id → target name+provider.
    ...
    for row in deployments {
        let (target_name, provider) = {
            ...
            state_guard
                .deploy_store
                .get_target(&row.target_id)
                .await
                .map(|t| (t.name, t.provider))
                .unwrap_or_else(|| (row.target_id.clone(), String::new()))
        };
        responses.push(deploy_store::DeploymentResponse::from_row(row, &target_name, &provider));
    }

    Json(json!({ "deployments": responses, "total": responses.len() }))
}
