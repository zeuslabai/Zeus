...
pub struct MarketplaceStore {
    ...
    /// #81a Phase 2 — canonical settlement sink. Off by default: when the
    /// coordinator is disabled (the default), `transfer()` settles purely on
    /// this store's SQLite token ledger exactly as before — no live money-path
    /// change. When enabled (operator flips `ZEUS_UNIFY_MARKETPLACE=1`), the
    /// trade is *also* settled atomically onto the canonical
    /// `zeus-economy::TokenLedger`, making economy.db the single source of truth.
    settlement: zeus_marketplace::settlement::SettlementCoordinator,
}
...
impl MarketplaceStore {
    ...
    pub fn new(db_path: &PathBuf) -> Result<Self, String> {
        ...
        Ok(Self {
            ...
            // #81a Phase 2 — default disabled. Reads ZEUS_UNIFY_MARKETPLACE so
            // the operator can flip the canonical-ledger settlement on without a
            // code change, but stays inert (legacy SQLite path only) by default.
            settlement: Self::default_settlement(db_path),
        })
    }

    /// Build the settlement coordinator for #81a. Off by default; when
    /// `ZEUS_UNIFY_MARKETPLACE` is set, settlement is routed to the canonical
    /// `zeus-economy::TokenLedger` at `economy.db` (sibling of the marketplace
    /// db). If the canonical ledger can't be opened, falls back to a disabled
    /// (inert) coordinator so the legacy path is never broken.
    fn default_settlement(
        marketplace_db_path: &std::path::Path,
    ) -> zeus_marketplace::settlement::SettlementCoordinator {
        use zeus_marketplace::settlement::SettlementCoordinator;
        let economy_db = marketplace_db_path
            .parent()
            .map(|dir| dir.join("economy.db"))
            .unwrap_or_else(|| PathBuf::from("economy.db"));
        match SettlementCoordinator::new(&economy_db) {
            Ok(coord) => coord.with_env(),
            Err(e) => {
                warn!(
                    "marketplace: canonical settlement ledger unavailable ({e}); \
                     staying on legacy SQLite ledger only"
                );
                SettlementCoordinator::disabled()
            }
        }
    }
    ...
    /// Search active listings by tag (JSON array substring match).
    pub async fn search_by_tag(&self, tag: &str) -> Vec<SkillListingRow> {
        let db = self.db.lock().await;
        let pattern = format!("%\"{}\"%", tag.to_lowercase());
        let mut stmt = match db.prepare(
            "SELECT id, name, description, publisher_id, capabilities, tags, price, version, rating, rating_count, downloads, active, source, metadata, created_at, updated_at
             FROM skill_listings
             WHERE active = 1 AND LOWER(tags) LIKE ?1
             ORDER BY downloads DESC"
        ) {
            Ok(s) => s,
            Err(_) => return vec![],
    ...
    /// Search active listings by capability (JSON array substring match).
    pub async fn search_by_capability(&self, capability: &str) -> Vec<SkillListingRow> {
        let db = self.db.lock().await;
        let pattern = format!("%\"{}\"%", capability.to_lowercase());
        let mut stmt = match db.prepare(
            "SELECT id, name, description, publisher_id, capabilities, tags, price, version, rating, rating_count, downloads, active, source, metadata, created_at, updated_at
             FROM skill_listings
             WHERE active = 1 AND LOWER(capabilities) LIKE ?1
             ORDER BY downloads DESC"
        ) {
            Ok(s) => s,
            Err(_) => return vec![],
    ...
    /// Search active listings by publisher id (exact match).
    pub async fn search_by_publisher(&self, publisher_id: &str) -> Vec<SkillListingRow> {
        let db = self.db.lock().await;
        let mut stmt = match db.prepare(
            "SELECT id, name, description, publisher_id, capabilities, tags, price, version, rating, rating_count, downloads, active, source, metadata, created_at, updated_at
             FROM skill_listings
             WHERE active = 1 AND publisher_id = ?1
             ORDER BY downloads DESC"
        ) {
            Ok(s) => s,
            Err(_) => return vec![],
        };
        match stmt.query_map(params![publisher_id], |row| Ok(row_to_listing(row))) {
            Ok(rows) => rows.filter_map(|r| r.ok()).collect(),
            Err(_) => vec![],
        }
    }

    /// All token balances keyed by agent_id.
    pub async fn all_balances(&self) -> std::collections::HashMap<String, i64> {
        let db = self.db.lock().await;
        let mut stmt = match db.prepare("SELECT agent_id, balance FROM token_balances") {
            Ok(s) => s,
    ...
    pub async fn all_reputations(&self) -> Vec<AgentReputationRow> {
        let db = self.db.lock().await;
        let mut stmt = match db.prepare(
    ...
    pub async fn transfer(&self, from: &str, to: &str, amount: u64, reason: &str) -> bool {
        ...
        // Drop the SQLite lock before touching the canonical ledger to avoid
        // holding two ledgers' locks at once.
        drop(db);

        // #81a Phase 2 — canonical settlement mirror. Off by default: when the
        // coordinator is disabled this returns `Skipped` and nothing else
        // happens (legacy path above is authoritative). When the operator has
        // flipped `ZEUS_UNIFY_MARKETPLACE=1`, the same trade settles atomically
        // onto `zeus-economy::TokenLedger` (economy.db) so balances converge on
        // the single source of truth. Fee is 0 until a fee policy lands.
        match self.settlement.settle(from, to, amount, 0, reference_id) {
            Ok(outcome) if outcome.is_settled() => {
                tracing::debug!(
                    %from, %to, amount,
                    "marketplace transfer mirrored onto canonical economy ledger"
                );
            }
            Ok(_) => { /* Skipped — disabled by default, legacy path stands */ }
            Err(e) => {
                // Canonical settlement is a mirror; a failure here must not
                // reverse the legacy transfer that already committed. Log loud.
                warn!(
                    %from, %to, amount,
                    "marketplace: canonical settlement mirror failed: {e}"
                );
            }
        }
        true
    ...
    /// Persist a proposed trade (web4 P0-1c cut-10: replaces the in-memory
    /// TradeManager as the trade source of truth). Returns the trade id.
    pub async fn propose_trade(&self, trade: &marketplace_dto::Trade) -> Result<String, String> {
        let db = self.db.lock().await;
        db.execute(
            "INSERT INTO trades (id, buyer_id, seller_id, skill_id, offered_price, final_price, status, message, created_at, updated_at)
            ...
            params![
                ...
                trade.seller_id,
                trade.skill_id,
                trade.offered_price as i64,
                trade.final_price.map(|p| p as i64),
                trade.message,
                trade.created_at.to_rfc3339(),
                trade.updated_at.to_rfc3339(),
            ],
        )
        .map_err(|e| format!("trade insert failed: {e}"))?;
        Ok(trade.id.clone())
    }
