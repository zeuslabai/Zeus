...
pub struct NodeInfo {
    ...
    /// Transport protocol: "websocket", "relay-tcp", etc. (#249)
    #[serde(default)]
    pub transport: String,
    /// Round-trip time in milliseconds (keepalive ping→pong delta). (#249)
    #[serde(default)]
    pub rtt_ms: u64,
}
...
pub struct ConnectedNode {
    ...
    /// Timestamp of the last keepalive Ping sent to this node. (#249)
    pub last_ping_at: Option<DateTime<Utc>>,
    /// Last measured round-trip time in ms (ping→pong delta). (#249)
    pub rtt_ms: u64,
}
...
impl NodeRegistry {
    ...
    /// Record that a keepalive Ping was sent to a node. (#249)
    /// Called just before the Ping message is sent over WS.
    pub fn record_ping(&self, node_id: &str) {
        if let Some(mut node) = self.nodes.get_mut(node_id) {
            node.last_ping_at = Some(Utc::now());
        }
    }

    /// Record that a Pong was received from a node, computing RTT. (#249)
    /// Called when the hub receives a Pong from the node.
    pub fn record_pong(&self, node_id: &str) {
        if let Some(mut node) = self.nodes.get_mut(node_id) {
            if let Some(ping_at) = node.last_ping_at.take() {
                let delta = Utc::now().signed_duration_since(ping_at);
                node.rtt_ms = delta.num_milliseconds().max(0) as u64;
            }
        }
    }
}
