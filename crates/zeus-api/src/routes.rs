...
fn build_base_router(state: SharedState, rate_limit_config: Option<RateLimitConfig>) -> Router {
    let mut router = Router::new()
        ...
        // Device-code OAuth (#216b): Qwen/MiniMax via provider-specific modules
        .route("/v1/auth/device/start", post(handlers::device_code_start))
        .route("/v1/auth/device/poll", get(handlers::device_code_poll))
        // CLI credential import (#216b): Gemini CLI creds
        .route("/v1/auth/cli-creds", get(handlers::cli_creds_status))
        .route(
            "/v1/auth/cli-creds/import",
            post(handlers::cli_creds_import),
        )
        // Onboarding
        ...
        .route(
            "/v1/aegis/approvals/stream",
            get(handlers::approvals_stream),
        )
        // Agent Tasks (S52-T1)
        ...
        .route(
            "/v1/pantheon/missions/:id/reject",
            post(handlers::reject_mission),
        )
        .route(
