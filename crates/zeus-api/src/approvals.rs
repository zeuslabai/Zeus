...
mod tests {
    ...
    // ------------------------------------------------------------------
    // B2 — SSE-stream-relevant broadcast flow
    //
    // These exercise the exact event source that `approvals_stream`
    // (security_handlers.rs) subscribes to and JSON-serializes: a fresh
    // subscriber receives ApprovalPending on submit and ApprovalResolved
    // on approve/deny, and each event serializes to the wire shape the
    // SSE handler emits.
    // ------------------------------------------------------------------

    #[test]
    fn test_approval_stream_pending_then_resolved() {
        let mut q = ApprovalQueue::new(vec!["shell".into()], 1800);
        let mut rx = q.subscribe();

        // submit → subscriber sees ApprovalPending
        let (id, _rx) = q.submit("shell".into(), json!({"cmd": "ls"}), None);
        match rx.try_recv().unwrap() {
            ApprovalEvent::ApprovalPending { approval } => {
        ...
        // approve → subscriber sees ApprovalResolved{Approved}
        q.approve(&id).unwrap();
        match rx.try_recv().unwrap() {
            ApprovalEvent::ApprovalResolved { id: rid, status } => {
                assert_eq!(rid, id);
    ...
    #[test]
    fn test_approval_stream_deny_resolves() {
        let mut q = ApprovalQueue::new(vec!["shell".into()], 1800);
        let mut rx = q.subscribe();
        ...
        q.deny(&id, Some("nope".into())).unwrap();
        match rx.try_recv().unwrap() {
            ApprovalEvent::ApprovalResolved { id: rid, status } => {
                assert_eq!(rid, id);
                assert_eq!(
    ...
    #[test]
    fn test_approval_event_serializes_for_sse() {
        // The SSE handler does serde_json::to_string(&event); confirm the
        // wire shape is well-formed for both variants.
        let mut q = ApprovalQueue::new(vec!["shell".into()], 1800);
        let mut rx = q.subscribe();
        let (id, _rx) = q.submit("shell".into(), json!({"k": "v"}), None);

        let pending = rx.try_recv().unwrap();
        let s = serde_json::to_string(&pending).expect("pending serializes");
        assert!(s.contains("shell"));

        q.approve(&id).unwrap();
        let resolved = rx.try_recv().unwrap();
        let s = serde_json::to_string(&resolved).expect("resolved serializes");
