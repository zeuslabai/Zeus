    ...
    loop {
        tokio::select! {
            ...
            _ = ping_interval.tick() => {
                // Record ping timestamp for RTT measurement (#249).
                {
                    let st = state.read().await;
                    st.node_registry.record_ping(&node_id);
                }
...
async fn handle_node_message(
    ...
    match msg {
        ...
        NodeClientMessage::Pong => {
            ...
            // Record RTT from the keepalive ping→pong delta (#249).
            state.read().await.node_registry.record_pong(&node_id);
        }
    }
}
