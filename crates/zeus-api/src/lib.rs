...
use zeus_marketplace::Marketplace;
...
impl AppState {
    ...
    pub async fn boot(state: &SharedState) {
        ...
        // Hydrate in-memory Marketplace from SQLite persistence.
        // This restores listings, balances, and reputation across gateway restarts.
        Self::sync_marketplace_from_store(state).await;
    ...
    /// Load persisted marketplace data (listings, balances, reputation) from
    /// `MarketplaceStore` (SQLite) into the in-memory `Marketplace` so that
    /// Pantheon and economy handlers see previously-published skills after a restart.
    async fn sync_marketplace_from_store(state: &SharedState) {
        let ms = state.read().await.marketplace_store.clone();

        // 1. Hydrate listings
        let rows = ms.list_active_listings().await;
        ...
        for row in &rows {
            ...
            let listing =
                zeus_marketplace::SkillListing::new(&row.name, &row.description, &row.publisher_id)
                    .with_capabilities(caps)
                    .with_tags(tags)
                    .with_price(row.price)
                    .with_version(&row.version);
            // Publish into in-memory registry (updates id to a new UUID, but the
            // registry key is name-based so lookups still work).
            let st = state.read().await;
            if let Ok(_id) = st.marketplace.registry.publish(listing).await {
                listing_count += 1;
            }
        }

        // 2. Hydrate token balances
        let st = state.read().await;
        // Seed the "system" wallet and any agent wallets with existing SQLite balances
        for row in &rows {
            let bal = ms.get_balance(&row.publisher_id).await;
            if bal > 0 {
                st.marketplace
                    .ledger
                    .credit(&row.publisher_id, bal as u64, "boot_sync")
                    .await;
            }
        }

        // 3. Hydrate reputation from SQLite
        for row in &rows {
            let rep = ms.get_reputation(&row.publisher_id).await;
            for _ in 0..rep.successful_trades {
                st.marketplace
                    .reputation
                    .record_trade_success(&row.publisher_id)
                    .await;
            }
            for _ in 0..rep.failed_trades {
                st.marketplace
                    .reputation
                    .record_trade_failure(&row.publisher_id)
                    .await;
            }
        }
        ...
        if listing_count > 0 {
            tracing::info!(
                "Marketplace hydrated from SQLite: {} listings restored",
                listing_count
            );
        }
    }
