...
impl CookingLoop {
    ...
    pub async fn run_with_history(
            ...
            if response.tool_calls.is_empty() || response.stop_reason != StopReason::ToolUse {
                ...
                if response.tool_calls.is_empty() {
                    ...
                    if stuck_streak >= STUCK_AGENT_THRESHOLD {
                        let msg = format!(
                            "stuck-agent: emitting same scratchpad without tool calls for {} consecutive iterations (iter {})",
                ...
                let under_retry_limit = planning_retry_count < 2;
        ...
        // Mark checkpoint session as completed
        if let Some(ref store) = self.checkpoint_store {
            store.mark_completed(&session_id).await;
        }
...
mod tests {
    ...
    fn test_cooking_config_max_iterations() {
        ...
        assert_eq!(config.max_tool_calls, 50); // default unchanged
    }
