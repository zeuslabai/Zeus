...
fn default_mutable_keys() -> Vec<(String, Vec<String>, String)> {
    vec![
        ...
        (
            "prometheus.cooking_loop_timeout".into(),
            vec!["30m".into(), "1h".into(), "2h".into(), "4h".into(), "8h".into(), "custom".into()],
            "Cooking-loop wall-clock timeout (NL: '30 hours', '1h30m', etc.)".into(),
        ),
    ]
}
...
mod tests {
    ...
    fn test_random_proposer_cycles_keys() {
        ...
        // Should cycle through the whitelist deterministically by index.
        // Period == number of mutable keys, so key[i] == key[i + N].
        let n = default_mutable_keys().len();
        assert_eq!(keys[0], keys[n]);
        assert_eq!(keys[1], keys[1 + n]);
    }
