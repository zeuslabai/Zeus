...
pub use goals::{Goal, GoalSource, GoalStack, GoalStatus, Priority};
...
impl Prometheus {
    ...
    pub async fn new(config: Config) -> Result<Self> {
        ...
        // Initialize the cron scheduler if configured via prometheus section
        let scheduler = prom_cfg.and_then(|p| {
            if p.enable_heartbeat {
                let sched_cfg = p
                    .scheduler
                    .as_ref()
        ...
        let checkpoint_store = {
            ...
            match CookingCheckpointStore::open(&db_path) {
                Ok(store) => {
                    // Cleanup old completed sessions on startup — prevents unbounded DB growth.
                    // (This function existed but was only called in tests before this fix.)
                    let store = Arc::new(store);
                    let cleanup_store = store.clone();
                    tokio::spawn(async move {
                        cleanup_store.cleanup_old_sessions(std::time::Duration::from_secs(86400)).await;
                    });
                    debug!("Cooking checkpoint store initialized (cleanup scheduled)");
                    Some(store)
                }
                Err(e) => {
                    warn!(
            ...
            && let Some(ref stack) = self.goal_stack
        {
            ...
            match stack.add(&goal) {
                Ok(id) => {
                    ...
                    // Store goal as working memory if Mnemosyne is available
                    if let Some(ref mnemosyne) = self.mnemosyne {
                        let goal_msg =
                            Message::assistant(format!("[Active Goal] {}: {}", id, message));
                        let _ = mnemosyne
                            .store_typed(
                                "goals",
                                &goal_msg,
    ...
    pub async fn start_scheduler(&mut self) -> Result<()> {
        if let Some(ref sched) = self.scheduler {
            sched.write().await
                .start(self.workspace.clone(), self.llm.clone())
                .await?;
        }
        Ok(())
    }
    ...
    pub async fn cook_with_history_cancellable(
        ...
        self.cook_with_history_interruptible(message, tools, conversation_history, cancel, None, vec![]).await
    }
    ...
    pub async fn cook_with_history_interruptible(
        ...
        let memory_context = if let Some(ref mnemosyne) = self.mnemosyne {
            ...
            match (query_ctx, proactive_ctx) {
                (Some(q), Some(p)) => Some(format!("{}\n{}", q, p)),
                (Some(q), None) => Some(q),
                (None, Some(p)) => Some(p),
                (None, None) => None,
            }
