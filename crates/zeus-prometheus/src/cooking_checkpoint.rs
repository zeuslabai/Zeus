...
impl CookingCheckpointStore {
    ...
    /// Prune a single completed session's row immediately.
    ///
    /// Called on clean cook-completion so finished sessions don't accumulate
    /// their (potentially multi-MB) `messages_json`/`records_json` blobs. This
    /// is the primary defense against the checkpoint DB ballooning — without
    /// it, completed rows linger until the age-based sweep, and a high-throughput
    /// loop accumulates GBs in between.
    pub async fn prune_completed(&self, session_id: &str) {
        let db = self.db.lock().await;
        let deleted = db
            .execute(
                "DELETE FROM cooking_checkpoints WHERE session_id = ?1 AND completed = 1",
                params![session_id],
            )
            .unwrap_or(0);
        if deleted > 0 {
            debug!(session = session_id, "Pruned completed checkpoint");
        }
    }

    /// Enforce a row-count retention cap: keep only the `max_rows` most-recently
    /// updated checkpoints, deleting the oldest beyond that. Complements the
    /// age-based sweep — guards against a burst of cooks inside the age window
    /// growing the DB unbounded. `0` disables the cap.
    pub async fn enforce_count_cap(&self, max_rows: usize) {
        if max_rows == 0 {
            return;
        }
        let db = self.db.lock().await;
        // Delete all but the N most-recent by updated_at. Interrupted (incomplete)
        // sessions are still subject to the cap — but ordering by updated_at keeps
        // the freshest, which are the ones most likely to be resumed.
        let deleted = db
            .execute(
                "DELETE FROM cooking_checkpoints WHERE session_id NOT IN (
                    SELECT session_id FROM cooking_checkpoints
        ...
        if deleted > 0 {
            info!(
                "Checkpoint count-cap: pruned {} rows (keeping {} most-recent)",
                deleted, max_rows
            );
        }
    }

    /// Reclaim on-disk space after deletes. SQLite `DELETE` frees pages for
    /// reuse but does not shrink the file — so a DB that ballooned to 2 GB stays
    /// 2 GB on disk until `VACUUM`. Run sparingly (it rewrites the whole file).
    pub async fn vacuum(&self) {
        let db = self.db.lock().await;
        if let Err(e) = db.execute_batch("VACUUM;") {
            warn!("Checkpoint VACUUM failed (non-fatal): {}", e);
        } else {
            debug!("Checkpoint DB vacuumed");
        }
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn test_prune_completed_removes_row() {
        let store = CookingCheckpointStore::in_memory().unwrap();
        store.start_session("done-1", "task", "sp").await;
        // not yet completed -> prune is a no-op
        store.prune_completed("done-1").await;
        assert!(store.load_checkpoint("done-1").await.is_some());
        // mark complete, then prune removes it entirely
        store.mark_completed("done-1").await;
        store.prune_completed("done-1").await;
        assert!(store.load_checkpoint("done-1").await.is_none());
    }

    #[tokio::test]
    async fn test_enforce_count_cap_keeps_most_recent() {
        let store = CookingCheckpointStore::in_memory().unwrap();
        for i in 0..10 {
            store
                .start_session(&format!("s-{i}"), "task", "sp")
                .await;
        ...
        store.enforce_count_cap(3).await;
        // 3 newest survive (s-7, s-8, s-9); the rest are pruned
        assert!(store.load_checkpoint("s-9").await.is_some());
        assert!(store.load_checkpoint("s-8").await.is_some());
        assert!(store.load_checkpoint("s-7").await.is_some());
        assert!(store.load_checkpoint("s-6").await.is_none());
        assert!(store.load_checkpoint("s-0").await.is_none());
    }

    #[tokio::test]
    async fn test_count_cap_zero_disables() {
        let store = CookingCheckpointStore::in_memory().unwrap();
        for i in 0..5 {
            store.start_session(&format!("z-{i}"), "task", "sp").await;
        }
        store.enforce_count_cap(0).await;
        // 0 = disabled: nothing pruned
        assert!(store.load_checkpoint("z-0").await.is_some());
        assert!(store.load_checkpoint("z-4").await.is_some());
    }

    #[tokio::test]
    async fn test_vacuum_runs_clean() {
        let store = CookingCheckpointStore::in_memory().unwrap();
        store.start_session("v-1", "task", "sp").await;
        store.mark_completed("v-1").await;
        store.prune_completed("v-1").await;
        // vacuum must not panic / error on an in-memory db
        store.vacuum().await;
        assert!(store.load_checkpoint("v-1").await.is_none());
    }
