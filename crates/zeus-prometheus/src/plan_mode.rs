...
pub enum PlanStatus {
    ...
    Interrupted,
    /// Plan exceeded its resume cap and was abandoned (terminal, like Failed:
    /// drops out of `find_incomplete`, never re-resumes). #157 bounded retries.
    Abandoned,
}
...
pub struct PlanMeta {
    ...
    /// #157 bounded retries: how many times this plan has been resumed by the
    /// heartbeat. Capped — at the cap the plan goes terminal (Abandoned) so a
    /// stuck plan that never reaches Completed can't re-resume every tick.
    #[serde(default)]
...
impl PlanMode {
    ...
    /// #157 bounded retries: increment the resume counter and persist it.
    /// Returns the new attempt count. The heartbeat calls this each time it
    /// resumes the plan; once it crosses `max_resume_attempts` the caller
    /// drives the plan terminal via [`mark_abandoned`].
    pub async fn record_resume_attempt(&mut self) -> Result<usize, String> {
        self.meta.resume_attempts += 1;
        self.write_status().await?;
        Ok(self.meta.resume_attempts)
    }

    /// #157 bounded retries: mark the plan abandoned (terminal). Drops it out
    /// of `find_incomplete` so a stuck plan stops re-resuming every tick.
    pub async fn mark_abandoned(&mut self, reason: &str) -> Result<(), String> {
        self.meta.status = PlanStatus::Abandoned;
        self.write_status().await?;
        self.write_outcome(reason).await
    }

    /// Current resume-attempt count (for callers gating on the cap).
    pub fn resume_attempts(&self) -> usize {
        self.meta.resume_attempts
    }
    ...
    fn parse_status(content: &str, slug: &str) -> Result<PlanMeta, String> {
        ...
        let steps_failed = get("steps_failed: ")
            .and_then(|s| s.parse().ok())
            .unwrap_or(0);
        let resume_attempts = get("resume_attempts: ")
            .and_then(|s| s.parse().ok())
            .unwrap_or(0);
...
mod tests {
    ...
    async fn test_find_incomplete_plans() {
        ...
        // Create a fresh Executing plan. `find_incomplete` intentionally skips
        // Executing plans younger than 300s (they're the heartbeat's own
        // just-created work, not abandoned plans needing resumption), so this
        // one must NOT be reported as incomplete.
        let mut pm1 = PlanMode::create(&workspace, "Task one", "Zeus100").await.unwrap();
        pm1.write_plan("- [ ] Step 1\n").await.unwrap();

        // Create a completed plan — never incomplete.
        let mut pm2 = PlanMode::create(&workspace, "Task two", "Zeus112").await.unwrap();
        pm2.complete("Done").await.unwrap();

        // Create an interrupted plan — always resumable, no age guard.
        ...
        // Only pm3 (Interrupted): pm1 is a fresh Executing plan (<300s, skipped),
        // pm2 is Completed.
        assert_eq!(incomplete.len(), 1);
        assert!(incomplete.iter().any(|s| s.contains("task-three")));
    }
    ...
    // ---- #157 bounded retries: plan-resume cap ----

    #[tokio::test]
    async fn test_resume_attempt_counter_persists() {
        let (_dir, workspace) = setup().await;
        let mut pm = PlanMode::create(&workspace, "Stuck task", "Zeus100").await.unwrap();
        pm.write_plan("- [ ] Step 1\n").await.unwrap();
        let slug = pm.meta().slug.clone();

        assert_eq!(pm.record_resume_attempt().await.unwrap(), 1);
        assert_eq!(pm.record_resume_attempt().await.unwrap(), 2);

        // Counter survives a reload from STATUS.md (the heartbeat re-loads each tick).
        let reloaded = PlanMode::load(&workspace, &slug).await.unwrap();
        assert_eq!(reloaded.resume_attempts(), 2);
    }

    #[tokio::test]
    async fn test_mark_abandoned_drops_out_of_find_incomplete() {
        let (_dir, workspace) = setup().await;
        let mut pm = PlanMode::create(&workspace, "Doomed task", "Zeus100").await.unwrap();
        pm.write_plan("- [ ] Step 1\n").await.unwrap();
        pm.mark_interrupted().await.unwrap();
        let slug = pm.meta().slug.clone();

        // Interrupted → resumable.
        assert!(PlanMode::find_incomplete(&workspace).await.iter().any(|s| *s == slug));

        // Abandon it (the cap action) → terminal, no longer resumable.
        pm.mark_abandoned("hit the resume cap").await.unwrap();
        assert_eq!(pm.meta().status, PlanStatus::Abandoned);
        assert!(!PlanMode::find_incomplete(&workspace).await.iter().any(|s| *s == slug));

        // And the terminal status round-trips through STATUS.md.
        let reloaded = PlanMode::load(&workspace, &slug).await.unwrap();
        assert_eq!(reloaded.meta().status, PlanStatus::Abandoned);
    }
}
