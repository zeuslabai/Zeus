...
/// Determine whether a message is *interrogative* — a question-shaped
/// conversational fragment rather than an imperative goal-plan.
///
/// This is the creation-side gate for plan persistence (#157). The intent
/// classifier can fire `ComplexTask` on a long-winded *question* (a task-verb
/// or word-count match), which then persists an ephemeral, non-resumable plan
/// dir (e.g. `2026-06-18-intent-question-answer-this-question`) that the
/// heartbeat re-resumes hourly until the 25-cap abandons it.
///
/// A message is interrogative iff it **leads with a question word** (or is
/// `?`-terminated) **AND lacks a leading task-imperative**. The imperative
/// guard is what prevents over-blocking: a legit goal phrased as
/// "implement X and then test Y" is imperative, not interrogative, so it
/// survives the gate. Only clearly question-shaped fragments are filtered.
///
/// Pure function — no `self`, fully unit-testable. Returns `true` for
/// "filter this (do not persist a plan)".
pub fn is_interrogative(message: &str) -> bool {
    let lower = message.trim().to_lowercase();
    if lower.is_empty() {
        return false;
    }

    // A leading task-imperative marks a genuine goal-plan — never filter it,
    // even if it later contains a "?" (e.g. "implement X; can we test it?").
    const IMPERATIVE_LEADS: &[&str] = &[
        "build ", "implement ", "create ", "add ", "write ", "refactor ",
        "fix ", "update ", "make ", "set up ", "setup ", "deploy ", "install ",
        "configure ", "generate ", "design ", "develop ", "port ", "migrate ",
        "remove ", "delete ", "rename ", "move ", "run ", "execute ",
    ];
    if IMPERATIVE_LEADS.iter().any(|kw| lower.starts_with(kw)) {
        return false;
    }

    // Question-word leads (mirrors is_simple_query) — these are conversational
    // question shapes, not goals.
    const QUESTION_LEADS: &[&str] = &[
        ...
        "should we ", "do you ", "does ", "did ", "is ", "are ", "was ",
        "were ", "tell me", "explain ", "describe ", "define ",
    ];
    if QUESTION_LEADS.iter().any(|kw| lower.starts_with(kw)) {
        return true;
    }

    // `?`-terminated with no imperative lead → conversational question.
    lower.ends_with('?')
}
...
mod tests {
    ...
    // ── #157 creation-side gate: is_interrogative ───────────────────────────

    #[test]
    fn test_is_interrogative_filters_question_fragment() {
        // (i) The offending shape — an interrogative single-clause fragment that
        // classified ComplexTask and persisted the ephemeral
        // `2026-06-18-intent-question-answer-this-question` plan dir. MUST filter.
        assert!(
            is_interrogative("can you answer this question for me about the system"),
            "interrogative 'can you ...' fragment must be filtered"
        );
        assert!(
            is_interrogative("what should the gateway do when an intent is ambiguous?"),
            "question-word lead must be filtered"
        );
        assert!(
            is_interrogative("is the plan resume loop fixed yet?"),
            "'is ...?' fragment must be filtered"
        );
        assert!(
            is_interrogative("should we persist this conversation as a plan?"),
            "'should we ...?' fragment must be filtered"
        );
        // Bare ?-terminated with no imperative lead.
        assert!(
            is_interrogative("the heartbeat keeps resuming stale plans, right?"),
            "?-terminated fragment must be filtered"
        );
    }

    #[test]
    fn test_is_interrogative_survives_imperative_goal() {
        // (ii) The over-block guard — a legit imperative multi-clause goal MUST
        // survive (return false → plan gets persisted).
        assert!(
            !is_interrogative(
                "build the channels-persist fix and then add unit tests for each struct"
            ),
            "imperative multi-clause goal must survive the gate"
        );
        assert!(
            !is_interrogative("implement the creation-side plan gate in gateway.rs"),
            "imperative 'implement ...' goal must survive"
        );
        assert!(
            !is_interrogative("refactor the intent classifier and migrate the tests"),
            "imperative 'refactor ...' goal must survive"
        );
        // Imperative lead wins even if a trailing clause is question-shaped.
        assert!(
            !is_interrogative("fix the resume loop; can we ship it today?"),
            "leading imperative must override a trailing question clause"
        );
        // Empty / whitespace → not interrogative (no plan either way, but must not panic).
        assert!(!is_interrogative("   "), "empty input must not be interrogative");
    }
}
