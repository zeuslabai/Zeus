...
pub mod validator;
pub use cook_state::{ActiveCookType, CookGuard, CookState};
...
fn default_gateway_timeout_secs() -> u64 {
    1800 // 30 minutes — long tasks (repo analysis, fleet coordination) need time
}
...
pub struct DiscordChannelConfig {
    ...
    /// "off" (default): skip all bot messages. "mentions": allow if @mentioned.
...
impl Config {
    ...
    pub fn load() -> Result<Self> {
        ...
        if config_path.exists() {
            ...
            // Auto-backup config before attempting parse — protects against
            // silent corruption if the new binary has incompatible schema changes.
    ...
    pub fn save(&self) -> Result<()> {
        ...
        // Guard: refuse to save if workspace or sessions point to temp directories.
        // This prevents Config::default() or onboarding temp state from overwriting
        // a real config file.
        let ws = self.workspace.to_string_lossy();
        let ss = self.sessions.to_string_lossy();
        if ws.contains("/var/folders/") || ws.starts_with("/tmp/") || ws.starts_with("/tmp.") {
            return Err(Error::Config(format!(
                "Refusing to save config: workspace path '{}' is a temp directory",
                ws
            )));
        }
        if ss.contains("/var/folders/") || ss.starts_with("/tmp/") || ss.starts_with("/tmp.") {
            return Err(Error::Config(format!(
                "Refusing to save config: sessions path '{}' is a temp directory",
                ss
            )));
        }
        ...
        let new_content = toml::to_string_pretty(self).map_err(|e| Error::Config(e.to_string()))?;
    ...
    pub fn parse_model(&self) -> (Provider, String) {
        if let Some((provider, model)) = self.model.split_once('/') {
            let p = match provider.to_lowercase().as_str() {
                ...
                _ => {
                    eprintln!(
                        "Warning: Unknown provider '{}', defaulting to Anthropic",
                        provider
                    );
                    Provider::Anthropic
                }
    ...
    pub fn builtins() -> Vec<Self> {
        vec![
            ...
            Self {
                ...
                config_overrides: None,
                default_skills: vec!["code-review".into(), "research".into()],
            ...
            Self {
                ...
                config_overrides: None,
                default_skills: vec!["devops".into()],
                default_tools: vec!["read_file".into(), "write_file".into(), "shell".into(), "web_fetch".into()],
            },
            Self {
                id: "research".into(),
                ...
                config_overrides: None,
                default_skills: vec!["research".into(), "summarize".into()],
            ...
            Self {
                ...
                config_overrides: None,
                default_skills: vec!["writing".into(), "research".into()],
