...
pub struct Marketplace {
    ...
    /// #81a money-path unification: routes settlement onto the canonical
    /// `zeus-economy` ledger. **Disabled by default** — when off, the legacy
    /// in-memory `ledger` above stays authoritative and nothing changes.
    pub settlement: settlement::SettlementCoordinator,
}
...
impl Marketplace {
    pub fn new() -> Self {
        Self {
            ...
            settlement: settlement::SettlementCoordinator::disabled(),
        }
    }

    /// Builder: attach a [`settlement::SettlementCoordinator`] to route the
    /// money-path onto the canonical economy ledger. Off by default; the
    /// coordinator itself is also off-by-default until explicitly enabled.
    pub fn with_settlement(mut self, coordinator: settlement::SettlementCoordinator) -> Self {
        self.settlement = coordinator;
        self
    }
    ...
    pub async fn execute_trade(&self, trade_id: &str) -> Result<Trade> {
        ...
        // #81a: when the settlement coordinator is enabled, route the money-path
        // atomically through the canonical zeus-economy ledger (single sink).
        // Disabled by default ⇒ Skipped, and the legacy in-memory transfer below
        // remains authoritative with no behavioral change.
        let outcome = self
            .settlement
            .settle(
                &trade.buyer_id,
                &trade.seller_id,
                price,
                0, // fee scaffolding: zero protocol fee until fee policy lands
                format!("trade:{}", trade.id),
            )
            .map_err(|e| MarketplaceError::SettlementFailed(e.to_string()))?;

        if !outcome.is_settled() {
            // Legacy in-memory path (default). Untouched money-path.
            self.ledger
                .transfer(
                    &trade.buyer_id,
                    &trade.seller_id,
