...
// ============================================================================
// memory_store (#164) — LLM-callable typed write to Mnemosyne
// ============================================================================

async fn registry_with_memory() -> (ToolRegistry, tempfile::TempDir, std::sync::Arc<zeus_mnemosyne::Mnemosyne>) {
    let dir = tempfile::tempdir().unwrap();
    let config = zeus_mnemosyne::MnemosyneConfig {
        db_path: dir.path().join("memstore.db"),
        enable_fts: false,
        enable_embeddings: false,
        ..Default::default()
    };
    let mn = std::sync::Arc::new(
        zeus_mnemosyne::Mnemosyne::new(config)
            .await
            .expect("Mnemosyne::new should succeed"),
    );
    let mut registry = ToolRegistry::new();
    registry.set_memory(mn.clone(), "test-session");
    (registry, dir, mn)
}

#[test]
fn memory_store_schema_present() {
    let registry = ToolRegistry::new();
    let schemas = registry.schemas();
    let ms = schemas
        .iter()
        .find(|s| s.name == "memory_store")
        .expect("memory_store must be in core schemas");
    // content is the one required param
    assert!(!ms.description.is_empty());
    let props = &ms.parameters["properties"];
    assert!(props.get("content").is_some(), "content param present");
    assert!(props.get("memory_type").is_some(), "memory_type param present");
    assert!(props.get("importance").is_some(), "importance param present");
}

#[tokio::test]
async fn memory_store_writes_row_and_returns_id() {
    let (registry, _dir, mn) = registry_with_memory().await;
    let result = registry
        .execute(
            "memory_store",
            serde_json::json!({"content": "Two-gate green means local == remote == SHA."}),
        )
        .await
        .expect("memory_store should succeed");
    assert!(result.contains("Banked to memory"), "got: {result}");
    // Default type is Semantic (durable banking, not episodic).
    assert!(result.contains("semantic"), "default type semantic, got: {result}");
    // The row actually landed.
    let stats = mn.stats().await.expect("stats");
    assert_eq!(stats.message_count, 1);
}

#[tokio::test]
async fn memory_store_unknown_type_falls_back_to_episodic() {
    let (registry, _dir, _mn) = registry_with_memory().await;
    let result = registry
        .execute(
            "memory_store",
            serde_json::json!({"content": "x", "memory_type": "not-a-real-type"}),
        )
        .await
        .expect("memory_store should succeed");
    // parse_label maps unknown labels to episodic, never errors.
    assert!(result.contains("episodic"), "got: {result}");
}

#[tokio::test]
async fn memory_store_no_handle_returns_tool_err_not_panic() {
    // ToolRegistry without set_memory → no Mnemosyne handle.
    let registry = ToolRegistry::new();
    let err = registry
        .execute("memory_store", serde_json::json!({"content": "x"}))
        .await
        .expect_err("no memory handle must be an Err, not a panic");
    assert!(
        err.to_string().contains("memory subsystem unavailable"),
        "got: {err}"
    );
}

#[tokio::test]
async fn memory_store_empty_content_rejected() {
    let (registry, _dir, _mn) = registry_with_memory().await;
    let err = registry
        .execute("memory_store", serde_json::json!({"content": "   "}))
        .await
