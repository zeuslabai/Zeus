...
use zeus_core::{
    AgentToolPolicy, Config, Error, Message, PersonaRegistry, Result, ToolCall, ToolResult,
    ToolSchema,
};
...
/// #232/#236: Content-based gate for the phantom-action guard. Returns true when
/// `content` either narrates a first-person tool action (cue + tool verb) or
/// contains a raw tool-call leak (tool-call-as-text). Replaces the old
/// provider allow-list so plain greetings/answers/acks keep a natural voice.
...
pub struct Agent {
    ...
    /// Capabilities manifest (live tools + subsystems + skill count) injected
    /// into the system prompt above [Skills] so the model stops denying
    /// capabilities it actually has. Built post-wiring in with_subsystems.
    capabilities_summary: Option<String>,
    ...
    /// GAP #4b: Persona registry, loaded **once** at agent init (not per-spawn).
    ///
    /// When `config.persona` names a persona that carries a `model`/`effort`/
    /// `tools` frontmatter, subagent spawns resolve that name → `RunProfile`
    /// here and apply it to the `SubagentConfig`. `None` when no personalities
    /// dir was found; a persona with no `model` falls through to the parent's
    /// live-selected `config.model` — never a hardcoded pin.
    persona_registry: Option<PersonaRegistry>,
}
...
impl Agent {
    ...
    pub fn new(
        ...
        Self {
            ...
            persona_registry: Self::load_persona_registry(),
        }
    }

    /// GAP #4b: Load the persona registry **once** at agent init.
    ///
    /// Searches the same `personalities/` locations onboarding uses (cwd,
    /// `~/Zeus/personalities`, `~/zeus/personalities`) and returns the first
    /// that loads. `None` when none is found — the spawn path then simply
    /// keeps the parent's live-selected model.
    fn load_persona_registry() -> Option<PersonaRegistry> {
        let candidates = [
        ...
        for dir in candidates.into_iter().flatten() {
            if dir.is_dir() {
                if let Ok(reg) = PersonaRegistry::load_from_dir(&dir) {
                    return Some(reg);
                }
            }
        }
    ...
    /// GAP #4b: Resolve the spawn `(model, effort, tools)` from the selected
    /// persona, with the parent-model fall-through rule.
    ///
    /// Name-resolve the user's explicit `[agent].persona` pick against the
    /// cached registry → `RunProfile`. The token-router (`registry.route(task)`)
    /// is intentionally **not** used here — that's a future/opt-in task-driven
    /// mode; a silent mis-route swapping model/tools under the user is a risk we
    /// don't take by default.
    ///
    /// - `model`: the persona's `model` when set, else the parent's
    ///   live-selected `self.config.model`. **Never a hardcoded pin.**
    /// - `effort`: the persona's `effort` (or `None`).
    /// - `tools`: the persona's `tools` allow-list (or empty).
    fn resolve_spawn_profile(&self) -> (Option<String>, Option<String>, Vec<String>) {
        let profile = self
            .config
            .persona
            .as_deref()
            .and_then(|name| self.persona_registry.as_ref().map(|r| (r, name)))
            .and_then(|(reg, name)| reg.by_name(name))
            .map(|p| p.run_profile());
        ...
        let effort = profile.as_ref().and_then(|p| p.effort.clone());
        let tools = profile.map(|p| p.tools).unwrap_or_default();

        (Some(model), effort, tools)
    }
    ...
    pub async fn with_subsystems(
        ...
        let mnemosyne = if let Some(ref mc) = config.mnemosyne {
            ...
            // Clone the config up-front: it's moved into the first `new()` below,
            // but the corruption-recovery path needs to retry `new()` once.
            let mnemosyne_config_retry = mnemosyne_config.clone();
            match Mnemosyne::new(mnemosyne_config).await {
                ...
                Err(e) => {
                    // Init failed. This used to silently degrade to `None` at warn
                    // level — a seat could lose memory for hours without any loud
                    // signal (#172). Escalate: run the integrity oracle against the
                    // same path to distinguish durable corruption from a transient
                    // lock/WAL window, log at error level with the db_path + the
                    // operator action, and attempt a single retry for the transient
                    // case.
                    error!(
                        "Mnemosyne init FAILED (db: {}): {} — memory disabled this session",
                        mc.db_path.display(),
                        e
                    );
                    match Mnemosyne::check_integrity(&mc.db_path) {
                        Ok(false) => {
                            error!(
                                "Mnemosyne DB integrity_check FAILED — corruption detected at {}. \
                                 Memory is disabled for this session. OPERATOR ACTION: recover/replace \
                                 the DB and BOUNCE this seat to re-wire the handle.",
                                mc.db_path.display()
                            );
                            None
                        }
                        _ => {
                            // integrity_check passed (or couldn't run) — the failure
                            // was likely a transient lock/WAL/mid-checkpoint window
                            // that releases between calls. One-shot retry. This only
                            // clears SHORT transients; a sustained corruption window
                            // still fails here and the seat-bounce remains the fix.
                            warn!(
                                "Mnemosyne DB integrity OK (or uncheckable) — retrying init once \
                                 (transient lock/WAL suspected)"
                            );
                            match Mnemosyne::new(mnemosyne_config_retry).await {
                                Ok(m) => {
                                    info!(
                                        "Mnemosyne initialized on retry (db: {})",
                                        mc.db_path.display()
                                    );
                                    Some(Arc::new(m))
                                }
                                Err(e2) => {
                                    error!(
                                        "Mnemosyne retry init FAILED (db: {}): {} — memory \
                                         disabled this session; bounce the seat",
                                        mc.db_path.display(),
                                        e2
                                    );
                                    None
        ...
        // Force-activate the selected persona's `default_skills` for this run.
        // Resolve `config.persona` → PersonaRegistry → its `default_skills`,
        // then mark those always-active in the SkillManager. This mutates, so it
        // must run before the manager is shared/wrapped. No persona, no match, or
        // skill names that aren't loaded → no-op (never force an absent skill).
        if let Some(persona_name) = config.persona.as_deref() {
            let personalities_dir = cfg_dir.join("personalities");
            if let Ok(registry) =
                zeus_core::PersonaRegistry::load_from_dir(&personalities_dir)
            {
                if let Some(persona) = registry.by_name(persona_name) {
                    if !persona.default_skills.is_empty() {
                        skill_manager.enable_skills(&persona.default_skills);
                        info!(
                            "Persona '{}' default_skills force-activated: {:?}",
                            persona_name, persona.default_skills
                        );
                    }
                }
            }
        ...
        // Wire Mnemosyne (+ session id for grouping) so the `memory_store` tool
        // can bank intentionally. Only when a handle exists; no handle → the
        // tool returns a graceful tool_err, never a panic.
        if let Some(ref mn) = mnemosyne {
            tools.set_memory(mn.clone(), session.id.clone());
        }

        // Build the [Capabilities] manifest AFTER all subsystem wiring above
        // (browser/channels/memory) so tools.schemas() reflects the fully-wired
        // registry — building it earlier (e.g. at the skills_summary site) would
        // under-report browser + memory tools. Skill count only here; the
        // [Skills] block already carries the L1 taglines (GAP#1 — don't dup).
        let capabilities_summary = {
            let (mut manifest, tool_count, subsystem_count) = tools.capabilities_manifest();
            let skill_count = skill_manager.list().len();
            if skill_count > 0 {
                manifest.push_str(&format!(
                    "Skills: {} loaded — see [Skills] below for details\n",
                    skill_count
                ));
            }
            info!(
                "Capabilities: {} tools, {} subsystems, {} skills",
                tool_count, subsystem_count, skill_count
            );
            Some(manifest)
        };
    ...
    /// #168 Phase 5 — does this message ask the agent to recall its own durable
    /// work state (active goals, tasks, progress)? Such messages must NOT take the
    /// subsystem-skipping fast path, or they'll be answered blind. Conservative
    /// substring match on the original input — false negatives are fine (worst case:
    /// a recall question takes the fast path, the pre-existing behavior); false
    /// positives just route a simple message through the full path (correct, slower).
    fn is_recall_class(input: &str) -> bool {
        ...
        RECALL_CUES.iter().any(|cue| s.contains(cue))
    }
    ...
    pub async fn run_fast(&mut self, user_input: &str) -> Result<String> {
        ...
        // #168 Phase 5 — defense: run_fast skips ALL subsystems (no Nous, no
        // Mnemosyne, no work-state). That's wrong for recall-class messages
        // ("what are you working on?", "status", "did you finish X?"): answering
        // those from a subsystem-blind fast path is exactly the status-blindness
        // #168 closes on the cook path. run_fast has no live caller today, but if
        // one is wired back up, route recall-class intents to the full `run` path
        // so the [Work State] / Mnemosyne recall still fires. Guard, not gospel.
        if Self::is_recall_class(user_input) {
            warn!("run_fast: recall-class intent detected — routing to full path so work-state/memory recall fires (#168 Phase 5)");
            return self.run(user_input).await;
        }
        ...
        // Wait for full response (5 min — matches main loop; per-request timeout
        // in LlmClient handles fail-fast for flaky endpoints like Zai/bigmodel.cn)
    ...
    async fn run_turn(
        ...
        // Inject fleet-wide shared prompt sections (voice + autonomy contract).
        // Modeled on OpenClaw's prompt overlay — defined once in
        // `shared_prompt.rs` and inherited by every persona, so the *shape* of
        // how titans talk and act lives in one place, not 30 files.
        system_prompt.push_str(&format!(
            "\n\n{}",
            crate::shared_prompt::shared_prompt_sections()
        ));
        ...
        // Inject the [Capabilities] manifest above [Skills] so the model has a
        // compact, always-present reminder of the tools/subsystems it can call.
        // The manifest text already begins with the "[Capabilities]" header.
        if let Some(ref caps) = self.capabilities_summary {
            system_prompt.push_str(&format!("\n\n{}", caps));
        }
        ...
        loop {
            ...
            // each iteration so live edits to workspace files take effect mid-session.
            {
                ...
                if !agents_fresh.is_empty() || !soul_fresh.is_empty() || !memory_fresh.is_empty() {
                    ...
                    let tail_markers = ["# User Context", "# Your Human", "# Identity", "# Local Tools", "# Proactive Tasks", "# Capabilities", "# Memory", "[Cognitive Context]", "[Learned Instincts]", "[Constitutional Laws]", "[Active Goals]", "[Capabilities]", "[Skills]", "[Relevant"];
            ...
            let (mut rx, handle) = match stream_result
            {
                ...
                Err(e) => {
                    ...
                    let agent_name = self.config.name.clone().or_else(|| self.config.network.as_ref().and_then(|n| n.agent_name.clone())).unwrap_or_else(|| "<unnamed agent>".to_string());
            ...
            match response.stop_reason {
                StopReason::EndTurn => {
                    ...
                    {
                        // #232: Gate on CONTENT, not provider. Fire only when the
                        // reply actually narrates a tool action or leaks a raw
                        // tool-call as text (#236). Plain replies → natural voice.
                        let is_phantom = response.tool_calls.is_empty()
                            && !tool_schemas.is_empty()
                            && response.content.len() > 50
                            && response.stop_reason != zeus_llm::StopReason::Error
                            && content_claims_tool_action(&response.content);
                        if is_phantom {
                            if !phantom_action_reprompt_used {
                                ...
                                self.session.add(Message::assistant(&response.content)).await?;
                                ...
                                continue; // re-enter the LLM call loop
                            } else {
                                warn!(
    ...
    async fn execute_tools_sequential_owned(&mut self, calls: &[ToolCall]) -> Vec<ToolResult> {
        ...
        for call in calls {
            ...
            } else if call.name == "send_rich" {
                self.execute_send_rich(call).await
    ...
    async fn execute_spawn(&mut self, call: &ToolCall) -> ToolResult {
        ...
        // GAP #4b: Name-resolve the selected persona → (model, effort, tools),
        // deterministically honouring the user's explicit `[agent].persona` pick.
        let (model, effort, tools) = self.resolve_spawn_profile();
    ...
    async fn execute_message(&self, call: &ToolCall) -> ToolResult {
        ...
        let result = match channel_spec {
            "telegram" | "discord" | "slack" | "email" | "imessage" | "irc" | "matrix" | "whatsapp" | "signal" | "mattermost" | "mqtt" | "x_twitter" => {
                if let Some(ref channels) = self.channels {
                    // X has no chat/recipient concept — target is optional
                    // (used as reply_to tweet ID when provided). All other
                    // platform channels require an explicit target.
                    if target.is_none() && channel_spec != "x_twitter" {
                        return ToolResult {
                            call_id: call.id.clone(),
                            success: false,
                            output: format!(
                    ...
                    let source = match channel_spec {
                        ...
                        // X: no chat_id; an optional target is a tweet ID to
                        // reply to, carried in reply_to_message_id.
                        "x_twitter" => {
                            let mut s = ChannelSource::new(channel_spec, "agent");
                            if !target.is_empty() {
                                s.reply_to_message_id = Some(target.to_string());
                            }
                            s
                        }
    ...
    /// #88 producer: emit a structured rich response via ChannelManager::send_rich.
    /// Delegates to the standalone `send_rich_to_channel` so it's reusable from
    /// the cooking loop / tool executor, mirroring `execute_send_file`.
    async fn execute_send_rich(&self, call: &ToolCall) -> ToolResult {
        let channels = self.channel_manager();
        if let Some(channels) = channels {
            let mut result =
                crate::tools::send_rich_to_channel(&call.arguments, &channels).await;
            result.call_id = call.id.clone();
            result
        } else {
            ToolResult {
                call_id: call.id.clone(),
                success: false,
                output: "No channel manager configured — cannot send rich responses".to_string(),
            }
        }
    }
...
mod tests {
    ...
    // ===== #232: Phantom-action content gate =====

    // (a) greeting / normal answer → false
    #[test]
    fn test_content_gate_greeting_false() {
        assert!(!content_claims_tool_action(
            "Hey! I'm running Claude Opus 4 here, happy to help with whatever you need."
        ));
        assert!(!content_claims_tool_action(
    ...
    // (b) "I'll read the file" with no tool_call → true
    #[test]
    fn test_content_gate_narrated_action_true() {
        assert!(content_claims_tool_action(
            "Sure — I'll read the file and report back."
        ));
        assert!(content_claims_tool_action("Let me run the tests now."));
        assert!(content_claims_tool_action("I'm going to create the branch."));
    }

    // (c) raw <function...> leak → true
    #[test]
    fn test_content_gate_tool_leak_true() {
        assert!(content_claims_tool_action(
            "Let me check <function=read_file>{\"path\":\"/etc/hosts\"}</function>"
        ));
        assert!(content_claims_tool_action(
    ...
    // cue without a tool verb → false (no false-positive on idle chatter)
    #[test]
    fn test_content_gate_cue_without_verb_false() {
        assert!(!content_claims_tool_action("I'll think about it for a bit."));
        assert!(!content_claims_tool_action("Let me know if that helps!"));
    }
    ...
    // ── GAP #4b: persona → spawn-config wiring ──────────────────────────

    use zeus_core::{Persona, PersonaRegistry};

    fn persona_with(name: &str, model: Option<&str>, effort: Option<&str>, tools: &[&str]) -> Persona {
        Persona {
            name: name.to_string(),
    ...
    /// Persona with a `model` → that model is applied to the spawn config.
    #[test]
    fn spawn_profile_applies_persona_model() {
        let mut config = Config::default();
        config.model = "parent/live-model".to_string();
        config.persona = Some("The Backend Dev".to_string());
        let mut agent = test_agent_with_config(config);
        agent.persona_registry = Some(PersonaRegistry::new(vec![persona_with(
            "The Backend Dev",
        ...
        let (model, effort, tools) = agent.resolve_spawn_profile();
        assert_eq!(model.as_deref(), Some("anthropic/claude-opus-4"));
        assert_eq!(effort.as_deref(), Some("high"));
        assert_eq!(tools, vec!["read_file".to_string(), "shell".to_string()]);
    }

    /// Persona with NO `model` → falls through to the parent's live model.
    /// Never pins a hardcoded model.
    #[test]
    fn spawn_profile_falls_through_when_persona_has_no_model() {
        let mut config = Config::default();
        config.model = "parent/live-model".to_string();
        config.persona = Some("The Writer".to_string());
        let mut agent = test_agent_with_config(config);
        agent.persona_registry = Some(PersonaRegistry::new(vec![persona_with(
            "The Writer",
            None,
        ...
        let (model, effort, tools) = agent.resolve_spawn_profile();
        // model falls through to parent — NOT pinned
        assert_eq!(model.as_deref(), Some("parent/live-model"));
        assert_eq!(effort.as_deref(), Some("medium"));
        assert!(tools.is_empty());
    }

    /// No persona selected → parent model, no effort/tools.
    #[test]
    fn spawn_profile_unset_persona_uses_parent_model() {
        let mut config = Config::default();
        config.model = "parent/live-model".to_string();
        config.persona = None;
    ...
    /// Persona name set but not found in registry → parent model fall-through.
    #[test]
    fn spawn_profile_unknown_persona_uses_parent_model() {
        let mut config = Config::default();
        config.model = "parent/live-model".to_string();
        config.persona = Some("Nonexistent".to_string());
        let mut agent = test_agent_with_config(config);
        agent.persona_registry =
            Some(PersonaRegistry::new(vec![persona_with("The Writer", None, None, &[])]));
    ...
    /// No registry loaded at all → parent model fall-through, no panic.
    #[test]
    fn spawn_profile_no_registry_uses_parent_model() {
        let mut config = Config::default();
        config.model = "parent/live-model".to_string();
        config.persona = Some("The Backend Dev".to_string());
        let mut agent = test_agent_with_config(config);
        agent.persona_registry = None;

        let (model, _effort, _tools) = agent.resolve_spawn_profile();
        assert_eq!(model.as_deref(), Some("parent/live-model"));
    }
    ...
    async fn test_execute_tools_constitution_block() {
        ...
        // Tool doesn't exist, so it must fail at execution (not panic, not succeed).
        assert!(
            !results[0].success,
            "nonexistent tool must produce an unsuccessful result"
        );
    }
