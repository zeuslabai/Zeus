...
pub struct CookConfig {
    ...
    /// Wall-clock budget (seconds). 0 = disabled.
...
impl Default for CookConfig {
    fn default() -> Self {
        Self {
            ...
            wall_budget_seconds: 1800, // 30 min default
...
pub async fn cook_until_done(
    ...
    let outcome = loop {
        ...
        // 2b. Budget check (wall clock)
        if config.wall_budget_seconds > 0
            && started.elapsed() >= Duration::from_secs(config.wall_budget_seconds)
        {
