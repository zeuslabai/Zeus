//! SQLite schema migration helper for zeus-mnemosyne.
//!
    ...
    for (i, sql) in migrations[current..].iter().enumerate() {
        ...
        match conn.execute_batch(sql) {
            ...
            Err(e) if e.to_string().contains("duplicate column name") => {
                tracing::debug!("sqlite migration v{} already applied (skipped)", next_version);
            }
            Err(e) => {
        ...
        conn.pragma_update(None, "user_version", next_version)
            .map_err(|e| Error::Database(format!("Failed to set user_version to {next_version}: {e}")))?;
    }
    Ok(())
}
