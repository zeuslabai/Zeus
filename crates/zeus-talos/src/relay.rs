...
async fn download_telegram_file(
    ...
    // Shared resolver: <workspace>/channels/telegram/received/<date>/<safe_name>
    let path = zeus_core::received_file_path("telegram", &filename)
...
async fn download_discord_file(
    ...
    // Shared resolver: <workspace>/channels/discord/received/<date>/<safe_name>
    let path = zeus_core::received_file_path("discord", filename)
        .map_err(|e| format!("resolve received path: {}", e))?;
    if let Some(parent) = path.parent() {
        let _ = std::fs::create_dir_all(parent);
    }
