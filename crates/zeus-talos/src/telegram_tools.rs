...
// ---------------------------------------------------------------------------
// 9. TelegramDeleteMessageTool
// ---------------------------------------------------------------------------

/// Delete a message from a Telegram chat
pub struct TelegramDeleteMessageTool;

#[async_trait]
impl TalosTool for TelegramDeleteMessageTool {
    fn name(&self) -> &'static str {
    ...
    fn schema(&self) -> ToolSchema {
        ToolSchema::new(self.name(), self.description())
            .with_param("chat_id", "string", "Chat ID containing the message", true)
    ...
    async fn execute(&self, args: Value) -> Result<String> {
        ...
        let message_id = args
            .get("message_id")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Error::Tool("Missing 'message_id' parameter".to_string()))?;
        ...
        telegram_api(&token, "deleteMessage", &params).await?;

        Ok(format!("Message {} deleted from chat {}", message_id, chat_id))
    }
}
