...
///
/// Honors the `ZEUS_SCHEDULER_DB` environment variable as a test/isolation
/// seam; when unset (the production default) it resolves to
/// `~/.zeus/scheduler.db`. This lets the full-loop e2e test point the real
/// `ScheduleCreateTool.execute()` writer at a throwaway DB instead of
/// clobbering the operator's live scheduler database.
fn scheduler_db_path() -> std::path::PathBuf {
    if let Ok(override_path) = std::env::var("ZEUS_SCHEDULER_DB") {
...
fn ensure_db() -> Result<rusqlite::Connection> {
    ...
    // One-shot support (#174 P1). Idempotent: ignore "duplicate column" if the
    // prometheus-side v2 migration already added it on a shared DB.
    let _ = conn.execute(
        "ALTER TABLE scheduled_tasks ADD COLUMN run_once INTEGER NOT NULL DEFAULT 0;",
        [],
    );
    // Wake-modes (#174 P2). Idempotent: ignore "duplicate column" if the
    // prometheus-side v3 migration already added it on a shared DB.
    let _ = conn.execute(
        "ALTER TABLE scheduled_tasks ADD COLUMN wake_mode TEXT NOT NULL DEFAULT 'now';",
        [],
    );
    // Delivery-modes (#174 P3). Idempotent: ignore "duplicate column" if the
    // prometheus-side v4 migration already added it on a shared DB.
    let _ = conn.execute(
...
impl TalosTool for ScheduleCreateTool {
    ...
    fn description(&self) -> &'static str {
        "Create a new scheduled task with a cron expression or human-readable schedule. \
         Use task_type='prompt' to schedule an autonomous agent action — at the scheduled \
         time the agent wakes and runs your payload as an LLM prompt with full tool access \
         (e.g. schedule 'post the daily standup to #devs' to fire every morning)."
    }
    ...
    fn schema(&self) -> ToolSchema {
        ToolSchema::new("schedule_create", "Create a new scheduled task")
            ...
            .with_param(
                "task_type",
                "string",
                "Task type: 'shell' (run a shell command), 'prompt' (schedule an autonomous \
                 agent action — the agent wakes at the scheduled time and executes the payload \
                 as an LLM prompt with full tool access), or 'note' (append to workspace notes)",
                true,
            )
            .with_param(
                "payload",
                "string",
                "Per task_type: for 'shell' the command to run; for 'prompt' the instruction \
                 the agent will execute (e.g. 'post the daily standup to #devs'); for 'note' \
                 the note content",
                true,
            )
            .with_param(
                "run_at",
                "string",
                "Optional one-shot fire time as RFC3339 timestamp (e.g. '2026-06-09T18:00:00Z'). \
                 When set, the task fires once at/after this instant instead of on a recurring \
                 schedule, then self-deletes. Overrides 'schedule'.",
                false,
            )
            .with_param(
                "run_once",
                "boolean",
                "If true, the task fires exactly once then self-deletes (no re-fire). \
                 Implied automatically when 'run_at' is set. Default: false (recurring).",
                false,
            )
            .with_param(
                "wake_mode",
                ...
                "When a due task fires relative to the scheduler tick: 'now' (default) \
                 fires immediately once due; 'next_heartbeat' holds a due task until the \
                 next heartbeat boundary so delivery aligns with the heartbeat cadence.",
                false,
            )
            .with_param(
                "delivery_mode",
                "string",
                "Where the fired task's result is delivered: 'channel' (default) injects \
                 into the agent's live context; 'heartbeat_note' surfaces as a passive \
                 note; 'silent_ledger' records to history only without notifying the agent.",
                false,
            )
    }
    ...
    async fn execute(&self, args: Value) -> Result<String> {
        ...
        // One-shot params. `run_at` (RFC3339) makes 'schedule' optional.
        let run_at_str = args.get("run_at").and_then(|v| v.as_str());
        let run_at = match run_at_str {
            Some(s) => Some(
                chrono::DateTime::parse_from_rfc3339(s)
                    .map_err(|e| {
                        Error::Tool(format!("Invalid 'run_at' RFC3339 timestamp '{}': {}", s, e))
                    })?
                    .with_timezone(&chrono::Utc),
            ),
            None => None,
        };
        // A run_at task is implicitly run-once; otherwise honor the explicit flag.
        let run_once =
            run_at.is_some() || args.get("run_once").and_then(|v| v.as_bool()).unwrap_or(false);

        // Wake-mode (#174 P2): 'now' (default) | 'next_heartbeat'. Unknown → 'now'.
        let wake_mode = match args.get("wake_mode").and_then(|v| v.as_str()) {
            Some("next_heartbeat") => "next_heartbeat",
            _ => "now",
        };

        let delivery_mode = match args.get("delivery_mode").and_then(|v| v.as_str()) {
            Some("heartbeat_note") => "heartbeat_note",
            Some("silent_ledger") => "silent_ledger",
            _ => "channel",
        };

        let schedule = match args.get("schedule").and_then(|v| v.as_str()) {
            Some(s) => s,
            None if run_at.is_some() => "", // one-shot: no recurring schedule needed
            None => return Err(Error::Tool("Missing 'schedule' parameter".to_string())),
        };
        let task_type = args
            .get("task_type")
            .and_then(|v| v.as_str())
        ...
        // One-shot: next_run is the run_at instant directly, no cron parse.
        let next_run = if let Some(at) = run_at {
            at.timestamp()
        } else {
        // Compute next_run from the cron expression
        let normalized_7field = normalize_cron_for_parse(&cron_expr);
        match cron::Schedule::from_str(&normalized_7field) {
            Ok(sched) => sched
                .upcoming(chrono::Utc)
                .next()
                .map(|dt| dt.timestamp())
        ...
        let conn = ensure_db()?;
        conn.execute(
            "INSERT INTO scheduled_tasks (id, name, schedule, schedule_type, task_payload, enabled, next_run, created_at, updated_at, run_once, wake_mode, delivery_mode) VALUES (?1, ?2, ?3, ?4, ?5, 1, ?6, ?7, ?7, ?8, ?9, ?10)",
            rusqlite::params![id, name, cron_expr, "cron", task_payload.to_string(), next_run, now, run_once as i32, wake_mode, delivery_mode],
        )
