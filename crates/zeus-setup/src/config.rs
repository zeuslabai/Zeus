...
/// 2. `~/.zeus` (default)
///
/// This logic MUST match `zeus_paths::zeus_home()` in the root binary and
...
# allow_bots = "mentions"    # bot-to-bot policy: "off" | "mentions" (default) | "on".
#                            #   "mentions" = only react to bot msgs that @mention you
#                            #   (prevents fleet agents triggering on each other's broadcasts).
#
# [channels.slack]
