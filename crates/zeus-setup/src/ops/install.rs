...
pub async fn run(
    ...
    // Step 6: Install system service
    tx.send(ProgressEvent::StepStarted {
        name: "Install service".into(),
        index: 5,
        total: total_steps,
    })
    .await?;

    if let Err(e) = crate::ops::service::run("install", tx.clone()).await {
        // Non-fatal: service install can fail (e.g. no sudo, container env)
        // The gateway can still be started manually.
        tx.send(ProgressEvent::LogLine(format!(
            "Service install skipped: {}",
            e
        )))
        .await?;
    }
    ...
    tx.send(ProgressEvent::StepCompleted {
        name: "Install service".into(),
        message: "System service configured".into(),
    })
    .await?;
    ...
    // Step 8: Send first message via gateway (if healthy)
    tx.send(ProgressEvent::StepStarted {
...
async fn send_first_message(zeus_version: &str, tx: &mpsc::Sender<ProgressEvent>) {
    // Read port from env (set by launchd plist / systemd unit) or default to 8080
    let gateway_port: u16 = std::env::var("ZEUS_GATEWAY_PORT")
        .ok()
        .and_then(|p| p.parse().ok())
        .unwrap_or(8080);
    let gateway_health = format!("http://127.0.0.1:{}/health", gateway_port);
    let gateway_send = format!("http://127.0.0.1:{}/v1/channels/send", gateway_port);
