...
mkdir -p "$PLIST_DIR" && \
cat > "$PLIST_DIR/ai.zeus.gateway.plist" << 'PLIST_EOF'
{}
PLIST_EOF
sed -i '' "s|__ZEUS_BIN__|$ZEUS_BIN|g; s|__HOME__|$HOME|g" "$PLIST_DIR/ai.zeus.gateway.plist""#,
            plist,
        );
        ...
        if status.success() {
            ...
            let _ = ssh_cmd(
                &ssh_target,
                "launchctl unload ~/Library/LaunchAgents/ai.zeus.gateway.plist 2>/dev/null; \
...
zeus_gateway_config="${zeus_gateway_home}/.zeus/config.toml"
...
zeus_gateway_start()
{
    ...
    /usr/sbin/daemon -f -p ${pidfile} -o ${zeus_gateway_logfile} \
        -u ${zeus_gateway_user} \
        /usr/bin/env HOME=${zeus_gateway_home} \
        ZEUS_HOME=${zeus_gateway_home}/.zeus \
        ${procname} gateway
}
...
sudo sysrc zeus_gateway_enable="YES""#,
        ...
        if status.success() {
            tx.send(ProgressEvent::LogLine(format!(
                "[{}] rc.d service installed and enabled",
                target.name,
            )))
            .await?;
        }
...
fn stop_service_cmd(target: &FleetNode) -> String {
    if target.is_macos() {
        "launchctl unload ~/Library/LaunchAgents/ai.zeus.gateway.plist 2>/dev/null || true".into()
    } else if target.is_freebsd() {
...
fn start_service_cmd(target: &FleetNode) -> String {
    if target.is_macos() {
        "launchctl load ~/Library/LaunchAgents/ai.zeus.gateway.plist 2>/dev/null || true".into()
    } else if target.is_freebsd() {
