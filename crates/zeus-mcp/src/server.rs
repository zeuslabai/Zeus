...
impl McpStdio {
    ...
    pub async fn run(config: &zeus_core::Config) -> anyhow::Result<()> {
        ...
        std::thread::spawn(move || {
            const IDLE_TIMEOUT_SECS: u64 = 31_536_000; // 365 days — mewndude: "do not crash or kill the mcp" — 5min was too aggressive, killed server during idle conversation periods
            loop {
