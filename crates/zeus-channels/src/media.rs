...
/// Default TTL for on-disk cache entries (24h)
...
impl MediaPipeline {
    ...
    // ── Cut 2 (#88/#85): URL fetch+cache for ContentBlock::Image ──────
    //
    // TODO(cut-3+): promote MediaPipeline to gateway state singleton if
    // image flow becomes hot path — currently per-call instantiation in
    // Discord/Slack send_rich overrides.

    /// Stable cache key for a URL — sha256 hex of the URL string.
    /// Pure function — no I/O, deterministic, safe across processes.
    ...
    /// Path under the cache subdir for a given URL hash + extension hint.
    fn cache_path(&self, key: &str, ext: &str) -> PathBuf {
        self.media_dir.join("cache").join(format!("{key}.{ext}"))
    }

    /// Path where the sidecar MIME hint is stored.
    fn cache_meta_path(&self, key: &str) -> PathBuf {
        self.media_dir.join("cache").join(format!("{key}.mime"))
    }

    /// Look up an existing fresh (within TTL) cache entry.
    /// Returns `(path, mime)` on hit; `None` on miss / expired / corrupt.
    async fn lookup_cache(&self, key: &str) -> Option<(PathBuf, String)> {
        let meta_raw = fs::read_to_string(self.cache_meta_path(key)).await.ok()?;
        // Sidecar format: "<mime>|<ext>"
        let (mime, ext) = match meta_raw.split_once('|') {
            Some((m, e)) => (m.to_string(), e.to_string()),
            None => (meta_raw.clone(), "bin".to_string()),
        };
        let data_path = self.cache_path(key, &ext);
        let meta = fs::metadata(&data_path).await.ok()?;
    ...
    /// Fetch a URL, returning a cached on-disk path + MIME type.
    ///
    /// On cache hit (entry exists + within TTL), returns the cached path
    /// without network I/O. On miss, downloads via [`Self::download_url`],
    /// persists under `media_dir/cache/<sha256>.<ext>`, and returns the
    /// freshly-written path.
    ///
    /// MIME detection: prefer magic-byte sniff ([`detect_mime_type`]);
    /// fall back to the HTTP content-type header from the download.
    pub async fn fetch_cached(&self, url: &str) -> Result<(PathBuf, String)> {
        ...
        // Miss → download
        let (bytes, http_mime) = self.download_url(url).await?;
        let sniffed = detect_mime_type(&bytes);
        ...
        // Persist
        fs::create_dir_all(self.media_dir.join("cache")).await?;
        let data_path = self.cache_path(&key, ext);
        fs::write(&data_path, &bytes).await?;
        // Sidecar: "<mime>|<ext>"
        fs::write(self.cache_meta_path(&key), format!("{mime}|{ext}")).await?;

        Ok((data_path, mime))
    }
}

/// Map MIME → preferred file extension for cached entries.
/// Conservative — only common image/media types; unknowns → "bin".
...
mod tests {
    ...
    // ── Cut 2: fetch_cached / cache-layer tests ───────────────────────

    #[test]
    fn test_cache_key_deterministic() {
        let a = MediaPipeline::cache_key("https://example.com/img.png");
        let b = MediaPipeline::cache_key("https://example.com/img.png");
        let c = MediaPipeline::cache_key("https://example.com/other.png");
        assert_eq!(a, b);
        assert_ne!(a, c);
    ...
    fn test_mime_to_ext_coverage() {
        ...
        assert_eq!(mime_to_ext("totally/unknown"), "bin");
    }

    #[tokio::test]
    async fn test_fetch_cached_hit_does_not_redownload() {
        // Seed the cache with a known entry, then call fetch_cached
        // pointed at a non-routable URL. The hit path returns cached data
        // without touching network; if it tried to download from
        // "http://127.0.0.1:1/never", reqwest would fail.
        let tmp = tempfile::tempdir().unwrap();
        let pipe = MediaPipeline::new(tmp.path().to_path_buf());

        let url = "http://127.0.0.1:1/never";
        let key = MediaPipeline::cache_key(url);
        let cache_dir = tmp.path().join("cache");
        tokio::fs::create_dir_all(&cache_dir).await.unwrap();

        // Write fake PNG bytes (just the magic header) + sidecar
        let png_magic = [0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A];
        let data_path = cache_dir.join(format!("{key}.png"));
        tokio::fs::write(&data_path, png_magic).await.unwrap();
        tokio::fs::write(cache_dir.join(format!("{key}.mime")), "image/png|png")
            .await
            .unwrap();

        let (got_path, got_mime) = pipe.fetch_cached(url).await.unwrap();
        assert_eq!(got_path, data_path);
        assert_eq!(got_mime, "image/png");
    }

    #[tokio::test]
    async fn test_fetch_cached_miss_then_hit_mime_consistency() {
        // Even without going to network, exercise the lookup_cache path
        // for mime-detection consistency: seed a JPEG entry, fetch_cached
        // returns the sniffed-mime ("image/jpeg") from the sidecar.
        let tmp = tempfile::tempdir().unwrap();
        let pipe = MediaPipeline::new(tmp.path().to_path_buf());

        let url = "http://127.0.0.1:1/jpeg";
        let key = MediaPipeline::cache_key(url);
        let cache_dir = tmp.path().join("cache");
        tokio::fs::create_dir_all(&cache_dir).await.unwrap();

        let jpeg_magic = [0xFF, 0xD8, 0xFF, 0xE0, 0x00, 0x10];
        let data_path = cache_dir.join(format!("{key}.jpg"));
        tokio::fs::write(&data_path, jpeg_magic).await.unwrap();
        tokio::fs::write(cache_dir.join(format!("{key}.mime")), "image/jpeg|jpg")
            .await
            .unwrap();

        let (path1, mime1) = pipe.fetch_cached(url).await.unwrap();
        let (path2, mime2) = pipe.fetch_cached(url).await.unwrap();
        assert_eq!(path1, path2);
        assert_eq!(mime1, mime2);
        assert_eq!(mime1, "image/jpeg");
        // Magic-byte sniff must agree with sidecar
        let bytes = tokio::fs::read(&path1).await.unwrap();
        assert_eq!(detect_mime_type(&bytes), "image/jpeg");
    }
}
