...
struct Handler {
    ...
    /// Bot user ID decoded from the configured token (#234). Compared against
    /// the runtime identity reported in the gateway READY event; a mismatch
    /// means the configured token belongs to a different bot than the one we
    /// believe we are, which silently causes mis-mentions. We fail LOUD on it.
    expected_bot_id: Option<u64>,
...
/// Decide whether a Discord message is "addressed" to this bot.
///
/// Returns `true` when any of these hold:
/// - DM (always addressed)
/// - Group message with @mention or slash-command (`is_mention`)
/// - Group message from a peer bot AND `allow_bots = "on"` (#69 fix:
///   coordinator titans need to cook on peer-bot status reports even
///   without an explicit @mention)
///
/// Pure function — extracted from `Handler::message` for unit testing.
#[inline]
fn compute_is_addressed(
    is_dm: bool,
    is_mention: bool,
    author_is_bot: bool,
    allow_bots: AllowBotsMode,
) -> bool {
    let peer_bot_with_on_mode = author_is_bot && matches!(allow_bots, AllowBotsMode::On);
    is_dm || is_mention || peer_bot_with_on_mode
}
...
impl EventHandler for Handler {
    async fn ready(&self, _ctx: Context, ready: Ready) {
        ...
        // #234: fail LOUD on token/identity mismatch. The token we were
        // configured with decodes to `expected_bot_id`; the gateway just told
        // us we're actually `runtime_id`. If they disagree, the running
        // identity does not match the configured token — every `<@id>` mention
        // we emit will point at the wrong bot. Surface it screaming instead of
        // silently mis-mentioning.
        let runtime_id = ready.user.id.get();
        if let Some(expected) = self.expected_bot_id
            && expected != runtime_id
        {
            tracing::error!(
                expected_bot_id = expected,
                runtime_bot_id = runtime_id,
                runtime_username = %ready.user.name,
                "╔══════════════════════════════════════════════════════════════╗\n\
                 ║ DISCORD TOKEN / IDENTITY MISMATCH — FAILING LOUD (#234)        ║\n\
                 ╠══════════════════════════════════════════════════════════════╣\n\
                 ║ The configured bot token decodes to ID {expected}, but the\n\
                 ║ Discord gateway authenticated us as ID {runtime_id} ({runtime}).\n\
                 ║ These MUST match. A mismatch means the token in your config\n\
                 ║ belongs to a DIFFERENT bot than this agent's identity — every\n\
                 ║ @-mention this agent emits will point at the wrong bot.\n\
                 ║ FIX: align [agent] identity with the token in ~/.zeus/.env\n\
                 ║ (DISCORD_BOT_TOKEN), or swap the token to match this identity.\n\
                 ╚══════════════════════════════════════════════════════════════╝",
                expected = expected,
                runtime_id = runtime_id,
                runtime = ready.user.name,
            );
        }

        // Store our bot user ID and username for mention detection.
        // Trust the runtime identity from READY over the pre-decoded token guess.
        *self.bot_user_id.write().await = Some(runtime_id);
    ...
    async fn message(&self, _ctx: Context, msg: DiscordMessage) {
        ...
        // #69: when allow_bots=on, peer-bot messages (non-mention) are still
        // "addressed" so coordinator titans cook on them. Pure helper for testability.
        let is_addressed =
            compute_is_addressed(is_dm, is_mention, msg.author.bot, self.allow_bots);
        if is_group {
...
impl ChannelAdapter for DiscordAdapter {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let handler = Handler {
                    ...
                    expected_bot_id: pre_decoded_bot_id,
    ...
    /// Discord is Tier-1 — supports embeds, file attachments, inline images.
    ...
    ///
    /// Pipeline:
    ///   1. `rich_render::render_discord(response)` → `DiscordRender`
    ///      (embed + file blobs + optional top-level content)
    ///   2. For each `ContentBlock::Image` URL in the response, fetch via
    ///      per-call `MediaPipeline::fetch_cached` and append as attachment
    ///   3. Dispatch via existing `send_embed` / `send_with_attachments`
    ///
    /// On any sub-step error, falls back to the default text-degradation
    /// path (`self.send(to, &response.to_text())`).
    async fn send_rich(
        &self,
        to: &ChannelSource,
        ...
        let channel_id_str = to
            .chat_id
            .as_deref()
            .ok_or_else(|| Error::channel("Discord send_rich requires chat_id"))?;
        let channel_id: u64 = channel_id_str
            .parse()
            .map_err(|_| Error::channel(format!("Invalid Discord channel_id: {channel_id_str}")))?;

        let rendered = crate::rich_render::render_discord(response);

        // Resolve any ContentBlock::Image URLs into downloaded file attachments.
        // Per-call MediaPipeline; TODO(cut-3+) promote to gateway singleton.
        let mut files: Vec<(String, Vec<u8>)> = rendered
            .files
            .iter()
            .map(|(name, bytes, _cap)| (name.clone(), bytes.clone()))
            .collect();

        let pipeline = crate::media::MediaPipeline::default();
        for block in &response.blocks {
            if let crate::rich::ContentBlock::Image { url, alt, .. } = block {
                match pipeline.fetch_cached(url).await {
                    Ok((path, mime)) => {
                        if let Ok(bytes) = tokio::fs::read(&path).await {
                            let ext = crate::media::mime_to_ext(&mime);
                            // Sanitize alt → filename
                            let stem: String = alt
                                .chars()
        ...
        // Dispatch: prefer embed-aware path when we have an embed.
        match (rendered.embed.as_ref(), files.is_empty()) {
            (Some(embed), true) => {
                self.send_embed(channel_id, rendered.content.as_deref(), embed)
                    .await
            }
            (None, false) => {
                let content = rendered.content.as_deref().unwrap_or("");
                self.send_with_attachments(channel_id, content, files).await
            }
            (Some(embed), false) => {
                // Embed + files: send embed first, then attachments with empty content
                self.send_embed(channel_id, rendered.content.as_deref(), embed)
                    .await?;
                self.send_with_attachments(channel_id, "", files).await
            }
            (None, true) => {
                // No embed, no files — degrade to plain text
                let text = rendered
                    .content
                    .unwrap_or_else(|| response.to_text());
...
mod tests {
    ...
    // -- #69: allow_bots=on peer-bot trigger gap --
    //
    // Coordinator titans (e.g. Zeus100) configure `allow_bots = "on"` so they
    // cook on peer-bot status reports without needing explicit @mentions. The
    // prior `is_addressed = is_dm || is_mention` formula silently dropped these,
    // breaking the coordinator use-case. These tests pin the corrected behavior.

    #[test]
    fn test_peer_bot_addressed_when_allow_bots_on() {
        // Group + bot author + no mention + allow_bots=On → is_addressed=true.
        let is_dm = false;
        let is_mention = false;
        let author_is_bot = true;
        let addressed =
            compute_is_addressed(is_dm, is_mention, author_is_bot, AllowBotsMode::On);
        assert!(
            addressed,
            "peer-bot message in group must be addressed when allow_bots=On"
        );
    }

    #[test]
    fn test_peer_bot_not_addressed_when_allow_bots_mentions() {
        // Regression guard: same setup but allow_bots=Mentions → is_addressed=false.
        let is_dm = false;
        let is_mention = false;
        let author_is_bot = true;
        let addressed =
            compute_is_addressed(is_dm, is_mention, author_is_bot, AllowBotsMode::Mentions);
        assert!(
            !addressed,
            "peer-bot message in group must NOT be addressed when allow_bots=Mentions"
        );
    }

    #[test]
    fn test_human_unaddressed_group_msg_still_unaddressed_with_allow_bots_on() {
        // Belt-and-suspenders: human author + no mention + allow_bots=On → false.
        // The peer-bot clause must not leak into human-author path.
        let addressed = compute_is_addressed(false, false, false, AllowBotsMode::On);
        assert!(!addressed, "human chatter must remain unaddressed regardless of allow_bots");
    }

    #[test]
    fn test_dm_always_addressed_regardless_of_allow_bots() {
        // DM short-circuit unchanged across all allow_bots modes.
        for mode in [AllowBotsMode::Off, AllowBotsMode::Mentions, AllowBotsMode::On] {
            assert!(compute_is_addressed(true, false, false, mode));
            assert!(compute_is_addressed(true, false, true, mode));
        }
    }

    // ── Cut 2 (#88/#85): send_rich override tests ─────────────────────
    //
    // The override dispatches through `send_embed` / `send_with_attachments`,
    // both of which require a live serenity Http. We can't mock that here, so
    // the tests cover the *pure-function* part of the pipeline: rendering
    // contract, capabilities advertisement, and filename-sanitization logic.
    // Full transport-layer integration is exercised in the gateway-side
    // dispatch tests (sub-item 3) where a mock ChannelAdapter is used.

    use crate::rich::{ContentBlock, RichResponse, ChannelCapabilities};
    use crate::rich_render::render_discord;

    #[test]
    fn test_send_rich_caps_advertises_tier_1() {
        // Sub-item 1 acceptance: DiscordAdapter advertises full rich support.
        // We verify the trait-method output matches TIER_1 in shape.
    ...
    #[test]
    fn test_send_rich_plain_text_render_path() {
        // Plain-text RichResponse → render_discord produces content/embed,
        // no files. The override would dispatch through send_embed.
        let r = RichResponse::new()
            .text("Just some words.");
        let rendered = render_discord(&r);
        assert!(rendered.files.is_empty(), "plain text must not yield file attachments");
        // Embed present OR content present
        assert!(rendered.embed.is_some() || rendered.content.is_some());
    }

    #[test]
    fn test_send_rich_embed_only_path() {
        // Title + body → render_discord produces an embed.
        let r = RichResponse::new()
            .title("Header")
            .text("Body text.");
        let rendered = render_discord(&r);
        assert!(rendered.embed.is_some(), "title+body must produce embed");
        let embed = rendered.embed.unwrap();
        assert_eq!(embed.title.as_deref(), Some("Header"));
    }

    #[test]
    fn test_send_rich_mixed_fragment_filename_sanitization() {
        // The send_rich override sanitizes ContentBlock::Image alt → filename
        // by replacing non-alphanumeric chars with '_' and capping at 40.
        // Verify the sanitizer produces a valid filename stem.
        let alt = "My Cool/Image:With Bad?Chars*!";
        let stem: String = alt
            .chars()
            .map(|c| if c.is_ascii_alphanumeric() { c } else { '_' })
            .take(40)
            .collect();
        assert_eq!(stem, "My_Cool_Image_With_Bad_Chars__");
        assert!(stem.chars().all(|c| c.is_ascii_alphanumeric() || c == '_'));
        assert!(stem.len() <= 40);

        // Empty alt → falls through to "image.<ext>" branch
        let empty_alt = "";
        let empty_stem: String = empty_alt
            .chars()
            .map(|c| if c.is_ascii_alphanumeric() { c } else { '_' })
            .take(40)
            .collect();
        assert!(empty_stem.is_empty());

        // Cross-check renderer integration: response with an Image block
        // produces a renderable shape (caption/alt becomes embed.image_url
        // when single-image — verified by render_discord's existing tests).
        let r = RichResponse::new()
            .text("See:")
            .block(ContentBlock::image("https://example.com/x.png", "alt", None::<&str>));
        let rendered = render_discord(&r);
        // Single Image block → renderer puts it into embed.image_url
        assert!(rendered.embed.is_some());
    }
}
