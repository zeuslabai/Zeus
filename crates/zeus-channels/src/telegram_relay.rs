                                    ...
                                    if msg["from"]["is_bot"].as_bool() == Some(true) {
                                        ...
                                        match allow_bots {
                                            ...
                                            AllowBotsMode::Mentions => {
                                                ...
                                                // Telegram for hours.
                                                //
                                                // Reverting Patch A per the operator's directive 2026-05-06. The original
...
async fn download_telegram_file(
    ...
    //   <workspace>/channels/telegram/received/<date>/<message_id>_<safe_name>
    // The helper date-nests and sanitizes the filename (path-traversal safe).
    let base_name = match original_name {
        Some(name) => format!("{}_{}", message_id, name),
        None => format!("{}_{}.{}", file_type, message_id, ext),
    };
    let path = zeus_core::received_file_path("telegram", &base_name)
