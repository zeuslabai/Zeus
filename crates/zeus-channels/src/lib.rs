...
impl ChannelManager {
    ...
    ///
    /// The LLM heartbeat system produces `HEARTBEAT_OK` responses when no
    /// action is needed.  These are internal housekeeping signals — they must
    /// never leak to users on external channels (Discord, Telegram, Slack,
    /// etc.).  The TUI is exempt because it is a local developer interface.
    ...
    pub async fn send(&self, to: &ChannelSource, content: &str) -> Result<()> {
        if Self::should_suppress_heartbeat_ok(to, content) {
            tracing::debug!(channel = to.channel_type(), "Suppressed HEARTBEAT_OK outbound");
            return Ok(());
        }
    ...
    pub async fn send_as(
        ...
        if Self::should_suppress_heartbeat_ok(to, content) {
            tracing::debug!(channel = to.channel_type(), "Suppressed HEARTBEAT_OK outbound (send_as)");
            return Ok(());
        }
        let adapter = self.find_adapter(to).ok_or_else(|| {
            zeus_core::Error::Channel(format!("No adapter for channel: {}", to.channel_type()))
    ...
    pub async fn send_threaded(
        ...
        if Self::should_suppress_heartbeat_ok(to, content) {
            tracing::debug!(channel = to.channel_type(), "Suppressed HEARTBEAT_OK outbound (send_threaded)");
            return Ok(());
        }
        let adapter = self.find_adapter(to).ok_or_else(|| {
            zeus_core::Error::Channel(format!("No adapter for channel: {}", to.channel_type()))
