...
const OfficeTab = ({ focusedAgent, setFocusedAgent }) => {
  ...
  const agents = [
    { id: "zeus100", name: "Zeus100", zone: "Engineering", x: 6, y: 4, status: "coding", task: "implementing onboarding wizard", trust: 95, color: C.accent, channel: "local" },
    { id: "zeus112", name: "Zeus112", zone: "Engineering", x: 12, y: 5, status: "reviewing PR", task: "reviewing #2847", trust: 89, color: C.accent, channel: "local" },
    { id: "zeus106", name: "zeus106", zone: "Engineering", x: 9, y: 7, status: "running tests", task: "cargo test --workspace", trust: 92, color: C.accent, channel: "discord" },
    { id: "zeus107", name: "zeus107", zone: "Comms", x: 26, y: 5, status: "monitoring CI", task: "watching pipelines", trust: 91, color: C.green, channel: "discord" },
    { id: "molty", name: "molty", zone: "Comms", x: 32, y: 4, status: "idle", task: "—", trust: 78, color: C.cyan, channel: "discord" },
    { id: "zeusmkt", name: "ZeusMkt", zone: "Research", x: 50, y: 4, status: "browsing", task: "competitor analysis", trust: 85, color: C.amber, channel: "discord" },
    { id: "fbsd1", name: "fbsd1", zone: "Research", x: 56, y: 6, status: "thinking", task: "synthesizing notes", trust: 88, color: C.purple, channel: "discord" },
    { id: "fbsd2", name: "fbsd2", zone: "Break", x: 18, y: 14, status: "idle", task: "—", trust: 82, color: C.dim, channel: "discord" },
  ];
...
const PantheonTab = ({ selectedMission, setSelectedMission }) => {
  const missions = [
    { id: "m1", name: "v0.4.7 release prep", status: "active", progress: 68, agents: 4, lead: "Zeus100", started: "2h ago" },
    { id: "m2", name: "Onboarding wizard impl", status: "planning", progress: 12, agents: 2, lead: "Zeus112", started: "30m ago" },
    { id: "m3", name: "Fleet shakedown audit", status: "reviewing", progress: 95, agents: 3, lead: "zeus107", started: "yesterday" },
    { id: "m4", name: "Q1 marketing campaign", status: "active", progress: 42, agents: 2, lead: "ZeusMkt", started: "3 days ago" },
    { id: "m5", name: "DGX Spark integration", status: "completed", progress: 100, agents: 5, lead: "zeus106", started: "1 week ago" },
  ...
  // Live event stream
  const events = [
    { t: "14:32:18", agent: "Zeus112", event: "started step 3/6", color: C.green },
    { t: "14:31:45", agent: "Zeus100", event: "approved plan card #847", color: C.amber },
    { t: "14:30:12", agent: "zeus106", event: "completed cargo test (7,801 passed)", color: C.green },
    { t: "14:28:03", agent: "Zeus112", event: "submitted PR #2847", color: C.cyan },
    { t: "14:25:51", agent: "zeus107", event: "watchdog tick · all green", color: C.dim },
    { t: "14:22:30", agent: "Zeus100", event: "assigned task to Zeus112", color: C.accent },
    { t: "14:20:08", agent: "Zeus112", event: "joined war room #v047-prep", color: C.cyan },
  ];
        ...
        <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", borderRight: `1px solid ${C.muted}` }}>
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "10px 14px", display: "flex", flexDirection: "column", gap: 6 }}>
              {[
                { agent: "Zeus100", time: "14:20", text: "Starting v0.4.7 release prep. Assembling team.", color: C.accent },
                { agent: "Zeus112", time: "14:21", text: "Copy. I'll take wizard impl + tests.", color: C.accent },
                { agent: "zeus106", time: "14:22", text: "I can run the integration suite once code lands.", color: C.accent },
                { agent: "zeus107", time: "14:23", text: "CI watcher armed. Will alert if anything regresses.", color: C.green },
                { agent: "Zeus112", time: "14:28", text: "PR #2847 submitted. Phase 1 widgets done.", color: C.accent },
                { agent: "zeus106", time: "14:30", text: "✓ cargo test --workspace · 7,801 passed", color: C.accent },
                { agent: "Zeus100", time: "14:31", text: "Approving plan card for Phase 2.", color: C.accent },
                { agent: "Zeus112", time: "14:32", text: "On it. Step 3/6 in progress.", color: C.accent },
              ].map((msg, i) => (
...
const ApprovalsTab = () => {
  const pending = [
    { id: "ap_47", agent: "Zeus112", tool: "shell", args: 'rm -rf node_modules && npm install', reason: "rm -rf flagged by Aegis — destructive operation", risk: "high", time: "32s ago" },
    { id: "ap_46", agent: "Zeus100", tool: "web_fetch", args: 'https://api.unallowlisted.com/data', reason: "URL not in allowlist", risk: "medium", time: "1m ago" },
    { id: "ap_45", agent: "zeus106", tool: "apply_patch", args: '@@ -147,3 +147,8 @@ ... (47 lines diff)', reason: "patch touches /etc/hosts", risk: "high", time: "3m ago" },
    { id: "ap_44", agent: "ZeusMkt", tool: "discord_send", args: '{"channel": "#announcements", "content": "v0.4.7 ships tomorrow!"}', reason: "channel announcements requires approval", risk: "low", time: "8m ago" },
  ];
...
const AdvancedSubview = ({ id }) => {
  ...
  if (id === "agents") {
    const agents = [
      { name: "Zeus100", host: "MacBook Pro M5", role: "Coordinator", local: true, status: "active", channels: 4 },
      { name: "Zeus112", host: "MacBook Pro", role: "Backend & Architecture", local: false, status: "active", channels: 2 },
      { name: "molty", host: "Mac Studio VM", role: "Experimental", local: false, status: "idle", channels: 1 },
      { name: "zeus106", host: "Mac Studio M1 Ultra", role: "Backend (dual)", local: false, status: "active", channels: 3 },
      { name: "ZeusMarketing", host: "Mac Mini M2", role: "Marketing & Content", local: false, status: "active", channels: 5 },
      { name: "zeus107", host: "Mac Mini M4 Pro", role: "Security & CI", local: false, status: "active", channels: 2 },
      { name: "fbsd1", host: "FreeBSD 15.0", role: "Fleet Provisioner", local: false, status: "active", channels: 1 },
      { name: "fbsd2", host: "FreeBSD 15.0", role: "Web Platform", local: false, status: "active", channels: 1 },
    ];
    return (
  ...
  if (id === "economy") {
    return (
      <div style={{ flex: 1, padding: "12px 18px", overflowY: "auto" }}>
        ...
        {[
          { t: "14:32", agent: "Zeus112", action: "purchased", item: "advanced-codegen skill", amt: "-$ 4.99", color: C.amber },
          { t: "13:15", agent: "Zeus100", action: "received", item: "x402 settlement · search task", amt: "+$ 0.18", color: C.green },
          { t: "12:48", agent: "Zeus100", action: "paid", item: "MiniMax inference (47k tok)", amt: "-$ 0.04", color: C.red },
          { t: "11:30", agent: "ZeusMkt", action: "received", item: "x402 · content generation", amt: "+$ 2.40", color: C.green },
        ].map((tx, i) => (
...
export default function ZeusProductionTUI() {
  ...
  // Other tab state
  const [focusedAgent, setFocusedAgent] = useState("zeus100");
