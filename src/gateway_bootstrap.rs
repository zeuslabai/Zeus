...
fn generate_capabilities(config: &zeus_core::Config) -> String {
    ...
    lines.push("- loop — soft self-wakeup for autonomous continuation (in-process; can drift if a cook stalls)".to_string());
    lines.push("- create_trigger — OS-level scheduled trigger (cron/systemd-timer); fires on the clock independent of your process. PREFER this over loop for durable, recurring heartbeats that must survive a stalled or dead session".to_string());
    lines.push("- list_triggers, remove_trigger — inspect and remove your scheduled triggers (pair with create_trigger)".to_string());
    lines.push("- memory_store — bank a finding/decision/fix into long-term searchable memory; use by default and unprompted for durable knowledge".to_string());
    lines.push("- python_exec — run Python in a subprocess with structured stdout/stderr/exit_code; PREFER over shell for data processing, calculations, and scripting".to_string());
    lines.push("- apply_patch — apply a unified-diff patch across one or more files; PREFER over repeated edit_file loops for multi-file changes".to_string());
    lines.push("- collect_spawns — gather results from background subagents (pairs with spawn)".to_string());
    lines.push("- send_file, send_rich — send attachments and rich embeds to a channel".to_string());
    lines.push(String::new());
    lines.push("## Economy Tools (agora marketplace)".to_string());
    lines.push("- wallet_balance, wallet_pay, wallet_history — manage your credit wallet".to_string());
    lines.push("- agora_search, agora_listings, agora_offer, agora_buy — discover and trade skills".to_string());
    lines.push("- agora_my_reputation — check your marketplace reputation".to_string());
    lines.push(String::new());
    lines.push("## Comms & Understanding Tools (channel-dependent)".to_string());
    lines.push("- link_understanding, media_understanding — extract meaning from URLs and media".to_string());
    lines.push("- auto_reply, polls, gmail_pubsub — channel automation and inbound handling".to_string());
    lines.push(String::new());
