...
pub async fn start_x_relay(
    ...
    match zeus_channels::XAdapter::new(x_config).await {
        Ok(adapter) => {
            ...
            // #100: build fan-out sinks once (hoisted before the loop). Empty
            // when no [channels.x_twitter].fanout is configured → zero overhead,
            // relay behaves exactly as before.
            let fanout_sinks = build_fanout_sinks(config, &xt.fanout).await;
            if !fanout_sinks.is_empty() {
                info!(
                    "X fanout: reposting polled X content to {} target(s): {}",
                    fanout_sinks.len(),
                    fanout_sinks
                        .iter()
            ...
            tasks.push(tokio::spawn(async move {
                ...
                while let Some(msg) = rx.recv().await {
                    ...
                    // #100 loop-prevention: never re-ingest our own fan-out
                    // reposts (content carrying the attribution marker).
                    if is_fanout_echo(&msg.content) {
                        debug!("X fanout: skipping re-ingested repost (loop guard)");
                        continue;
                    }

                    // #100: fan out the polled X content to configured channels
                    // with author attribution. Best-effort — failures are logged
                    // and never block the inbound relay path below.
                    if !fanout_sinks.is_empty() {
                        let text = format_fanout_text(&msg.source.user_id, &msg.content);
                        for sink in &fanout_sinks {
                            if let Err(e) = sink.adapter.send(&sink.dest, &text).await {
                                error!("X fanout: failed to repost to {}: {}", sink.label, e);
                            } else {
                                debug!("X fanout: reposted to {}", sink.label);
                            }
                        }
                    }
...
/// Attribution marker prefixed to fanned-out X content (#100).
///
/// Doubles as the loop-prevention sentinel: any inbound X content already
/// carrying this marker is a re-ingested repost and is skipped, so the fan-out
/// can never feed its own output back into the relay.
const X_FANOUT_MARKER: &str = "🔁 via @";

/// A resolved fan-out target: an outbound adapter + the destination source.
struct FanoutSink {
    adapter: Arc<dyn zeus_channels::ChannelAdapter>,
    dest: zeus_channels::ChannelSource,
    label: String,
}

/// Build outbound fan-out sinks from the configured `[channels.x_twitter].fanout`
/// targets, resolving each against the available TG/Discord channel configs.
///
/// Unsupported channels or targets whose backing channel config is absent are
/// logged and skipped — fan-out is best-effort and never blocks the relay.
async fn build_fanout_sinks(
    config: &Config,
    targets: &[zeus_core::FanoutTarget],
) -> Vec<FanoutSink> {
    use zeus_channels::ChannelAdapter;

    let mut sinks: Vec<FanoutSink> = Vec::new();
    let channels = config.channels.as_ref();

    for target in targets {
        if !target.is_supported() {
            error!(
                "X fanout: target '{}' (chat {}) unsupported or missing chat_id — skipping",
                target.channel, target.chat_id
            );
            continue;
        }
        match target.channel_kind().as_str() {
            "telegram" => {
                let Some(tg) = channels.and_then(|c| c.telegram.as_ref()) else {
                    error!("X fanout: telegram target configured but [channels.telegram] absent — skipping");
                    continue;
                };
                let Some(token) = tg.bot_token.clone().filter(|t| !t.is_empty()) else {
                    error!("X fanout: [channels.telegram] has no bot_token — skipping telegram target");
                    continue;
                };
                let bot_config = zeus_channels::telegram_bot::TelegramBotConfig::new(token);
                let adapter: Arc<dyn ChannelAdapter> =
                    Arc::new(zeus_channels::telegram_bot::TelegramBotAdapter::new(bot_config));
                let mut dest = zeus_channels::ChannelSource::new("telegram", &target.chat_id);
                dest.chat_id = Some(target.chat_id.clone());
                sinks.push(FanoutSink {
                    adapter,
                    dest,
                    label: format!("telegram:{}", target.chat_id),
                });
            }
            "discord" => {
                let Some(dc) = channels.and_then(|c| c.discord.as_ref()) else {
                    error!("X fanout: discord target configured but [channels.discord] absent — skipping");
                    continue;
                };
                if dc.token.is_empty() {
                    error!("X fanout: [channels.discord] has no token — skipping discord target");
                    continue;
                }
                let disc_config = zeus_channels::DiscordConfig {
                    bot_token: dc.token.clone(),
                    ..Default::default()
                };
                match zeus_channels::DiscordAdapter::new(disc_config).await {
                    Ok(adapter) => {
                        let mut dest =
                            zeus_channels::ChannelSource::new("discord", &target.chat_id);
                        dest.chat_id = Some(target.chat_id.clone());
                        sinks.push(FanoutSink {
                            adapter: Arc::new(adapter),
                            dest,
                            label: format!("discord:{}", target.chat_id),
                        });
                    }
                    Err(e) => {
                        error!("X fanout: failed to build discord adapter: {} — skipping", e);
                    }
                }
            }
            other => {
                error!("X fanout: unknown channel kind '{}' — skipping", other);
            }
        }
    }
    sinks
...
/// Format an X message for fan-out with author/source attribution.
///
/// The leading [`X_FANOUT_MARKER`] doubles as the loop-prevention sentinel.
fn format_fanout_text(author: &str, content: &str) -> String {
    format!("{X_FANOUT_MARKER}{author} on X:\n{content}")
}

/// Whether inbound X content is a re-ingested fan-out repost (loop guard).
fn is_fanout_echo(content: &str) -> bool {
    content.contains(X_FANOUT_MARKER)
}
...
// ── Tests (#100 X→TG/Discord fanout) ────────────────────────────────────────
#[cfg(test)]
mod tests {
    use super::*;
    use zeus_core::FanoutTarget;

    // config_validates: only telegram/discord with a non-empty chat_id are
    // accepted; everything else is rejected before adapter construction.
    #[test]
    fn config_validates() {
        let tg = FanoutTarget {
        ...
        let dc = FanoutTarget {
            channel: "Discord".into(), // case-insensitive
            chat_id: "98765".into(),
        };
        assert!(tg.is_supported(), "telegram target should validate");
        assert!(dc.is_supported(), "discord target (mixed case) should validate");
        assert_eq!(dc.channel_kind(), "discord");

        // Unsupported channel kind.
        let slack = FanoutTarget {
            channel: "slack".into(),
            chat_id: "C123".into(),
        };
        assert!(!slack.is_supported(), "slack is not a supported fanout target");

        // Supported channel but empty/whitespace chat_id.
        let empty = FanoutTarget {
            channel: "telegram".into(),
            chat_id: "   ".into(),
        };
        assert!(!empty.is_supported(), "empty chat_id must be rejected");
    }

    // fanout_routes: attributed text is built with the marker + author + content,
    // so a downstream channel sees who/where it came from.
    #[test]
    fn fanout_routes() {
        let text = format_fanout_text("alice", "hello world");
        assert!(text.contains("alice"), "attribution must name the author");
        assert!(text.contains("hello world"), "original content must be preserved");
        assert!(
            text.starts_with(X_FANOUT_MARKER),
            "fanned-out text must carry the loop-prevention marker"
        );
    }

    // fanout_skips_loop: content carrying the attribution marker is recognized
    // as a re-ingested repost and must be skipped (no infinite fan-out loop).
    #[test]
    fn fanout_skips_loop() {
        let original = "just a normal mention @bot";
        assert!(
            !is_fanout_echo(original),
            "a fresh mention must not be treated as an echo"
        );

        let reposted = format_fanout_text("bob", "some earlier content");
        assert!(
            is_fanout_echo(&reposted),
            "our own fanned-out text must be detected as an echo and skipped"
        );
    }

    // build_fanout_sinks gracefully yields no sinks when the backing channel
    // configs are absent — fan-out never blocks or panics the relay.
    #[tokio::test]
    async fn build_sinks_skips_when_channels_absent() {
        let config = Config::default();
        let targets = vec![FanoutTarget {
            channel: "telegram".into(),
            chat_id: "-1001234".into(),
        }];
        let sinks = build_fanout_sinks(&config, &targets).await;
        assert!(
            sinks.is_empty(),
            "no [channels.telegram] config → no sink built (graceful skip)"
        );
    }
}
