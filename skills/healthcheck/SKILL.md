---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Zeus deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Zeus cron scheduling for periodic checks, or version status checks on a machine running Zeus (laptop, workstation, Pi, VPS).
---
...
Assess and harden the host running Zeus, then align it to a user-defined risk tolerance without breaking access. Use Zeus security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Zeus changes the host firewall, SSH, or OS updates; it does not.
...
5. Zeus gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Zeus can access sensitive data.
...
### 2) Run Zeus security audits (read-only)

As part of the default read-only checks, run `zeus security audit --deep`. Only offer alternatives if the user requests them:

1. `zeus security audit` (faster, non-probing)
2. `zeus security audit --json` (structured output)

Offer to apply Zeus safe defaults (numbered):

1. `zeus security audit --fix`

Be explicit that `--fix` only tightens Zeus defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Zeus version/update status (read-only)

As part of the default read-only checks, run `zeus update status`.
...
- Credential hygiene notes (location of Zeus creds, prefer disk encryption)
...
After Zeus install or first hardening pass, run at least one baseline audit and version check:

- `zeus security audit`
- `zeus security audit --deep`
- `zeus update status`

Ongoing monitoring is recommended. Use the Zeus cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `zeus cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `zeus update status`
...
Before creating, `zeus cron list` and match on exact `name`. If found, `zeus cron edit <id> ...`.
If not found, `zeus cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `zeus update status` (preferred for source checkouts and channels)
2. `npm view zeus version` (published npm version)

## Zeus command accuracy

Use only supported commands and flags:

- `zeus security audit [--deep] [--fix] [--json]`
- `zeus status` / `zeus status --deep`
- `zeus health --json`
- `zeus update status`
- `zeus cron add|list|runs|run`

Do not invent CLI flags or imply Zeus enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Zeus compaction:
