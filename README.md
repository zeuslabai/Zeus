...
- **On-chain wallet stack** — new `/v1/wallet/onchain/*` API surface (address + SOL/token balance, transaction history, devnet-guarded SPL transfer with preflight fee/balance checks) alongside a TUI overlay showing live on-chain data next to the off-chain economy balance. Zero key material ever leaves the server — public addresses and tx signatures only.
- **WebUI onboarding parity, phase 1** — wizard reordered from 14 to 20 steps to match the TUI flow (instance selector, auth summary, fallback models, channel config, gateway host/port, workspace path, voice/image providers), plus 3 added providers (OpenRouter, xAI, Sakana) bringing WebUI to the same 13-provider set as the TUI.
