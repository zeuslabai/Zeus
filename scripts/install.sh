...
#
# install_launchd_plist <repo_root> <install_dir> <log_dir>
...
if [ -d .git ]; then
    ...
    if [ "$CURRENT_BRANCH" = "main" ]; then
        git pull origin main --ff-only 2>/dev/null && ok "Pulled latest main" || warn "Pull failed (offline?)"
    else
        warn "Not on main branch (current: '$CURRENT_BRANCH') — switching to main for stable build."
        git checkout main 2>/dev/null && git pull origin main --ff-only 2>/dev/null && ok "Switched to main and pulled latest" || warn "Switch failed — building from $CURRENT_BRANCH"
    fi
fi
...
if $DO_BUILD; then
    ...
    cargo build --release -j"$BUILD_JOBS" > "$BUILD_LOG" 2>&1 &
    BUILD_PID=$!
...
if $UPDATE_ONLY; then
    ...
    case "$OS" in
        ...
        FreeBSD)
            # Ensure rc.d service is registered + rc.conf is enabled before service restart.
            # `zeus daemon install` writes /usr/local/etc/rc.d/zeus_gateway and adds
            # zeus_gateway_enable="YES" to /etc/rc.conf. Skip if already installed.
            if [ ! -f /usr/local/etc/rc.d/zeus_gateway ]; then
                info "Registering zeus_gateway rc.d service..."
                sudo zeus daemon install 2>/dev/null && ok "rc.d service installed + enabled in rc.conf" \
...
if [ -f "$ZEUS_HOME/config.toml" ]; then
    ...
    # Create a minimal config so the gateway can start.
    # The onboarding wizard will fill in the rest on first TUI launch.
...
CFGEOF
    ...
    ok "Created minimal config.toml (run 'zeus' for setup wizard)"
fi
...
if $DO_LAUNCH; then
    ...
    if $DO_LAUNCH && grep -q 'onboarding_complete = false' "$ZEUS_HOME/config.toml" 2>/dev/null; then
        if ! $WITH_WEBUI; then
            info "Skipping gateway — TUI onboarding will start it after setup"
            DO_LAUNCH=false
        fi
    fi
fi
...
if $DO_LAUNCH; then
    ...
    $GATEWAY_WAS_RUNNING && sleep 2 || true
    $GATEWAY_WAS_RUNNING && ok "Old gateway stopped — restarting"
    ...
    case "$OS" in
        ...
        FreeBSD)
            if [ -f /usr/local/etc/rc.d/zeus ]; then
                sudo sysrc zeus_enable=YES 2>/dev/null || true
                sudo service zeus restart 2>/dev/null && ok "Gateway via rc.d" || {
                    nohup zeus gateway > "$ZEUS_HOME/logs/gateway.out.log" 2> "$ZEUS_HOME/logs/gateway.err.log" &
